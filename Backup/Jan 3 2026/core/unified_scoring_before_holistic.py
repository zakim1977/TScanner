...
"""
            ...
            if squeeze_pct >= 80:
                unified_parts.append(f"⚡ BB LOADED ({squeeze_pct:.0f}%) → {exp_direction} explosion imminent!")
            elif squeeze_pct >= 60:
                unified_parts.append(f"⚡ Compression building ({squeeze_pct:.0f}%) → {exp_direction}")
            elif squeeze_pct <= 30:
                unified_parts.append(f"BB relaxed ({squeeze_pct:.0f}%)")
...
def blend_hybrid_prediction(
    ...
    #
    # OI and Price TOGETHER tell the flow story:
    ...
    # ═══════════════════════════════════════════════════════════════════════════════
        ...
        # 2H: Bullish divergence but bearish flow → CONFLICT, wait
        elif flow_dir == "BEARISH":
            final_direction = "WAIT"
            final_score = 38
            setup_type = "DIVERGENCE_CONFLICT"
            conflict_note = f"⚠️ CONFLICT: {div_desc} vs {flow_desc}. Wait for clarity."
    ...
    # ═══════════════════════════════════════════════════════════════════════════════
        ...
        # 3G: Bearish divergence but bullish flow → CONFLICT, wait
    ...
    # Boost if ML agrees with our holistic verdict
    if final_direction == "LONG" and ml_is_bullish:
        final_score += 8
        conflict_note += " | ML confirms ✅"
