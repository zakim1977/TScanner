...
class AnalysisRouter:
    ...
    def _load_ml_engines(self):
        ...
        try:
            ...
            # Only create new engine if not already loaded
            if self._ml_engine is None:
                self._ml_engine = MLEngine()
                self._hybrid_engine = HybridEngine()
            else:
                # Refresh the existing engine's model check
                self._ml_engine._load_models()
...
def get_router() -> AnalysisRouter:
    ...
    # REFRESH ML availability check (in case models were trained after startup)
    st.session_state.analysis_router._load_ml_engines()
