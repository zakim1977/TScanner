    ...
    try:
            ...
            try:
                ...
                if is_ml_available():
                    ...
                    if current_analysis_mode == AnalysisMode.ML:
                        ...
                        # Update direction based on winner
        ...
        if signal:
            try:
                ...
                # ═══════════════════════════════════════════════════════════
                try:
                    # Get Order Block data from SMC
                    ob_data = smc.get('order_blocks', {}) if smc else {}
                    
                    # Calculate ATR for reasonable pullback
                    atr = (df['High'] - df['Low']).rolling(14).mean().iloc[-1] if df is not None else 0
                    
                    # Initialize limit entry attributes
                    signal.limit_entry = None
                    signal.limit_entry_reason = ""
                    signal.limit_entry_rr_tp1 = 0.0
                    signal.has_limit_entry = False
                    ...
                    if signal.direction == 'LONG':
                        ...
                        bearish_ob_bottom = ob_data.get('bearish_ob_bottom', 0)  # Can act as support too
                        
                        # Find recent lows (last 20 candles) for better support detection
                        recent_low = df['Low'].iloc[-20:].min() if df is not None else 0
                        
                        ob_entry = 0
                        reason = ""
                        
                        # Priority 1: Use bullish OB if exists below price
                        if bullish_ob_top > 0 and bullish_ob_top < current_price:
                            ob_entry = bullish_ob_top
                            distance_pct = ((current_price - ob_entry) / current_price) * 100
                            reason = f"Bullish OB ({distance_pct:.1f}% below)"
                            
                        # Priority 2: Use bearish OB bottom as support (if below price but close)
                        elif bearish_ob_bottom > 0 and bearish_ob_bottom < current_price:
                            distance_pct = ((current_price - bearish_ob_bottom) / current_price) * 100
                            if distance_pct <= 3.0:  # Only if within 3%
                                ob_entry = bearish_ob_bottom
                                reason = f"OB Support ({distance_pct:.1f}% below)"
                        
                        # Priority 3: Use recent low if within reasonable range (5%)
                        if ob_entry == 0 and recent_low > 0 and recent_low < current_price:
                            distance_pct = ((current_price - recent_low) / current_price) * 100
                            if distance_pct <= 5.0:
                                ob_entry = recent_low
                                reason = f"Recent Low ({distance_pct:.1f}% below)"
                        
                        # Priority 4: Calculate ATR-based pullback (1.5 ATR below)
                        if ob_entry == 0 and atr > 0:
                            ob_entry = current_price - (atr * 1.5)
                            distance_pct = ((current_price - ob_entry) / current_price) * 100
                            reason = f"1.5 ATR Pullback ({distance_pct:.1f}% below)"
                        
                        # Set limit entry if we found a valid level
                        if ob_entry > 0:
                            signal.limit_entry = ob_entry
                            signal.limit_entry_reason = reason
                            signal.has_limit_entry = True
                            
                            # Calculate R:R from limit entry
                        ...
                        bullish_ob_top = ob_data.get('bullish_ob_top', 0)  # Can act as resistance too
                        
                        # Find recent highs (last 20 candles) for better resistance detection
                        recent_high = df['High'].iloc[-20:].max() if df is not None else 0
                        
                        ob_entry = 0
                        reason = ""
                        
                        # Priority 1: Use bearish OB if exists above price
                        if bearish_ob_bottom > 0 and bearish_ob_bottom > current_price:
                            ob_entry = bearish_ob_bottom
                            distance_pct = ((ob_entry - current_price) / current_price) * 100
                            reason = f"Bearish OB ({distance_pct:.1f}% above)"
                            
                        # Priority 2: Use bullish OB top as resistance (if above price but close)
                        elif bullish_ob_top > 0 and bullish_ob_top > current_price:
                            distance_pct = ((bullish_ob_top - current_price) / current_price) * 100
                            if distance_pct <= 3.0:  # Only if within 3%
                                ob_entry = bullish_ob_top
                                reason = f"OB Resistance ({distance_pct:.1f}% above)"
                        
                        # Priority 3: Use recent high if within reasonable range (5%)
                        if ob_entry == 0 and recent_high > 0 and recent_high > current_price:
                            distance_pct = ((recent_high - current_price) / current_price) * 100
                            if distance_pct <= 5.0:
                                ob_entry = recent_high
                                reason = f"Recent High ({distance_pct:.1f}% above)"
                        
                        # Priority 4: Calculate ATR-based pullback (1.5 ATR above)
                        if ob_entry == 0 and atr > 0:
                            ob_entry = current_price + (atr * 1.5)
                            distance_pct = ((ob_entry - current_price) / current_price) * 100
                            reason = f"1.5 ATR Pullback ({distance_pct:.1f}% above)"
...
with st.sidebar:
    ...
    if app_mode == "🔍 Scanner":
        ...
        # Get router to check ML availability
        router = get_router()
        ...
        if not ml_available and selected_mode in [AnalysisMode.ML, AnalysisMode.HYBRID]:
            ...
            st.caption("🤖 ML predictions with TP/SL")
        else:
            st.caption("⚡ Best of ML + Rules")
        ...
        for r in results:
            ...
            with st.expander(f"{bd['grade_emoji']} **{sig.symbol}** | {display_label} | {action_emoji} {pred_action} ({pred_score}/100)", expanded=False):
                ...
                if pred_result:
                    ...
                    # Get ML prediction and final verdict for combined learning
                    ml_pred_pulse = analysis.get('ml_prediction')
                    ...
                    # Build reason for WAIT if applicable
                    final_reason_pulse = None
                    if final_trade_dir_pulse == 'WAIT':
                        reasons_p = []
                        pos_label_p = pred_result.move_position if hasattr(pred_result, 'move_position') else 'MIDDLE'
                        pos_pct_p = pred_result.move_position_pct if hasattr(pred_result, 'move_position_pct') else 50
                        if pos_label_p == 'LATE':
                            reasons_p.append(f"LATE ({pos_pct_p:.0f}%)")
                        if ml_pred_str_pulse and ml_pred_str_pulse not in ['LONG'] and pred_result.direction in ['BULLISH', 'LEAN_BULLISH']:
                            reasons_p.append(f"ML not bullish")
                        if reasons_p:
                            final_reason_pulse = " + ".join(reasons_p)
                    ...
                    combined_learning = generate_combined_learning(
                        ...
                        at_support=ob_data.get('at_support', False),
                        at_resistance=ob_data.get('at_resistance', False),
                        # NEW: Pass final verdict from main analysis
                        final_verdict=final_trade_dir_pulse,
                        final_verdict_reason=final_reason_pulse,
                    ...
                    if current_engine_mode in [AnalysisMode.ML, AnalysisMode.HYBRID] and ml_pred_pulse:
                        ...
                        if (ml_bull_mp and rules_bull_mp) or (ml_bear_mp and rules_bear_mp):
                    ...
                    with st.expander(learning_title_pulse, expanded=False):
                        ...
                        if current_engine_mode == AnalysisMode.ML:
                            ...
                            if (rules_bullish_p and ml_bullish_p) or (rules_bearish_p and ml_bearish_p):
                                agreement_p = True
                            else:
                                agreement_p = False
                ...
                if pred_result:
                    try:
                        ...
                        # Build reason for WAIT if applicable
                        final_reason_scanner = None
                        if final_trade_dir_scanner == 'WAIT':
                            reasons_sc = []
                            pos_label_sc = pred_result.move_position if hasattr(pred_result, 'move_position') else 'MIDDLE'
                            pos_pct_sc = position_in_range
                            if pos_label_sc == 'LATE':
                                reasons_sc.append(f"LATE ({pos_pct_sc:.0f}%)")
                            if ml_pred_str_scanner and ml_pred_str_scanner not in ['LONG'] and pred_result.direction in ['BULLISH', 'LEAN_BULLISH']:
                                reasons_sc.append(f"ML not bullish")
                            if reasons_sc:
                                final_reason_scanner = " + ".join(reasons_sc)
                        ...
                        combined_learning_scanner = generate_combined_learning(
                            ...
                            final_verdict_reason=final_reason_scanner,
                        ...
                        if current_analysis_mode in [AnalysisMode.ML, AnalysisMode.HYBRID] and ml_pred_scanner:
                            ...
                            if (ml_bull_sc2 and rules_bull_sc2) or (ml_bear_sc2 and rules_bear_sc2):
                        ...
                        with st.expander(learning_title_scanner, expanded=False):
                            ...
                            if current_analysis_mode == AnalysisMode.ML:
                                ...
                                if (rules_bullish_sc and ml_bullish_sc) or (rules_bearish_sc and ml_bearish_sc):
                                    agreement_sc = True
                                else:
                                    agreement_sc = False
                                ...
                                if df_analysis is not None and len(df_analysis) >= 50:
                                    ...
                                    if analysis and 'error' not in analysis:
                                        ...
                                        if pred_result and not is_in_danger_zone:
                                            ...
                                            # Get final verdict for combined learning
                                            final_trade_dir_mon = pred_result.trade_direction if hasattr(pred_result, 'trade_direction') else 'WAIT'
                                            
                                            # Build reason for WAIT if applicable
                                            final_reason_mon = None
                                            if final_trade_dir_mon == 'WAIT':
                                                reasons_m = []
                                                pos_label_m = pred_result.move_position if hasattr(pred_result, 'move_position') else 'MIDDLE'
                                                if pos_label_m == 'LATE':
                                                    reasons_m.append(f"LATE ({position_pct_cl:.0f}%)")
                                                if reasons_m:
                                                    final_reason_mon = " + ".join(reasons_m)
                                            ...
                                            combined_learning = generate_combined_learning(
                                                ...
                                                final_verdict_reason=final_reason_mon,
                                            )
    ...
    if show_analysis:
        with st.spinner(f"Generating comprehensive analysis for {symbol_input}..."):
            ...
            if df is not None and len(df) >= 50:
                ...
                swing_high_unified = analysis.get('swing_high', current_price * 1.05)
                swing_low_unified = analysis.get('swing_low', current_price * 0.95)
                ...
                # ═══════════════════════════════════════════════════════════
                # ML Integration - Get prediction for display
                # (Note: ML is already integrated in analyze_symbol_full via 
                #  blend_hybrid_prediction, but we need ml_prediction for display)
                # ═══════════════════════════════════════════════════════════
                ml_prediction = None
                ml_error = None
                current_analysis_mode = get_analysis_mode()
                    ...
                    try:
                        ...
                        if is_ml_available():
                            ml_prediction = get_ml_prediction(
                                ...
                                price_change_24h=price_change_single,
                                position_pct=pos_in_range_single,
                                current_price=current_price,
                                swing_high=swing_high_unified,
                                swing_low=swing_low_unified,
                                ta_score=ta_score,
                                trend=smc.get('structure', {}).get('structure', 'Unknown') if smc else 'Unknown',
                                money_flow_phase=money_flow_phase,
                            )
                        else:
                            ml_error = "ML models not trained - run train_ml.py first"
                    except Exception as e:
                        ml_error = f"ML error: {str(e)}"
                
                # Show ML error if any
                if ml_error and current_analysis_mode in [AnalysisMode.ML, AnalysisMode.HYBRID]:
                    st.warning(f"⚠️ {ml_error} - Using rule-based scoring")
                ...
                # Get ML prediction for combined learning context
                ml_pred_for_cl = None
                if ml_prediction and hasattr(ml_prediction, 'direction'):
                    ml_pred_for_cl = ml_prediction.direction
                
                # Get engine mode string
                ...
                if final_trade_dir == 'WAIT':
                    # Build the reason from context
                    reasons = []
                    pos_label = predictive_result.move_position if hasattr(predictive_result, 'move_position') else 'MIDDLE'
                    pos_pct = predictive_result.move_position_pct if hasattr(predictive_result, 'move_position_pct') else 50
                    if pos_label == 'LATE':
                        reasons.append(f"LATE ({pos_pct:.0f}%)")
                    if ml_pred_for_cl and ml_pred_for_cl not in ['LONG'] and predictive_result.direction in ['BULLISH', 'LEAN_BULLISH']:
                        reasons.append(f"ML not bullish")
                    if ml_pred_for_cl and ml_pred_for_cl not in ['SHORT'] and predictive_result.direction in ['BEARISH', 'LEAN_BEARISH']:
                        reasons.append(f"ML not bearish")
                    if reasons:
                        final_verdict_reason = " + ".join(reasons)
                ...
                if current_analysis_mode in [AnalysisMode.ML, AnalysisMode.HYBRID] and ml_prediction:
                    ...
                    if (ml_bullish and rules_bullish) or (ml_bearish and rules_bearish):
                    ...
                    if current_analysis_mode == AnalysisMode.ML:
                        ...
                        if ml_prediction:
                            ...
                            if 'position_in_range' in top_feature_names:
                                ml_stories.append(f"📍 Position in range ({position_pct_learn:.0f}%) is a key factor")
                            if 'whale_pct' in top_feature_names:
                                ml_stories.append(f"🐋 Whale positioning ({whale_pct_learn:.0f}% long) heavily weighted")
                            if 'oi_change' in top_feature_names:
                                ml_stories.append(f"📈 Open Interest change ({oi_24h:+.1f}%) significant")
                            ...
                            st.warning("ML prediction not available - check if models are trained")
                        ...
                        # Then: Show Rules analysis (THE FULL STORY)
                        st.markdown("**📊 THE FULL STORY (Rules Analysis):**")
                        st.markdown(cl_html['stories_html'], unsafe_allow_html=True)
                ...
                with col_levels:
                    if is_investment:
                        ...
                        if has_limit:
                            ...
                            market_rr = getattr(signal, 'rr_tp1', 0)
                            ...
                            <div style='background: linear-gradient(135deg, #1a2a3a 0%, #0d1a2a 100%); padding: 15px; border-radius: 10px; margin-bottom: 15px; border: 2px solid #00d4ff;'>
                                ...
                                <div style='display: flex; gap: 20px;'>
                                    ...
                                    <div style='flex: 1; background: #0a2015; padding: 15px; border-radius: 8px; border-left: 4px solid #00ff88;'>
                                        <div style='color: #00ff88; font-size: 0.9em; font-weight: bold;'>⭐ Limit Entry (Recommended)</div>
                                        <div style='color: #888; font-size: 0.75em;'>{limit_reason}</div>
                                ...
                                <div style='color: #aaa; font-size: 0.8em; margin-top: 12px; text-align: center;'>
                                    💡 Limit entry at Order Block = Better R:R + Less chance of stop hunt
                                </div>
                            </div>
                            """, unsafe_allow_html=True)
                        else:
