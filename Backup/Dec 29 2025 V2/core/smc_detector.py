...
def detect_order_blocks(df: pd.DataFrame, lookback: int = 50) -> Dict:
    ...
    try:
        ...
        # A bullish OB is a bearish candle before a strong move UP
                ...
                if df['Close'].iloc[i] < df['Open'].iloc[i]:  # Bearish candle
                    ...
                    if move_size > atr * 0.5:
                        ...
                        if ob_top > ob_bottom:
                            ...
                            break  # Found most recent valid OB, stop searching
            except:
                continue
        
        # Look for bearish order blocks (find the MOST RECENT valid OB)
        # A bearish OB is a bullish candle before a strong move DOWN
        for i in range(len(df) - 3, max(len(df) - lookback, 3), -1):
            try:
                # Bullish candle followed by strong bearish move
                if df['Close'].iloc[i] > df['Open'].iloc[i]:  # Bullish candle
                    # Check for strong bearish move after (within next 3 candles)
...
def detect_all_order_blocks(df: pd.DataFrame, current_price: float, lookback: int = 50, max_obs: int = 5) -> Dict:
    ...
    try:
        ...
        # Scan for ALL OBs with lower threshold for better detection
        for i in range(len(df) - 3, max(len(df) - lookback, 3), -1):
            try:
                ...
                if df['Close'].iloc[i] < df['Open'].iloc[i]:
                    ...
                    if move_size > atr * 0.5:  # Lower threshold for more OBs
                        ob_top = df['Open'].iloc[i]
                        ob_bottom = df['Close'].iloc[i]
