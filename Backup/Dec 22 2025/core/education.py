...
def get_unified_verdict(oi_change: float, price_change: float,
                        whale_pct: float, retail_pct: float,
                        funding: float, tech_action: str = 'WAIT') -> Dict:
    """
    Generate a unified verdict combining all signals.
    
    ═══════════════════════════════════════════════════════════════════════════════
    THIS FUNCTION NOW USES rules_engine.py FOR DETECTION
    Education content still comes from SCENARIO_EDUCATION dict
    ═══════════════════════════════════════════════════════════════════════════════
    """
    # Get scenario from rules engine (single source of truth for detection)
    scenario_key = identify_current_scenario(
        oi_change, price_change, whale_pct, retail_pct, funding
    )
    
    # Get education content from local dict
    scenario_info = SCENARIO_EDUCATION.get(scenario_key, SCENARIO_EDUCATION['no_edge'])
    
    # Determine unified action based on confidence
    if scenario_info['confidence'] == 'HIGH':
        if 'LONG' in scenario_info['recommendation']:
            unified_action = 'LONG'
        elif 'SHORT' in scenario_info['recommendation']:
            unified_action = 'SHORT'
        else:
            unified_action = 'WAIT'
    elif scenario_info['confidence'] == 'MEDIUM':
    ...
    inst_bullish = whale_pct >= 55  # Using correct threshold
    inst_bearish = whale_pct <= 45  # Using correct threshold
