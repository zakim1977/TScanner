...
def get_setup_info(direction: str, position: str = None, position_pct: float = None, setup_type: str = None) -> tuple:
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # ⚠️ PRIORITY 3: STRONG ML DISAGREE
    # ═══════════════════════════════════════════════════════════════════════════
...
def render_signal_header_html(
    ...
    if vwap_bounce:
        ...
        if vwap_text:
            ...
            # Compact display: VWAP: 🟢 FRESH (15m ago)
            time_str = f" ({time_ago})" if time_ago and time_ago != 'now' else ""
            vwap_display = f" | <span style='color: {vwap_color};'>{vwap_text}</span> <span style='color: {age_color};'>{age_badge}{signal_age}{time_str}</span>"
    ...
    html_parts.append(f"<h2 style='margin: 0; color: {header_color}; font-size: 1.8em;'>{header_emoji} {symbol}: {action_word} ({score}% predictive score){alignment_badge}</h2>")
