...
def detect_vwap_bounce(df: pd.DataFrame, lookback: int = 5, timeframe: str = '15m') -> dict:
    """
    ...
    try:
        ...
        for i in range(1, min(lookback + 1, len(df))):
            ...
            # BULLISH BOUNCE: Low touched VWAP, then price moved UP
            if candle_low <= vwap_at_candle + touch_tolerance and candle_low >= vwap_at_candle - touch_tolerance:
                ...
                if candle_close > candle_open and current_price > vwap_at_candle:
                    ...
                    # Signal freshness based on candles AND time
                    if i <= 2:
                    ...
                    # Strength based on distance and freshness
                    if i <= 2 and abs(vwap_data['distance_pct']) < 1.0:
                        result['strength'] = 'STRONG'
                        result['score_adjustment'] = 12
                    elif i <= 3 and abs(vwap_data['distance_pct']) < 2.0:
            ...
            # BEARISH BOUNCE: High touched VWAP, then price moved DOWN  
        ...
        # Also check: AT VWAP (potential entry zone even without confirmed bounce)
        if not result['bounce_detected'] and abs(vwap_data['distance_pct']) < 0.5:
            result['bounce_type'] = 'AT_VWAP'
            result['strength'] = 'POTENTIAL'
            result['signal_age'] = 'NOW'
            result['time_ago'] = 'now'
            result['score_adjustment'] = 5
            result['description'] = f"At VWAP ({vwap_data['distance_pct']:+.2f}%) - Watch for bounce"
