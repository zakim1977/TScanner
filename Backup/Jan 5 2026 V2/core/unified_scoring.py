...
# HONEST ML LABELING
# ═══════════════════════════════════════════════════════════════════════════════
# This ensures we NEVER misleadingly say "ML" when it's actually heuristic

def _get_ml_label_info() -> Dict:
    """
    Get honest label information for ML/Heuristic predictions.
    
    Returns dict with:
        - is_real_ml: True if actual trained ML, False if heuristic
        - confirms_label: Text like "✅ Trained ML confirms" or "✅ Heuristic agrees"
        - says_label: Text like "🤖 ML says" or "📊 Heuristic says"
        - source: "Trained ML" or "Heuristic"
    """
    try:
    ...
    if is_real:
        ...
        return {
            'is_real_ml': False,
            'confirms_label': "✅ Heuristic agrees",
            'says_label': "📊 Heuristic says",
            'disagrees_label': "📊 Heuristic says",
            'source': source,
            'icon': "📊"
        }
...
def generate_combined_learning(
    ...
    if MASTER_RULES_AVAILABLE and whale_pct is not None and retail_pct is not None:
        try:
            ...
            # Add ML/Heuristic story if in hybrid mode - USE HONEST LABELS!
            if engine_mode == 'hybrid' and ml_prediction:
                ml_labels = get_ml_labels()
                ml_emoji = "🟢" if ml_prediction == 'LONG' else "🔴" if ml_prediction == 'SHORT' else "⏳"
                stories.append((f'{ml_labels["icon"]} {ml_labels["source"]}', f"{ml_emoji} says: {ml_prediction}"))
