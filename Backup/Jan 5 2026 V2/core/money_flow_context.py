...
def get_money_flow_context(
    ...
    if position_label == "EARLY":
        if is_accumulating:
            ...
            # Selling at lows = capitulation (often a bottom signal)
            phase = "CAPITULATION"
            phase_color = "#ff9500"
            phase_emoji = "🔶"
            phase_detail = "Panic selling - potential bottom"
            bullish_bias = 10  # Counter-trend opportunity
            confidence = "MEDIUM"
            warning = "Capitulation can signal bottom - watch for reversal"
