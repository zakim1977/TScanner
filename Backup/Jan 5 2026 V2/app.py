...
from core.indicators import calculate_rsi, calculate_ema, calculate_atr, calculate_bbands, calculate_vwap, detect_vwap_bounce
...
from utils.trade_storage import (
    ...
    calculate_ml_performance_stats, get_trade_duration_warning  # NEW: ML performance analytics
)
...
def create_trade_chart(trade: dict, df: pd.DataFrame = None) -> 'go.Figure':
    ...
    fvg_data = {}
    try:
        from core.smc_detector import detect_order_blocks_ict, detect_fvg
        ob_data = detect_order_blocks_ict(df_chart, swing_length=10)
...
def analyze_symbol_full(df, symbol: str, timeframe: str, market_type: str, trade_mode: str, fetch_whale_api: bool = False) -> dict:
    ...
    try:
        ...
        # ═══════════════════════════════════════════════════════════════
        # 🤖 ML CONFLICT DETECTION - WARN but DON'T override for WAIT
        # Professional approach:
        # - ML says WAIT: Just warn, don't override (user decides)
        # - ML says opposite direction: Override at 85%+ (actionable)
        # This way Scanner shows signals, user sees ML warning
        # ═══════════════════════════════════════════════════════════════
        ml_conflict_info = None
        if ml_prediction and current_analysis_mode in [AnalysisMode.ML, AnalysisMode.HYBRID]:
            ml_conf = ml_prediction.confidence
            ml_dir = ml_prediction.direction
            
            # Check for conflict between ML and current trade_direction
            rules_bullish = trade_direction == 'LONG'
            rules_bearish = trade_direction == 'SHORT'
            ml_bullish = ml_dir == "LONG"
            ml_bearish = ml_dir == "SHORT"
            ml_wait = ml_dir in ["WAIT", "NEUTRAL"]
            
            has_ml_conflict = not ((ml_bullish and rules_bullish) or 
                                   (ml_bearish and rules_bearish) or 
                                   (ml_wait and trade_direction == 'WAIT'))
            
            # Store conflict info for display
            if has_ml_conflict and ml_conf >= 65:
                ml_conflict_info = {
                    'ml_direction': ml_dir,
                    'ml_confidence': ml_conf,
                    'rules_direction': trade_direction,
                }
                if predictive_result:
                    predictive_result.ml_conflict = ml_conflict_info
            
            # ML says WAIT: Only warn if confidence is reasonable (not suspiciously high)
            # Models trained on imbalanced data often predict WAIT with 95%+ confidence on everything
            # If ML says WAIT with 90%+ confidence, it's likely a biased model - reduce warning
            if ml_wait and has_ml_conflict:
                # High confidence WAIT (90%+) is suspicious - likely biased model
                if ml_conf >= 90:
                    # Don't add warning - model is probably just biased
                    pass
                elif ml_conf >= 75:
                    # Reasonable confidence - add warning
                    if predictive_result:
                        original_action = predictive_result.final_action or ""
                        predictive_result.final_action = f"⚠️ {original_action} (ML: WAIT {ml_conf:.0f}%)"
            
            # ML says opposite direction (LONG vs SHORT): Override at 85%+
            # This is actionable - ML is suggesting a DIFFERENT trade, not "do nothing"
            elif ml_conf >= 85 and has_ml_conflict and not ml_wait:
                if ml_bullish and rules_bearish:
                    trade_direction = "LONG"
                    if predictive_result:
                        predictive_result.trade_direction = "LONG"
                        predictive_result.final_action = f"🟢 LONG (ML Override {ml_conf:.0f}%)"
                elif ml_bearish and rules_bullish:
                    trade_direction = "SHORT"
                    if predictive_result:
                        predictive_result.trade_direction = "SHORT"
                        predictive_result.final_action = f"🔴 SHORT (ML Override {ml_conf:.0f}%)"
        ...
        # ═══════════════════════════════════════════════════════════════
        # STEP 6b: Calculate SL/TP using UNIFIED Trade Optimizer
        # SAME function as Single Analysis - ONE SOURCE OF TRUTH!
        # ═══════════════════════════════════════════════════════════════
        step = "trade_optimizer"
        if signal:
            try:
                ...
                if ml_prediction:
                    ml_pred_dict = {
                        ...
                        'optimal_tp3_pct': getattr(ml_prediction, 'optimal_tp3_pct', 0),
                        'optimal_sl_pct': getattr(ml_prediction, 'optimal_sl_pct', 0),
                    }
                
                # Build SMC data dict (SAME FORMAT as Single Analysis)
                ob_data_scanner = smc.get('order_blocks', {}) if smc else {}
                smc_data_for_scanner = {
                    'bullish_ob': ob_data_scanner.get('bullish_ob', False),
                    'bullish_ob_top': ob_data_scanner.get('bullish_ob_top', 0),
                    'bullish_ob_bottom': ob_data_scanner.get('bullish_ob_bottom', 0),
                    'bearish_ob': ob_data_scanner.get('bearish_ob', False),
                    'bearish_ob_top': ob_data_scanner.get('bearish_ob_top', 0),
                    'bearish_ob_bottom': ob_data_scanner.get('bearish_ob_bottom', 0),
                    # FULL OB LISTS for TP targeting
                    'bullish_obs': ob_data_scanner.get('bullish_obs', []),
                    'bearish_obs': ob_data_scanner.get('bearish_obs', []),
                    'swing_high': swing_high,
                    'swing_low': swing_low,
                    'structure': 'bullish' if signal.direction == 'LONG' else 'bearish',
                    ...
                    # FVG data - use CORRECT keys from SMC detector!
                    'fvg_bullish': smc.get('fvg', {}).get('bullish_fvg', False) if smc else False,
                    'fvg_bearish': smc.get('fvg', {}).get('bearish_fvg', False) if smc else False,
                    'fvg_bullish_top': smc.get('fvg', {}).get('bullish_fvg_high', 0) if smc else 0,
                    'fvg_bullish_bottom': smc.get('fvg', {}).get('bullish_fvg_low', 0) if smc else 0,
                    'fvg_bearish_top': smc.get('fvg', {}).get('bearish_fvg_high', 0) if smc else 0,
                    'fvg_bearish_bottom': smc.get('fvg', {}).get('bearish_fvg_low', 0) if smc else 0,
                    # FULL FVG LISTS for TP targeting
                    'bullish_fvgs': smc.get('fvg', {}).get('bullish_fvgs', []) if smc else [],
                    'bearish_fvgs': smc.get('fvg', {}).get('bearish_fvgs', []) if smc else [],
                }
                
                # Build HTF data dict (SAME FORMAT as Single Analysis)
                htf_bullish_ob_top_scanner = 0
                htf_bullish_ob_bottom_scanner = 0
                htf_bearish_ob_top_scanner = 0
                htf_bearish_ob_bottom_scanner = 0
                
                if htf_obs:
                    # Get nearest Bearish OB (resistance above)
                    bearish_obs_list = htf_obs.get('bearish_obs', [])
                    if bearish_obs_list:
                        nearest_bearish = bearish_obs_list[0]
                        htf_bearish_ob_top_scanner = nearest_bearish.get('top', 0)
                        htf_bearish_ob_bottom_scanner = nearest_bearish.get('bottom', 0)
                    
                    # Get nearest Bullish OB (support below)
                    bullish_obs_list = htf_obs.get('bullish_obs', [])
                    if bullish_obs_list:
                        nearest_bullish = bullish_obs_list[0]
                        htf_bullish_ob_top_scanner = nearest_bullish.get('top', 0)
                        htf_bullish_ob_bottom_scanner = nearest_bullish.get('bottom', 0)
                
                htf_data_for_scanner = {
                    'bullish_ob_top': htf_bullish_ob_top_scanner,
                    'bullish_ob_bottom': htf_bullish_ob_bottom_scanner,
                    'bearish_ob_top': htf_bearish_ob_top_scanner,
                    'bearish_ob_bottom': htf_bearish_ob_bottom_scanner,
                    'htf_swing_high': htf_obs.get('htf_swing_high', 0) if htf_obs else 0,
                    'htf_swing_low': htf_obs.get('htf_swing_low', 0) if htf_obs else 0,
                }
                
                # Get HTF bias
                htf_bias_scanner = 'NEUTRAL'
                if htf_obs:
                    htf_structure = htf_obs.get('htf_structure', 'NEUTRAL')
                    if htf_structure in ['BULLISH', 'LEAN_BULLISH']:
                        htf_bias_scanner = 'BULLISH'
                    elif htf_structure in ['BEARISH', 'LEAN_BEARISH']:
                        htf_bias_scanner = 'BEARISH'
                
                # Get analysis mode for ML blending
                analysis_mode_str = 'hybrid' if current_analysis_mode == AnalysisMode.HYBRID else 'ml' if current_analysis_mode == AnalysisMode.ML else 'rules'
                
                # ═══════════════════════════════════════════════════════════
                # USE SAME FUNCTION AS SINGLE ANALYSIS - ONE SOURCE OF TRUTH!
                # get_optimized_trade() handles HTF, volume, ML blending
                # ═══════════════════════════════════════════════════════════
                trade_setup = get_optimized_trade(
                    ...
                    timeframe=timeframe,
                    smc_data=smc_data_for_scanner,
                    htf_data=htf_data_for_scanner,
                    volume_data=None,  # Scanner doesn't calculate volume profile
                    htf_bias=htf_bias_scanner,
                    ml_prediction=ml_pred_dict,
                    analysis_mode=analysis_mode_str,
                )
                
                # Apply levels from trade_setup if valid
                if trade_setup and trade_setup.entry > 0:
                    # Apply SL with educational description
                    if trade_setup.stop_loss:
                        signal.stop_loss = trade_setup.stop_loss
                        signal.sl_reason = getattr(trade_setup, 'sl_description', '') or (trade_setup.sl_level.description if trade_setup.sl_level else trade_setup.sl_type)
                        signal.sl_type = trade_setup.sl_type
                    
                    # Apply TPs with educational descriptions
                    if trade_setup.tp1:
                        signal.tp1 = trade_setup.tp1
                        signal.tp1_reason = getattr(trade_setup, 'tp1_description', '') or (trade_setup.tp1_level.description if trade_setup.tp1_level else trade_setup.tp1_type)
                        signal.tp1_source = trade_setup.tp1_type
                        # Calculate R:R
                        risk = abs(signal.entry - signal.stop_loss) if signal.stop_loss else 1
                        signal.rr_tp1 = abs(signal.tp1 - signal.entry) / risk if risk > 0 else 0
                    
                    if trade_setup.tp2:
                        signal.tp2 = trade_setup.tp2
                        signal.tp2_reason = getattr(trade_setup, 'tp2_description', '') or (trade_setup.tp2_level.description if trade_setup.tp2_level else trade_setup.tp2_type)
                        signal.tp2_source = trade_setup.tp2_type
                        risk = abs(signal.entry - signal.stop_loss) if signal.stop_loss else 1
                        signal.rr_tp2 = abs(signal.tp2 - signal.entry) / risk if risk > 0 else 0
                    
                    if trade_setup.tp3:
                        signal.tp3 = trade_setup.tp3
                        signal.tp3_reason = getattr(trade_setup, 'tp3_description', '') or (trade_setup.tp3_level.description if trade_setup.tp3_level else trade_setup.tp3_type)
                        signal.tp3_source = trade_setup.tp3_type
                        risk = abs(signal.entry - signal.stop_loss) if signal.stop_loss else 1
                        signal.rr_tp3 = abs(signal.tp3 - signal.entry) / risk if risk > 0 else 0
                    
                    # Calculate risk %
                    if signal.stop_loss and signal.entry:
                        signal.risk_pct = abs(signal.entry - signal.stop_loss) / signal.entry * 100
        ...
        # ═══════════════════════════════════════════════════════════════
        # STEP 8b: VWAP BOUNCE DETECTION - Key institutional entry signal
        # ═══════════════════════════════════════════════════════════════
        vwap_bounce = None
        try:
            vwap_bounce = detect_vwap_bounce(df, lookback=5, timeframe=timeframe)
            
            if vwap_bounce and vwap_bounce.get('bounce_detected') and predictive_result:
                # Get mode-specific VWAP weight
                from core.signal_generator import get_mode_config
                mode_cfg = get_mode_config(timeframe)
                vwap_weight = mode_cfg.get('vwap_weight', 8)
                
                # Calculate adjusted score bonus (bounce_score * mode_weight / 10)
                base_adjustment = vwap_bounce.get('score_adjustment', 0)
                weighted_adjustment = int(base_adjustment * vwap_weight / 10)
                
                # Check if bounce aligns with trade direction
                bounce_type = vwap_bounce.get('bounce_type', '')
                direction_aligned = (
                    (trade_direction == 'LONG' and bounce_type == 'BULLISH_BOUNCE') or
                    (trade_direction == 'SHORT' and bounce_type == 'BEARISH_BOUNCE')
                )
                
                if direction_aligned:
                    original_score = predictive_result.final_score
                    predictive_result.final_score = min(100, predictive_result.final_score + weighted_adjustment)
                    vwap_bounce['applied_adjustment'] = weighted_adjustment
                    vwap_bounce['direction_aligned'] = True
                else:
                    vwap_bounce['applied_adjustment'] = 0
                    vwap_bounce['direction_aligned'] = False
                    
                # Store in signal for display
                if signal:
                    signal.vwap_bounce = vwap_bounce
        except Exception as vwap_err:
            pass  # VWAP check is optional
            ...
            for tf in timeframes_to_scan:
                ...
                try:
                    ...
                    if df is not None and len(df) >= 50:
                        ...
                        # ═══════════════════════════════════════════════════════════
                        # 🔧 UNIFIED VERDICT: Use trade_direction from analysis
                        # analyze_symbol_full() already applied ML override!
                        # Just use the computed direction - no duplicate logic needed
                        # ═══════════════════════════════════════════════════════════
                        live_direction = analysis.get('trade_direction', signal.direction)
                        
                        # Update pred_action to match (if ML overrode)
                        if predictive_result and hasattr(predictive_result, 'final_action'):
                            pred_action = predictive_result.final_action
        ...
        for r in results:
            ...
            with st.expander(f"{bd['grade_emoji']} **{sig.symbol}** | {display_label} | {action_emoji} {pred_action} ({pred_score}/100)", expanded=False):
                ...
                if pred_result:
                    ...
                    # Get ML info for alignment badge in header
                    ml_dir_header_scan = ml_pred_pulse.direction if ml_pred_pulse else None
                    ml_conf_header_scan = ml_pred_pulse.confidence if ml_pred_pulse and hasattr(ml_pred_pulse, 'confidence') else None
                    rules_dir_header_scan = pred_result.direction if pred_result else None
                    
                    # Check for conflict (for header badge)
                    is_conflict_scan = False
                    if ml_pred_pulse and pred_result:
                        ml_bull_h = ml_dir_header_scan == "LONG"
                        ml_bear_h = ml_dir_header_scan == "SHORT"
                        ml_wait_h = ml_dir_header_scan in ["WAIT", "NEUTRAL"]
                        rules_bull_h = rules_dir_header_scan in ['BULLISH', 'LEAN_BULLISH']
                        rules_bear_h = rules_dir_header_scan in ['BEARISH', 'LEAN_BEARISH']
                        rules_neutral_h = rules_dir_header_scan in ['NEUTRAL', 'WAIT', 'MIXED']
                        is_conflict_scan = not ((ml_bull_h and rules_bull_h) or (ml_bear_h and rules_bear_h) or (ml_wait_h and rules_neutral_h))
                    ...
                    header_html = render_signal_header_html(
                        ...
                        # ML alignment info for header badge
                        ml_direction=ml_dir_header_scan,
                        ml_confidence=ml_conf_header_scan,
                        rules_direction=rules_dir_header_scan,
                        is_conflict=is_conflict_scan,
                        final_verdict=sig.direction,
                        # VWAP bounce info
                        vwap_bounce=r.get('vwap_bounce')
                    )
                    st.markdown(header_html, unsafe_allow_html=True)
                        ...
                        if any(t['symbol'] == sig.symbol for t in st.session_state.active_trades):
                            ...
                            with btn_col1:
                                ...
                                if st.button(f"➕ Add to Monitor", key=f"add_{sig.symbol}_{timeframe}", type="primary" if status == "READY" else "secondary"):
                                    ...
                                    # Get ML prediction info for tracking
                                    ml_pred = r.get('ml_prediction')
                                    ml_direction = ml_pred.direction if ml_pred else None
                                    ml_confidence = ml_pred.confidence if ml_pred else None
                                    ml_eta_candles = getattr(ml_pred, 'eta_candles', None) if ml_pred else None
                                    
                                    # Get rules direction
                                    pred_result = r.get('predictive_result')
                                    rules_direction = pred_result.direction if pred_result else sig.direction
                                    
                                    # Check for ML conflict
                                    ml_bullish = ml_direction == "LONG" if ml_direction else False
                                    ml_bearish = ml_direction == "SHORT" if ml_direction else False
                                    ml_wait = ml_direction in ["WAIT", "NEUTRAL"] if ml_direction else False
                                    rules_bullish = rules_direction in ['BULLISH', 'LEAN_BULLISH', 'LONG']
                                    rules_bearish = rules_direction in ['BEARISH', 'LEAN_BEARISH', 'SHORT']
                                    has_ml_conflict = ml_direction is not None and not (
                                        (ml_bullish and rules_bullish) or 
                                        (ml_bearish and rules_bearish) or 
                                        (ml_wait and not rules_bullish and not rules_bearish)
                                    )
                                    
                                    # Get liquidity info if available
                                    liquidity = r.get('liquidity')
                                    liquidity_tier = liquidity.tier if liquidity else None
                                    liquidity_volume = liquidity.volume_24h if liquidity else None
                                    ...
                                    trade = {
                                        ...
                                        'limit_entry_reason': getattr(sig, 'limit_entry_reason', ''),
                                        'limit_entry_rr': getattr(sig, 'limit_entry_rr_tp1', 0),
                                        # NEW: ML vs Rules tracking
                                        'ml_direction': ml_direction,
                                        'ml_confidence': ml_confidence,
                                        'ml_eta_candles': ml_eta_candles,
                                        'rules_direction': rules_direction,
                                        'has_ml_conflict': has_ml_conflict,
                                        # NEW: Liquidity tracking
                                        'liquidity_tier': liquidity_tier,
                                        'liquidity_volume_24h': liquidity_volume,
                                    }
                                    add_trade(trade)
                                    st.session_state.active_trades = get_active_trades()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if not st.session_state.active_trades:
        ...
        # Calculate AVERAGE P&L (not sum!)
        avg_pnl = total_pnl_sum / pnl_count if pnl_count > 0 else 0
        ...
        for i, trade in sorted_trades:
            ...
            if price > 0:
                ...
                with st.expander(f"{emoji} {trade['symbol']} | P&L: {pnl:+.2f}% | {status}", expanded=is_expanded):
                    ...
                    # ML alignment badge
                    ml_dir_tm = trade.get('ml_direction')
                    ml_conf_tm = trade.get('ml_confidence', 0)
                    has_conflict_tm = trade.get('has_ml_conflict', False)
                    
                    alignment_badge_tm = ""
                    if ml_dir_tm:
                        if has_conflict_tm:
                            alignment_badge_tm = "<span style='background: #ff9800; color: #000; padding: 4px 10px; border-radius: 15px; font-size: 0.8em; font-weight: bold;'>⚠️ CONFLICT</span>"
                        elif ml_conf_tm and ml_conf_tm >= 90:
                            alignment_badge_tm = "<span style='background: #00ff88; color: #000; padding: 4px 10px; border-radius: 15px; font-size: 0.8em; font-weight: bold;'>✅ FULL ALIGNMENT</span>"
                        elif ml_conf_tm and ml_conf_tm >= 70:
                            alignment_badge_tm = "<span style='background: #00d4aa; color: #000; padding: 4px 10px; border-radius: 15px; font-size: 0.8em; font-weight: bold;'>✅ ALIGNED</span>"
                    ...
                    # ═══════════════════════════════════════════════════════════
                    # 🤖 ML vs RULES STATUS + DURATION WARNING (NEW!)
                    # ═══════════════════════════════════════════════════════════
                    
                    # Show ML conflict warning if present
                    ml_direction = trade.get('ml_direction')
                    ml_confidence = trade.get('ml_confidence')
                    has_ml_conflict = trade.get('has_ml_conflict', False)
                    rules_direction = trade.get('rules_direction')
                    
                    if ml_direction and has_ml_conflict:
                        conflict_color = "#ff9800" if ml_confidence and ml_confidence >= 75 else "#ffcc00"
                        st.markdown(f"""
                        <div style='background: {conflict_color}15; border: 1px solid {conflict_color}; 
                                    border-radius: 8px; padding: 10px 15px; margin-bottom: 10px;'>
                            <span style='color: {conflict_color}; font-weight: bold;'>⚠️ ML CONFLICT at Entry</span>
                            <span style='color: #888; margin-left: 15px;'>
                                ML: <span style='color: #a855f7;'>{ml_direction} ({ml_confidence:.0f}%)</span> vs 
                                Rules: <span style='color: #3b82f6;'>{rules_direction}</span>
                            </span>
                        </div>
                        """, unsafe_allow_html=True)
                    
                    # Check for duration warning
                    duration_warning = get_trade_duration_warning(trade)
                    if duration_warning:
                        st.markdown(f"""
                        <div style='background: #ffcc0015; border: 1px solid #ffcc00; 
                                    border-radius: 8px; padding: 10px 15px; margin-bottom: 10px;'>
                            <span style='color: #ffcc00;'>{duration_warning}</span>
                        </div>
                        """, unsafe_allow_html=True)
                    
                    # Show liquidity tier if available
                    liquidity_tier = trade.get('liquidity_tier')
                    if liquidity_tier and liquidity_tier in ['LOW', 'VERY_LOW', 'ILLIQUID']:
                        liq_color = "#ff6b6b" if liquidity_tier == 'ILLIQUID' else "#ff9800"
                        st.markdown(f"""
                        <div style='background: {liq_color}15; border: 1px solid {liq_color}; 
                                    border-radius: 8px; padding: 8px 12px; margin-bottom: 10px; display: inline-block;'>
                            <span style='color: {liq_color};'>💧 {liquidity_tier} Liquidity</span>
                            <span style='color: #888; font-size: 0.85em; margin-left: 8px;'>→ Slower execution expected</span>
                        </div>
                        """, unsafe_allow_html=True)
                            ...
                            if has_real_data:
                                ...
                                <div style='background: #1a1a2e; border-radius: 8px; padding: 12px; margin-bottom: 12px;
                                    ...
                                    <div style='display: grid; grid-template-columns: repeat(5, 1fr); gap: 8px;'>
                                        ...
                                        <div style='background: #252540; padding: 6px; border-radius: 4px; text-align: center;'>
                                            <div style='color: #888; font-size: 0.7em;'>📊 Price 24h</div>
                                            <div style='color: {"#00d4aa" if price_change > 0 else "#ff4444" if price_change < 0 else "#888"}; font-weight: bold;'>{price_change:+.1f}%</div>
                                        </div>
                                        <div style='background: #252540; padding: 6px; border-radius: 4px; text-align: center;'>
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if closed:
        ...
        # ═══════════════════════════════════════════════════════════════════
        # 🤖 ML VS RULES PERFORMANCE ANALYSIS
        # Key insight: How does ML alignment affect outcomes?
        # ═══════════════════════════════════════════════════════════════════
        
        st.markdown("### 🤖 ML vs Rules Performance Analysis")
        st.markdown("<p style='color: #888;'>Understanding when to trust ML warnings</p>", unsafe_allow_html=True)
        
        try:
            ml_stats = calculate_ml_performance_stats()
            
            # Main comparison cards
            ml_c1, ml_c2, ml_c3 = st.columns(3)
            
            with ml_c1:
                aligned = ml_stats.get('aligned', {})
                aligned_wr = aligned.get('win_rate', 0)
                aligned_dur = aligned.get('avg_duration_mins', 0)
                dur_display = f"{aligned_dur/60:.1f}hrs" if aligned_dur >= 60 else f"{aligned_dur:.0f}min"
                st.markdown(f"""
                <div style='background: linear-gradient(135deg, #0a2a0a 0%, #1a3a1a 100%); border: 2px solid #00ff88; border-radius: 12px; padding: 20px; text-align: center;'>
                    <div style='color: #00ff88; font-size: 0.9em; margin-bottom: 5px;'>✅ ML + RULES ALIGNED</div>
                    <div style='color: #fff; font-size: 2.5em; font-weight: bold;'>{aligned_wr:.0f}%</div>
                    <div style='color: #888; font-size: 0.85em;'>Win Rate ({aligned.get('count', 0)} trades)</div>
                    <div style='color: #00d4aa; font-size: 0.9em; margin-top: 8px;'>Avg Time to TP1: {dur_display}</div>
                </div>
                """, unsafe_allow_html=True)
            
            with ml_c2:
                conflict = ml_stats.get('conflict', {})
                conflict_wr = conflict.get('win_rate', 0)
                conflict_dur = conflict.get('avg_duration_mins', 0)
                dur_display = f"{conflict_dur/60:.1f}hrs" if conflict_dur >= 60 else f"{conflict_dur:.0f}min"
                st.markdown(f"""
                <div style='background: linear-gradient(135deg, #2a1a0a 0%, #3a2a1a 100%); border: 2px solid #ff9800; border-radius: 12px; padding: 20px; text-align: center;'>
                    <div style='color: #ff9800; font-size: 0.9em; margin-bottom: 5px;'>⚠️ ML vs RULES CONFLICT</div>
                    <div style='color: #fff; font-size: 2.5em; font-weight: bold;'>{conflict_wr:.0f}%</div>
                    <div style='color: #888; font-size: 0.85em;'>Win Rate ({conflict.get('count', 0)} trades)</div>
                    <div style='color: #ffcc00; font-size: 0.9em; margin-top: 8px;'>Avg Time to TP1: {dur_display}</div>
                </div>
                """, unsafe_allow_html=True)
            
            with ml_c3:
                ml_wait = ml_stats.get('ml_wait_ignored', {})
                wait_wr = ml_wait.get('win_rate', 0)
                wait_dur = ml_wait.get('avg_duration_mins', 0)
                dur_display = f"{wait_dur/60:.1f}hrs" if wait_dur >= 60 else f"{wait_dur:.0f}min"
                st.markdown(f"""
                <div style='background: linear-gradient(135deg, #2a0a0a 0%, #3a1a1a 100%); border: 2px solid #ff4444; border-radius: 12px; padding: 20px; text-align: center;'>
                    <div style='color: #ff4444; font-size: 0.9em; margin-bottom: 5px;'>🚫 ML WAIT IGNORED</div>
                    <div style='color: #fff; font-size: 2.5em; font-weight: bold;'>{wait_wr:.0f}%</div>
                    <div style='color: #888; font-size: 0.85em;'>Win Rate ({ml_wait.get('count', 0)} trades)</div>
                    <div style='color: #ff6b6b; font-size: 0.9em; margin-top: 8px;'>Avg Time to TP1: {dur_display}</div>
                </div>
                """, unsafe_allow_html=True)
            
            # Insight box
            if aligned.get('count', 0) >= 3 and (conflict.get('count', 0) >= 3 or ml_wait.get('count', 0) >= 3):
                insight_parts = []
                if aligned_wr > conflict_wr + 10:
                    insight_parts.append(f"Aligned trades win {aligned_wr - conflict_wr:.0f}% MORE often")
                if aligned_dur > 0 and conflict_dur > aligned_dur * 1.5:
                    insight_parts.append(f"Conflict trades take {conflict_dur/aligned_dur:.1f}x LONGER to hit TP")
                
                if insight_parts:
                    st.markdown(f"""
                    <div style='background: #1a2a3a; border-left: 4px solid #00d4ff; padding: 15px; border-radius: 8px; margin: 15px 0;'>
                        <div style='color: #00d4ff; font-weight: bold;'>💡 KEY INSIGHT</div>
                        <div style='color: #fff; margin-top: 8px;'>{' • '.join(insight_parts)}</div>
                        <div style='color: #888; font-size: 0.85em; margin-top: 8px;'>
                            → Consider waiting or reducing size when ML conflicts with Rules
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
            
            # Slow trades warning
            slow_trades = ml_stats.get('slow_trades', [])
            if slow_trades:
                with st.expander(f"⏰ {len(slow_trades)} Slow Trades (3x+ longer than expected)", expanded=False):
                    for st_trade in slow_trades[:10]:
                        exp_hrs = st_trade['expected_mins'] / 60
                        act_hrs = st_trade['actual_mins'] / 60
                        ml_warn = f"ML: {st_trade['ml_direction']} ({st_trade['ml_confidence']:.0f}%)" if st_trade.get('ml_direction') else ""
                        liq = st_trade.get('liquidity_tier', 'Unknown')
                        st.markdown(f"""
                        <div style='background: #1a1a2e; padding: 10px 15px; border-radius: 8px; margin: 5px 0; border-left: 3px solid #ffcc00;'>
                            <span style='color: #00d4ff; font-weight: bold;'>{st_trade['symbol']}</span>
                            <span style='color: #888; margin-left: 10px;'>Expected: {exp_hrs:.1f}hrs → Actual: {act_hrs:.1f}hrs ({st_trade['ratio']:.1f}x)</span>
                            <span style='color: #ff9800; margin-left: 10px;'>{ml_warn}</span>
                            <span style='color: #666; margin-left: 10px;'>Liquidity: {liq}</span>
                        </div>
                        """, unsafe_allow_html=True)
            
            # Liquidity analysis
            liq_stats = ml_stats.get('by_liquidity', {})
            if liq_stats and len(liq_stats) > 1:
                with st.expander("💧 Performance by Liquidity Tier", expanded=False):
                    liq_cols = st.columns(len(liq_stats))
                    for i, (tier, tier_stats) in enumerate(sorted(liq_stats.items())):
                        with liq_cols[i % len(liq_cols)]:
                            tier_color = "#00ff88" if tier in ['EXCELLENT', 'HIGH'] else "#ffcc00" if tier == 'MEDIUM' else "#ff6b6b"
                            st.markdown(f"""
                            <div style='background: #1a1a2e; border: 1px solid {tier_color}; border-radius: 8px; padding: 12px; text-align: center;'>
                                <div style='color: {tier_color}; font-weight: bold;'>{tier}</div>
                                <div style='color: #fff; font-size: 1.5em;'>{tier_stats.get('win_rate', 0):.0f}%</div>
                                <div style='color: #888; font-size: 0.8em;'>{tier_stats.get('count', 0)} trades</div>
                            </div>
                            """, unsafe_allow_html=True)
        except Exception as e:
            st.info(f"ML Performance data will appear after more trades with ML tracking. ({e})")
    ...
    if show_analysis:
        with st.spinner(f"Generating comprehensive analysis for {symbol_input}..."):
            ...
            if df is not None and len(df) >= 50:
                ...
                # ═══════════════════════════════════════════════════════════
                # 🤖 ML CONFLICT DISPLAY: Show warning but keep original setup
                # ML WAIT = Warning only (doesn't override direction)
                # ML LONG vs SHORT = Override at 85%+ (actionable)
                # ═══════════════════════════════════════════════════════════
                if ml_prediction and current_analysis_mode in [AnalysisMode.ML, AnalysisMode.HYBRID]:
                    final_trade_dir = analysis.get('trade_direction', 'WAIT')
                    ml_conf = ml_prediction.confidence
                    ml_dir = ml_prediction.direction
                    ml_wait = ml_dir in ["WAIT", "NEUTRAL"]
                    
                    rules_bullish = predictive_result.direction in ['BULLISH', 'LEAN_BULLISH']
                    rules_bearish = predictive_result.direction in ['BEARISH', 'LEAN_BEARISH']
                    
                    # ML WAIT with conflict: Keep original setup, just note conflict
                    # (trade_direction stays LONG, user sees warning in ML box)
                    if ml_wait and ml_conf >= 75 and (rules_bullish or rules_bearish):
                        # DON'T change setup_text - keep it as LONG SETUP / SHORT SETUP
                        # But add note that ML disagrees
                        pass  # ML vs Rules box below shows the conflict
                    
                    # ML direction override (LONG vs SHORT at 85%+)
                    elif ml_conf >= 85 and final_trade_dir == 'LONG' and rules_bearish:
                        setup_text = "🟢 LONG"
                        setup_color = "#00ff88"
                        action_word = f"🟢 LONG (ML Override {ml_conf:.0f}%)"
                    elif ml_conf >= 85 and final_trade_dir == 'SHORT' and rules_bullish:
                        setup_text = "🔴 SHORT"
                        setup_color = "#ff6b6b"
                        action_word = f"🔴 SHORT (ML Override {ml_conf:.0f}%)"
                ...
                # Determine ML vs Rules conflict for header display
                ml_dir_header = ml_prediction.direction if ml_prediction else None
                ml_conf_header = ml_prediction.confidence if ml_prediction else None
                rules_dir_header = predictive_result.direction if predictive_result else None
                
                # Check for conflict (professional display)
                is_conflict_header = False
                final_verdict_header = analysis.get('trade_direction', 'WAIT')  # Use already-computed direction!
                
                if ml_prediction and predictive_result:
                    ml_bullish = ml_dir_header == "LONG"
                    ml_bearish = ml_dir_header == "SHORT"
                    ml_wait = ml_dir_header in ["WAIT", "NEUTRAL"]
                    rules_bullish = rules_dir_header in ['BULLISH', 'LEAN_BULLISH']
                    rules_bearish = rules_dir_header in ['BEARISH', 'LEAN_BEARISH']
                    rules_neutral = rules_dir_header in ['NEUTRAL', 'WAIT', 'MIXED']
                    
                    # Conflict when they disagree
                    is_conflict_header = not ((ml_bullish and rules_bullish) or 
                                              (ml_bearish and rules_bearish) or 
                                              (ml_wait and rules_neutral))
                ...
                header_html = render_signal_header_html(
                    ...
                    # NEW: ML vs Rules info
                    ml_direction=ml_dir_header,
                    ml_confidence=ml_conf_header,
                    rules_direction=rules_dir_header,
                    is_conflict=is_conflict_header,
                    final_verdict=final_verdict_header,
                    # VWAP bounce info
                    vwap_bounce=analysis.get('vwap_bounce')
                )
                st.markdown(header_html, unsafe_allow_html=True)
                ...
                with st.expander(learning_title, expanded=False):
                    ...
                    if current_analysis_mode == AnalysisMode.ML:
                        ...
                        # Agreement or conflict? Show RAW comparison
                        # User wants to see what each system ACTUALLY says before overrides
                        ...
                        # True agreement only when RAW directions match
                ...
                if liquidity_data:
                    ...
                    liq_warning_html = ""
                    if liq.warning:
                        liq_warning_html = "<div style='color: #ffaa00; font-size: 0.85em; margin-top: 8px;'>" + str(liq.warning) + "</div>"
                    
                    liq_adj_html = ""
                    if liq.score_adjustment != 0:
                        adj_color = '#ff4444' if liq.score_adjustment < 0 else '#00ff88'
                        liq_adj_html = "<div style='color: " + adj_color + "; font-size: 0.8em; margin-top: 4px;'>Score: " + f"{liq.score_adjustment:+d}" + " pts</div>"
                    
                    liq_html_parts = []
                    liq_html_parts.append("<div style='background: #1a1a2e; border-radius: 8px; padding: 12px; margin: 10px 0; border-left: 3px solid " + liq_color + ";'>")
                    liq_html_parts.append("<div style='display: flex; justify-content: space-between; align-items: center;'>")
                    liq_html_parts.append("<span style='color: #888;'>💧 Liquidity:</span>")
                    liq_html_parts.append("<span style='color: " + liq_color + "; font-weight: bold;'>" + str(liq.label) + "</span>")
                    liq_html_parts.append("</div>")
                    liq_html_parts.append("<div style='display: flex; gap: 30px; margin-top: 8px;'>")
                    liq_html_parts.append("<div><span style='color: #888; font-size: 0.85em;'>24h Volume: </span>")
                    liq_html_parts.append("<span style='color: #fff;'>" + format_volume(liq.volume_24h) + "</span></div>")
                    liq_html_parts.append("<div><span style='color: #888; font-size: 0.85em;'>Max Position: </span>")
                    liq_html_parts.append("<span style='color: #ccc;'>" + format_volume(liq.max_position_usd) + "</span></div>")
                    liq_html_parts.append("</div>")
                    if liq_warning_html:
                        liq_html_parts.append(liq_warning_html)
                    if liq_adj_html:
                        liq_html_parts.append(liq_adj_html)
                    liq_html_parts.append("</div>")
                    st.markdown("".join(liq_html_parts), unsafe_allow_html=True)
                
                # ═══════════════════════════════════════════════════════════
                # 📍 VWAP BOUNCE DETECTION - Institutional Entry Signal
                # ═══════════════════════════════════════════════════════════
                vwap_bounce = analysis.get('vwap_bounce')
                if vwap_bounce and (vwap_bounce.get('bounce_detected') or vwap_bounce.get('bounce_type') == 'AT_VWAP'):
                    bounce_type = vwap_bounce.get('bounce_type', '')
                    strength = vwap_bounce.get('strength', '')
                    distance = vwap_bounce.get('distance_pct', 0)
                    vwap_level = vwap_bounce.get('vwap_level', 0)
                    applied_adj = vwap_bounce.get('applied_adjustment', 0)
                    aligned = vwap_bounce.get('direction_aligned', False)
                    
                    # Color based on bounce type
                    if bounce_type == 'BULLISH_BOUNCE':
                        vwap_color = "#00ff88"
                        vwap_icon = "🟢"
                        vwap_label = "BULLISH BOUNCE"
                    elif bounce_type == 'BEARISH_BOUNCE':
                        vwap_color = "#ff4444"
                        vwap_icon = "🔴"
                        vwap_label = "BEARISH BOUNCE"
                    else:
                        vwap_color = "#ffcc00"
                        vwap_icon = "🟡"
                        vwap_label = "AT VWAP"
                    ...
                    # Strength badge
                    strength_colors = {'STRONG': '#00ff88', 'MODERATE': '#ffcc00', 'WEAK': '#888', 'POTENTIAL': '#00d4ff'}
                    str_color = strength_colors.get(strength, '#888')
                    dist_color = "#00ff88" if abs(distance) < 1 else "#ffcc00"
                    
                    # Build HTML with single-line parts
                    vwap_parts = []
                    vwap_parts.append("<div style='background: #1a1a2e; border-radius: 8px; padding: 12px; margin: 10px 0; border-left: 3px solid " + vwap_color + ";'>")
                    vwap_parts.append("<div style='display: flex; justify-content: space-between; align-items: center;'>")
                    vwap_parts.append("<span style='color: #888;'>" + vwap_icon + " VWAP:</span>")
                    vwap_parts.append("<span style='color: " + vwap_color + "; font-weight: bold;'>" + vwap_label + "</span>")
                    vwap_parts.append("<span style='background: " + str_color + "33; color: " + str_color + "; padding: 2px 8px; border-radius: 10px; font-size: 0.8em;'>" + strength + "</span>")
                    vwap_parts.append("</div>")
                    vwap_parts.append("<div style='display: flex; gap: 25px; margin-top: 8px;'>")
                    vwap_parts.append("<div><span style='color: #888; font-size: 0.85em;'>VWAP Level: </span>")
                    vwap_parts.append("<span style='color: #00d4ff;'>" + fmt_price(vwap_level) + "</span></div>")
                    vwap_parts.append("<div><span style='color: #888; font-size: 0.85em;'>Distance: </span>")
                    vwap_parts.append("<span style='color: " + dist_color + ";'>" + f"{distance:+.2f}%" + "</span></div>")
                    vwap_parts.append("</div>")
                    
                    if applied_adj > 0 and aligned:
                        trade_dir = predictive_result.trade_direction if predictive_result else 'signal'
                        vwap_parts.append("<div style='margin-top: 8px; color: #00ff88; font-size: 0.85em;'>")
                        vwap_parts.append("✅ +" + str(applied_adj) + " score (VWAP bounce aligned with " + trade_dir + ")")
                        vwap_parts.append("</div>")
                    elif not aligned and bounce_type != 'AT_VWAP':
                        vwap_parts.append("<div style='margin-top: 8px; color: #ff9800; font-size: 0.85em;'>")
                        vwap_parts.append("⚠️ VWAP bounce opposite to trade direction - use caution")
                        vwap_parts.append("</div>")
                    
                    vwap_parts.append("</div>")
                    st.markdown("".join(vwap_parts), unsafe_allow_html=True)
                ...
                with col_levels:
                    if is_investment:
                        ...
                        # Initialize trade_setup before try block to prevent NameError
                        trade_setup = None
                        ...
                        try:
                            ...
                            # Prepare ML prediction dict for optimizer
                            ml_pred_dict_single = None
                            if ml_prediction:
                                ml_pred_dict_single = {
                                    'optimal_tp1_pct': getattr(ml_prediction, 'optimal_tp1_pct', 0),
                                    'optimal_tp2_pct': getattr(ml_prediction, 'optimal_tp2_pct', 0),
                            ...
                            # Get analysis mode for ML blending
                            analysis_mode_str_single = 'hybrid' if current_analysis_mode == AnalysisMode.HYBRID else 'ml' if current_analysis_mode == AnalysisMode.ML else 'rules'
                            ...
                            try:
                                trade_setup = get_optimized_trade(
                                    ...
                                    ml_prediction=ml_pred_dict_single,
                                    analysis_mode=analysis_mode_str_single,
                                )
                            except Exception as opt_err:
                                print(f"❌ Optimizer error for {symbol}: {opt_err}")
                                import traceback
                                traceback.print_exc()
                                trade_setup = None
                            ...
                            try:
                                ...
                                if use_backtest:
                                    ...
                                    primary_desc = f"Entries during {current_session.value.upper()}"
                                else:
                            ...
                            print(f"❌ Strategy/Optimizer error: {strat_err}")
                            import traceback
                            traceback.print_exc()
                        ...
                        # OPTIMIZER IS SOURCE OF TRUTH - ALWAYS use it for display
                        if trade_setup and trade_setup.stop_loss > 0 and trade_setup.tp1 > 0:
                            ...
                            # Optimizer failed - use mode-based defaults
                            single_mode_cfg = st.session_state.get('single_mode_config', {})
                            mode_max_sl = single_mode_cfg.get('max_sl', 2.5)
                            mode_min_tp1 = single_mode_cfg.get('min_tp1', 1.5)
                            direction_for_calc = signal.direction if signal else 'LONG'
                            
                            display_entry = current_price
                            if direction_for_calc == 'LONG':
                                display_sl = current_price * (1 - mode_max_sl / 100)
                                display_tp1 = current_price * (1 + mode_min_tp1 / 100)
                                display_tp2 = current_price * (1 + mode_min_tp1 * 2 / 100)
                                display_tp3 = current_price * (1 + mode_min_tp1 * 3 / 100)
                            else:
                                display_sl = current_price * (1 + mode_max_sl / 100)
                                display_tp1 = current_price * (1 - mode_min_tp1 / 100)
                                display_tp2 = current_price * (1 - mode_min_tp1 * 2 / 100)
                            ...
                            display_risk_pct = mode_max_sl
                            display_rr = mode_min_tp1 / mode_max_sl if mode_max_sl > 0 else 0
                        ...
                        if use_strategy_entry:
                            ...
                            # OPTIMIZER IS SOURCE OF TRUTH - ALWAYS use it
                            if trade_setup and trade_setup.stop_loss > 0:
                                strategy_sl = trade_setup.stop_loss
                                strategy_tp1 = trade_setup.tp1
                                strategy_tp2 = trade_setup.tp2
                                strategy_tp3 = trade_setup.tp3
                            else:
                                # Optimizer failed - calculate from mode constraints
                                single_mode_cfg = st.session_state.get('single_mode_config', {})
                                mode_max_sl = single_mode_cfg.get('max_sl', 2.5)
                                mode_min_tp1 = single_mode_cfg.get('min_tp1', 1.5)
                                direction_for_calc = signal.direction if signal else 'LONG'
                                if direction_for_calc == 'LONG':
                                    strategy_sl = current_price * (1 - mode_max_sl / 100)
                                    strategy_tp1 = current_price * (1 + mode_min_tp1 / 100)
                                    strategy_tp2 = current_price * (1 + mode_min_tp1 * 2 / 100)
                                    strategy_tp3 = current_price * (1 + mode_min_tp1 * 3 / 100)
                                else:
                                    strategy_sl = current_price * (1 + mode_max_sl / 100)
                                    strategy_tp1 = current_price * (1 - mode_min_tp1 / 100)
                                    strategy_tp2 = current_price * (1 - mode_min_tp1 * 2 / 100)
                            ...
                            # Even with limit entry from signal, STILL use optimizer!
                        ...
                        if optimizer_wait:
                            ...
                            # Show SL for WAIT
                            # Get EDUCATIONAL descriptions from OPTIMIZER (source of truth)
                            wait_sl_desc = ""
                            wait_tp1_desc = ""
                            wait_tp2_desc = ""
                            wait_tp3_desc = ""
                            ...
                            if trade_setup:
                                # Use NEW educational description fields
                                wait_sl_desc = getattr(trade_setup, 'sl_description', '') or trade_setup.sl_type or "Structure-based"
                                wait_tp1_desc = getattr(trade_setup, 'tp1_description', '') or trade_setup.tp1_type or "Structure-based"
                                wait_tp2_desc = getattr(trade_setup, 'tp2_description', '') or trade_setup.tp2_type or "ATR extension"
                                wait_tp3_desc = getattr(trade_setup, 'tp3_description', '') or trade_setup.tp3_type or "ATR extension"
                            ...
                            wait_sl_desc_html = wait_sl_desc.replace('\n', '<br>') if wait_sl_desc else ""
                            wait_tp1_desc_html = wait_tp1_desc.replace('\n', '<br>') if wait_tp1_desc else ""
                            wait_tp2_desc_html = wait_tp2_desc.replace('\n', '<br>') if wait_tp2_desc else ""
                            wait_tp3_desc_html = wait_tp3_desc.replace('\n', '<br>') if wait_tp3_desc else ""
                            
                            # NOW display SL with educational description
                            st.markdown(f"""
                            <div style='background: #3a1a1a; padding: 12px; border-radius: 8px; margin-bottom: 10px;'>
                                <div style='display: flex; justify-content: space-between; align-items: center;'>
                                    ...
                                    <span style='color: #ff6666; font-weight: bold;'>Risk: {calc_risk_pct:.1f}%</span>
                                </div>
                                <div style='color: #ffaaaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{wait_sl_desc_html}</div>
                            </div>
                            """, unsafe_allow_html=True)
                            ...
                            st.markdown("<div style='color: #aaa; font-size: 0.9em; margin: 15px 0 8px 0; font-weight: bold;'>📊 Take Profit Targets:</div>", unsafe_allow_html=True)
                            
                            # TP1 with educational description
                            st.markdown(f"""
                            <div style='background: #1a2a1a; padding: 12px 15px; border-radius: 8px; margin: 8px 0; border-left: 4px solid #00d4aa;'>
                                <div style='display: flex; justify-content: space-between; align-items: center;'>
                                    <span><strong style='color: #00d4aa; font-size: 1.1em;'>TP1:</strong> {fmt_price(display_tp1)} <span style='color: #88ddaa;'>(+{tp1_pct:.1f}%)</span></span>
                                    <div style='text-align: right;'>
                                        <span style='color: {rr1_color}; font-weight: bold; font-size: 1.1em;'>R:R {rr_tp1:.1f}:1</span>
                                        <div style='color: #ffcc00; font-size: 0.8em;'>Take 33%</div>
                                    </div>
                                </div>
                                <div style='color: #aaddaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{wait_tp1_desc_html}</div>
                            </div>
                            """, unsafe_allow_html=True)
                            
                            # TP2 with educational description
                            st.markdown(f"""
                            <div style='background: #1a2a1a; padding: 12px 15px; border-radius: 8px; margin: 8px 0; border-left: 4px solid #00d4aa;'>
                                <div style='display: flex; justify-content: space-between; align-items: center;'>
                                    <span><strong style='color: #00d4aa; font-size: 1.1em;'>TP2:</strong> {fmt_price(display_tp2)} <span style='color: #88ddaa;'>(+{tp2_pct:.1f}%)</span></span>
                                    <div style='text-align: right;'>
                                        <span style='color: {rr2_color}; font-weight: bold; font-size: 1.1em;'>R:R {rr_tp2:.1f}:1</span>
                                        <div style='color: #ffcc00; font-size: 0.8em;'>Take 33%</div>
                                    </div>
                                </div>
                                <div style='color: #aaddaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{wait_tp2_desc_html}</div>
                            </div>
                            """, unsafe_allow_html=True)
                            
                            # TP3 with educational description
                            st.markdown(f"""
                            <div style='background: #1a2a1a; padding: 12px 15px; border-radius: 8px; margin: 8px 0; border-left: 4px solid #00ff88;'>
                                <div style='display: flex; justify-content: space-between; align-items: center;'>
                                    <span><strong style='color: #00ff88; font-size: 1.1em;'>TP3:</strong> {fmt_price(display_tp3)} <span style='color: #88ffaa;'>(+{tp3_pct:.1f}%)</span></span>
                                    <div style='text-align: right;'>
                                        <span style='color: {rr3_color}; font-weight: bold; font-size: 1.1em;'>R:R {rr_tp3:.1f}:1</span>
                                        <div style='color: #00ff88; font-size: 0.8em;'>Take remaining</div>
                                    </div>
                                </div>
                                <div style='color: #aaddaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{wait_tp3_desc_html}</div>
                            </div>
                            """, unsafe_allow_html=True)
                            ...
                            # Calculate ROI from MARKET ENTRY (current_price), not limit entry
                            market_entry = current_price
                            
                            if direction_for_calc == 'LONG':
                                risk_pct = abs((market_entry - strategy_sl) / market_entry * 100) if market_entry > 0 else 1
                                tp1_roi = ((strategy_tp1 - market_entry) / market_entry * 100) if market_entry > 0 else 0
                                tp2_roi = ((strategy_tp2 - market_entry) / market_entry * 100) if market_entry > 0 else 0
                                tp3_roi = ((strategy_tp3 - market_entry) / market_entry * 100) if market_entry > 0 else 0
                            else:
                                risk_pct = abs((strategy_sl - market_entry) / market_entry * 100) if market_entry > 0 else 1
                                tp1_roi = ((market_entry - strategy_tp1) / market_entry * 100) if market_entry > 0 else 0
                                tp2_roi = ((market_entry - strategy_tp2) / market_entry * 100) if market_entry > 0 else 0
                                tp3_roi = ((market_entry - strategy_tp3) / market_entry * 100) if market_entry > 0 else 0
                            ...
                            # Get EDUCATIONAL descriptions from trade_setup (OPTIMIZER IS SOURCE OF TRUTH)
                            sl_desc = ""
                            tp1_desc = ""
                            tp2_desc = ""
                            tp3_desc = ""
                            ...
                            if trade_setup:
                                # Use NEW educational description fields
                                sl_desc = getattr(trade_setup, 'sl_description', '') or trade_setup.sl_type or "Structure-based"
                                tp1_desc = getattr(trade_setup, 'tp1_description', '') or trade_setup.tp1_type or "Structure-based"
                                tp2_desc = getattr(trade_setup, 'tp2_description', '') or trade_setup.tp2_type or "ATR extension"
                                tp3_desc = getattr(trade_setup, 'tp3_description', '') or trade_setup.tp3_type or "ATR extension"
                            else:
                                # Fallback descriptions when optimizer unavailable
                                single_mode_cfg = st.session_state.get('single_mode_config', {})
                                mode_name = single_mode_cfg.get('name', 'Day Trade')
                                mode_max_sl = single_mode_cfg.get('max_sl', 2.5)
                                mode_min_tp1 = single_mode_cfg.get('min_tp1', 1.5)
                                sl_desc = f"📍 Source: {mode_name} mode maximum {mode_max_sl}%\n⚠️ Using mode defaults (optimizer unavailable)\n💡 Consider waiting for better structure"
                                tp1_desc = f"📍 Source: {mode_name} minimum {mode_min_tp1}%\n💡 Mode-based target"
                                tp2_desc = f"📍 Source: ATR-based extension\n💡 Extended from TP1"
                                tp3_desc = f"📍 Source: ATR-based extension\n💡 Extended from TP2"
                            
                            # Convert newlines to HTML breaks for display
                            sl_desc_html = sl_desc.replace('\n', '<br>') if sl_desc else ""
                            tp1_desc_html = tp1_desc.replace('\n', '<br>') if tp1_desc else ""
                            tp2_desc_html = tp2_desc.replace('\n', '<br>') if tp2_desc else ""
                            tp3_desc_html = tp3_desc.replace('\n', '<br>') if tp3_desc else ""
                            
                            # Show OPTIMIZER levels with metrics and EDUCATIONAL DESCRIPTIONS
                            st.markdown(f"""
                            <div style='background: #3a1a1a; padding: 12px; border-radius: 8px; margin-bottom: 10px;'>
                                <div style='display: flex; justify-content: space-between; align-items: center;'>
                                    <span><strong style='color: #ff4444;'>Stop Loss:</strong> {fmt_price(strategy_sl)}</span>
                                    <span style='color: #ff6666; font-weight: bold;'>Risk: {risk_pct:.1f}%</span>
                                </div>
                                <div style='color: #ffaaaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{sl_desc_html}</div>
                            </div>
                            """, unsafe_allow_html=True)
                            ...
                            <div style='background: #1a2a1a; padding: 12px 15px; border-radius: 8px; margin: 8px 0; border-left: 4px solid #00d4aa;'>
                                <div style='display: flex; justify-content: space-between; align-items: center;'>
                                    <span><strong style='color: #00d4aa; font-size: 1.1em;'>TP1:</strong> {fmt_price(strategy_tp1)} <span style='color: #88ddaa;'>(+{tp1_roi:.1f}%)</span></span>
                                    <div style='text-align: right;'>
                                        <span style='color: {rr1_color}; font-weight: bold; font-size: 1.1em;'>R:R {rr_tp1:.1f}:1</span>
                                        <div style='color: #ffcc00; font-size: 0.8em;'>Take 33%</div>
                                    </div>
                                </div>
                                <div style='color: #aaddaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{tp1_desc_html}</div>
                            </div>
                            """, unsafe_allow_html=True)
                            ...
                            <div style='background: #1a2a1a; padding: 12px 15px; border-radius: 8px; margin: 8px 0; border-left: 4px solid #00d4aa;'>
                                <div style='display: flex; justify-content: space-between; align-items: center;'>
                                    <span><strong style='color: #00d4aa; font-size: 1.1em;'>TP2:</strong> {fmt_price(strategy_tp2)} <span style='color: #88ddaa;'>(+{tp2_roi:.1f}%)</span></span>
                                    <div style='text-align: right;'>
                                        <span style='color: {rr2_color}; font-weight: bold; font-size: 1.1em;'>R:R {rr_tp2:.1f}:1</span>
                                        <div style='color: #ffcc00; font-size: 0.8em;'>Take 33%</div>
                                    </div>
                                </div>
                                <div style='color: #aaddaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{tp2_desc_html}</div>
                            </div>
                            """, unsafe_allow_html=True)
                            ...
                            <div style='background: #1a2a1a; padding: 12px 15px; border-radius: 8px; margin: 8px 0; border-left: 4px solid #00ff88;'>
                                <div style='display: flex; justify-content: space-between; align-items: center;'>
                                    <span><strong style='color: #00ff88; font-size: 1.1em;'>TP3:</strong> {fmt_price(strategy_tp3)} <span style='color: #88ffaa;'>(+{tp3_roi:.1f}%)</span></span>
                                    <div style='text-align: right;'>
                                        <span style='color: {rr3_color}; font-weight: bold; font-size: 1.1em;'>R:R {rr_tp3:.1f}:1</span>
                                        <div style='color: #00ff88; font-size: 0.8em;'>Take remaining</div>
                                    </div>
                                </div>
                                <div style='color: #aaddaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{tp3_desc_html}</div>
                            </div>
                            """, unsafe_allow_html=True)
                            ...
                            if limit_is_valid:
                                # Get descriptions from trade_setup
                                sl_desc_limit = ""
                                tp1_desc_limit = ""
                                tp2_desc_limit = ""
                                tp3_desc_limit = ""
                                ...
                                if trade_setup:
                                    sl_desc_limit = getattr(trade_setup, 'sl_description', '') or trade_setup.sl_type or "Structure-based"
                                    tp1_desc_limit = getattr(trade_setup, 'tp1_description', '') or trade_setup.tp1_type or "Structure-based"
                                    tp2_desc_limit = getattr(trade_setup, 'tp2_description', '') or trade_setup.tp2_type or "ATR extension"
                                    tp3_desc_limit = getattr(trade_setup, 'tp3_description', '') or trade_setup.tp3_type or "ATR extension"
                                
                                sl_desc_limit_html = sl_desc_limit.replace('\n', '<br>') if sl_desc_limit else ""
                                tp1_desc_limit_html = tp1_desc_limit.replace('\n', '<br>') if tp1_desc_limit else ""
                                tp2_desc_limit_html = tp2_desc_limit.replace('\n', '<br>') if tp2_desc_limit else ""
                                tp3_desc_limit_html = tp3_desc_limit.replace('\n', '<br>') if tp3_desc_limit else ""
                                ...
                                # Show SL with description
                                st.markdown(f"""
                                <div style='background: #3a1a1a; padding: 12px; border-radius: 8px; margin-bottom: 10px;'>
                                    <div style='display: flex; justify-content: space-between; align-items: center;'>
                                        <span><strong style='color: #ff4444;'>Stop Loss:</strong> {fmt_price(display_sl)}</span>
                                        <span style='color: #ff6666; font-weight: bold;'>Risk: {display_risk_pct:.1f}%</span>
                                    </div>
                                    <div style='color: #ffaaaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{sl_desc_limit_html}</div>
                                </div>
                                """, unsafe_allow_html=True)
                                ...
                                <div style='background: #1a2a1a; padding: 12px 15px; border-radius: 8px; margin: 8px 0; border-left: 4px solid #00d4aa;'>
                                    ...
                                    <div style='color: #aaddaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{tp1_desc_limit_html}</div>
                                </div>
                                """, unsafe_allow_html=True)
                                
                                # TP2 with description
                                st.markdown(f"""
                                <div style='background: #1a2a1a; padding: 12px 15px; border-radius: 8px; margin: 8px 0; border-left: 4px solid #00d4aa;'>
                                    <div style='display: flex; justify-content: space-between; align-items: center;'>
                                        <span><strong style='color: #00d4aa; font-size: 1.1em;'>TP2:</strong> {fmt_price(display_tp2)} <span style='color: #88ddaa;'>(+{tp2_pct:.1f}%)</span></span>
                                    ...
                                    <div style='color: #aaddaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{tp2_desc_limit_html}</div>
                                </div>
                                """, unsafe_allow_html=True)
                                
                                # TP3 with description
                                st.markdown(f"""
                                <div style='background: #1a2a1a; padding: 12px 15px; border-radius: 8px; margin: 8px 0; border-left: 4px solid #00ff88;'>
                                    <div style='display: flex; justify-content: space-between; align-items: center;'>
                                        <span><strong style='color: #00ff88; font-size: 1.1em;'>TP3:</strong> {fmt_price(display_tp3)} <span style='color: #88ffaa;'>(+{tp3_pct:.1f}%)</span></span>
                                    ...
                                    <div style='color: #aaddaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{tp3_desc_limit_html}</div>
                                </div>
                                """, unsafe_allow_html=True)
                            else:
                                ...
                                if trade_setup:
                                    sl_desc_inv = getattr(trade_setup, 'sl_description', '') or trade_setup.sl_type or "Structure-based"
                                    tp1_desc_inv = getattr(trade_setup, 'tp1_description', '') or trade_setup.tp1_type or "Structure-based"
                                    tp2_desc_inv = getattr(trade_setup, 'tp2_description', '') or trade_setup.tp2_type or "ATR extension"
                                    tp3_desc_inv = getattr(trade_setup, 'tp3_description', '') or trade_setup.tp3_type or "ATR extension"
                                
                                sl_desc_inv_html = sl_desc_inv.replace('\n', '<br>') if sl_desc_inv else ""
                                tp1_desc_inv_html = tp1_desc_inv.replace('\n', '<br>') if tp1_desc_inv else ""
                                tp2_desc_inv_html = tp2_desc_inv.replace('\n', '<br>') if tp2_desc_inv else ""
                                tp3_desc_inv_html = tp3_desc_inv.replace('\n', '<br>') if tp3_desc_inv else ""
                                ...
                                <div style='background: #3a1a1a; padding: 12px; border-radius: 8px; margin-bottom: 10px;'>
                                    ...
                                    <div style='color: #ffaaaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{sl_desc_inv_html}</div>
                                </div>
                                """, unsafe_allow_html=True)
                                ...
                                <div style='background: #1a2a1a; padding: 12px 15px; border-radius: 8px; margin: 8px 0; border-left: 4px solid #00d4aa;'>
                                    <div style='display: flex; justify-content: space-between; align-items: center;'>
                                        <span><strong style='color: #00d4aa; font-size: 1.1em;'>TP1:</strong> {fmt_price(display_tp1)} <span style='color: #88ddaa;'>(+{tp1_pct_m:.1f}%)</span></span>
                                        <div style='text-align: right;'>
                                            <span style='color: {rr1_color_m}; font-weight: bold; font-size: 1.1em;'>R:R {rr_tp1_m:.1f}:1</span>
                                            <div style='color: #ffcc00; font-size: 0.8em;'>Take 33%</div>
                                        </div>
                                    </div>
                                    <div style='color: #aaddaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{tp1_desc_inv_html}</div>
                                </div>
                                """, unsafe_allow_html=True)
                                ...
                                <div style='background: #1a2a1a; padding: 12px 15px; border-radius: 8px; margin: 8px 0; border-left: 4px solid #00d4aa;'>
                                    <div style='display: flex; justify-content: space-between; align-items: center;'>
                                        <span><strong style='color: #00d4aa; font-size: 1.1em;'>TP2:</strong> {fmt_price(display_tp2)} <span style='color: #88ddaa;'>(+{tp2_pct_m:.1f}%)</span></span>
                                        <div style='text-align: right;'>
                                            <span style='color: {rr2_color_m}; font-weight: bold; font-size: 1.1em;'>R:R {rr_tp2_m:.1f}:1</span>
                                            <div style='color: #ffcc00; font-size: 0.8em;'>Take 33%</div>
                                        </div>
                                    </div>
                                    <div style='color: #aaddaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{tp2_desc_inv_html}</div>
                                </div>
                                """, unsafe_allow_html=True)
                                ...
                                <div style='background: #1a2a1a; padding: 12px 15px; border-radius: 8px; margin: 8px 0; border-left: 4px solid #00ff88;'>
                                    <div style='display: flex; justify-content: space-between; align-items: center;'>
                                        <span><strong style='color: #00ff88; font-size: 1.1em;'>TP3:</strong> {fmt_price(display_tp3)} <span style='color: #88ffaa;'>(+{tp3_pct_m:.1f}%)</span></span>
                                        <div style='text-align: right;'>
                                            <span style='color: {rr3_color_m}; font-weight: bold; font-size: 1.1em;'>R:R {rr_tp3_m:.1f}:1</span>
                                            <div style='color: #00ff88; font-size: 0.8em;'>Take remaining</div>
                                        </div>
                                    </div>
                                    <div style='color: #aaddaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{tp3_desc_inv_html}</div>
                                </div>
                                """, unsafe_allow_html=True)
                            ...
                            if trade_setup:
                                sl_desc_mkt = getattr(trade_setup, 'sl_description', '') or trade_setup.sl_type or "Structure-based"
                                tp1_desc_mkt = getattr(trade_setup, 'tp1_description', '') or trade_setup.tp1_type or "Structure-based"
                                tp2_desc_mkt = getattr(trade_setup, 'tp2_description', '') or trade_setup.tp2_type or "ATR extension"
                                tp3_desc_mkt = getattr(trade_setup, 'tp3_description', '') or trade_setup.tp3_type or "ATR extension"
                            
                            sl_desc_mkt_html = sl_desc_mkt.replace('\n', '<br>') if sl_desc_mkt else ""
                            tp1_desc_mkt_html = tp1_desc_mkt.replace('\n', '<br>') if tp1_desc_mkt else ""
                            tp2_desc_mkt_html = tp2_desc_mkt.replace('\n', '<br>') if tp2_desc_mkt else ""
                            tp3_desc_mkt_html = tp3_desc_mkt.replace('\n', '<br>') if tp3_desc_mkt else ""
                            ...
                            <div style='background: #3a1a1a; padding: 12px; border-radius: 8px; margin-bottom: 10px;'>
                                ...
                                <div style='color: #ffaaaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{sl_desc_mkt_html}</div>
                            </div>
                            """, unsafe_allow_html=True)
                            ...
                            <div style='background: #1a2a1a; padding: 12px 15px; border-radius: 8px; margin: 8px 0; border-left: 4px solid #00d4aa;'>
                                ...
                                <div style='color: #aaddaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{tp1_desc_mkt_html}</div>
                            </div>
                            """, unsafe_allow_html=True)
                            ...
                            <div style='background: #1a2a1a; padding: 12px 15px; border-radius: 8px; margin: 8px 0; border-left: 4px solid #00d4aa;'>
                                ...
                                <div style='color: #aaddaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{tp2_desc_mkt_html}</div>
                            </div>
                            """, unsafe_allow_html=True)
                            ...
                            <div style='background: #1a2a1a; padding: 12px 15px; border-radius: 8px; margin: 8px 0; border-left: 4px solid #00ff88;'>
                                ...
                                <div style='color: #aaddaa; font-size: 0.85em; margin-top: 8px; line-height: 1.4;'>{tp3_desc_mkt_html}</div>
                            </div>
                            """, unsafe_allow_html=True)
                        ...
                        # ═══════════════════════════════════════════════════════════
                        # SESSION TIMING WARNING
                        # Check if ETA extends beyond current session
                        # ═══════════════════════════════════════════════════════════
                        try:
                            from datetime import datetime
                            from core.strategy_backtester import detect_current_session, SESSION_HOURS
                            
                            current_sess = detect_current_session()
                            if current_sess:
                                sess_end = SESSION_HOURS.get(current_sess, (0, 24))[1]
                                now_utc = datetime.utcnow()
                                hours_to_session_end = (sess_end - now_utc.hour) % 24
                                if hours_to_session_end == 0:
                                    hours_to_session_end = 24  # If at session end, next end is 24h away
                                
                                # Parse ETA hours
                                eta_str = tp1_eta.get('time_str', '0h')
                                try:
                                    if 'h' in eta_str:
                                        eta_hours = float(eta_str.replace('~', '').replace('h', '').strip())
                                    elif 'm' in eta_str:
                                        eta_hours = float(eta_str.replace('~', '').replace('m', '').strip()) / 60
                                    else:
                                        eta_hours = 0
                                except:
                                    eta_hours = 0
                                
                                if eta_hours > hours_to_session_end and eta_hours > 0:
                                    next_session = {
                                        'asia': 'LONDON',
                                        'london': 'NEW YORK',
                                        'new_york': 'ASIA'
                                    }.get(current_sess.value, 'NEXT')
                                    
                                    st.markdown(f"""
                                    <div style='background: #2a2a1a; border: 1px solid #ffcc00; border-radius: 8px; 
                                                padding: 10px 15px; margin-top: 10px;'>
                                        <span style='color: #ffcc00;'>⏰ Session Timing Note</span>
                                        <span style='color: #aaa; margin-left: 10px;'>
                                            ETA ~{eta_hours:.1f}h but {current_sess.value.upper()} ends in ~{hours_to_session_end:.0f}h
                                        </span>
                                        <div style='color: #888; font-size: 0.85em; margin-top: 5px;'>
                                            Trade may close during {next_session} session • WR is based on ENTRY session
                                        </div>
                                    </div>
                                    """, unsafe_allow_html=True)
                        except Exception as sess_warn_err:
                    ...
                    if already_added:
                        ...
                        if st.button(f"➕ Add {symbol_input} to Monitor", type="primary", use_container_width=True, key=add_key):
                            ...
                            trade = {
                                ...
                                # ML vs Rules tracking - CRITICAL for performance analysis
                                'ml_direction': ml_prediction.direction if ml_prediction and hasattr(ml_prediction, 'direction') else None,
                                'ml_confidence': ml_prediction.confidence if ml_prediction and hasattr(ml_prediction, 'confidence') else 0,
                                'rules_direction': signal.direction if signal else direction,
                                'has_ml_conflict': (
                                    ml_prediction and hasattr(ml_prediction, 'direction') and 
                                    signal and ml_prediction.direction != signal.direction
                                ) if ml_prediction else False,
                            }
