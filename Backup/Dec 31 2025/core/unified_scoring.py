...
def blend_hybrid_prediction(
    ...
    money_flow_phase: str = None,  # Money Flow phase: ACCUMULATION, MARKUP, DISTRIBUTION, MARKDOWN
    structure: str = None,  # Market structure: Bullish, Bearish, Ranging
    squeeze_pct: float = 50,  # BB compression % (0-100, higher = more compressed)
    explosion_data: dict = None,  # Full explosion detector data
) -> dict:
    """
    ...
    Args:
        ...
        money_flow_phase: Money Flow phase (ACCUMULATION, MARKUP, DISTRIBUTION, MARKDOWN)
        structure: Market structure (Bullish, Bearish, Ranging)
        squeeze_pct: BB compression percentage (0-100). >80% = LOADED
        explosion_data: Full explosion detector dict with score, ready, direction, squeeze
        rule_direction: Direction from rules ('BULLISH', 'BEARISH', etc.)
        ml_direction: Direction from ML ('LONG', 'SHORT', 'WAIT')
        ml_confidence: ML confidence percentage (0-100)
        position_pct: Position in range (0-100). 0=bottom, 100=top
    ...
    Setup Detection:
        ...
        - STRUCTURE OVERRIDE: Structure + Flow + Whales all agree → Trust structure over ML
    ...
    oi_negative = oi_change < 0  # Any negative OI = positions closing
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # ═══════════════════════════════════════════════════════════════════════════
    # 🏗️ STRUCTURE OVERRIDE CHECK - Pre-calculate for later use
    # If Structure + Money Flow + Whales ALL agree → Trust structure over ML!
    # ═══════════════════════════════════════════════════════════════════════════
    # Normalize inputs (ensure booleans, not None)
    flow_is_bullish = bool(money_flow_phase and money_flow_phase in ['ACCUMULATION', 'MARKUP'])
    flow_is_bearish = bool(money_flow_phase and money_flow_phase in ['DISTRIBUTION', 'MARKDOWN'])
    structure_is_bullish = bool(structure and structure.lower() in ['bullish', 'hh+hl', 'higher highs'])
    structure_is_bearish = bool(structure and structure.lower() in ['bearish', 'lh+ll', 'lower lows'])
    
    # Count bullish confirmations (all must be bool for sum())
    bullish_confirmations = sum([
        bool(whales_bullish),     # Whales > 65%
        flow_is_bullish,          # Money Flow = ACCUMULATION or MARKUP
        structure_is_bullish,     # Structure = Bullish (HH+HL)
    ])
    
    # Count bearish confirmations (all must be bool for sum())
    bearish_confirmations = sum([
        bool(whales_bearish),     # Whales < 35%
        flow_is_bearish,          # Money Flow = DISTRIBUTION or MARKDOWN
        structure_is_bearish,     # Structure = Bearish (LH+LL)
    ])
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # ⚠️ ML EXIT CAUTION: MIDDLE position + ML SHORT + OI negative
    # BUT: If Structure + Flow + Whales all agree → Trust structure over ML!
    # ═══════════════════════════════════════════════════════════════════════════
    elif whales_bullish and is_middle and ml_is_bearish and ml_confidence >= 60 and oi_negative:
        # Check for STRUCTURE OVERRIDE - all 3 bullish signals agree
        if bullish_confirmations >= 3:
            setup_type = 'STRUCTURE_OVERRIDE_LONG'
            # Structure + Flow + Whales ALL agree bullish → Trust them over ML
            # ML might be seeing a temporary pullback, not a reversal
        else:
            setup_type = 'ML_EXIT_CAUTION'
            # Only 1-2 bullish signals → ML caution is valid
        
    elif whales_bearish and is_middle and ml_is_bullish and ml_confidence >= 60 and oi_negative:
        # Check for STRUCTURE OVERRIDE - all 3 bearish signals agree
        if bearish_confirmations >= 3:
            setup_type = 'STRUCTURE_OVERRIDE_SHORT'
            # Structure + Flow + Whales ALL agree bearish → Trust them over ML
        else:
            setup_type = 'ML_REVERSAL_CAUTION'
            # Only 1-2 bearish signals → ML caution is valid
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # 🚀 EXPLOSION READY: Full explosion detector signals imminent move!
    # This combines: Compression + Squeeze + Liquidity + Session timing
    # When explosion_ready=True OR explosion_score >= 60, trust the direction!
    # ═══════════════════════════════════════════════════════════════════════════
    ...
    exp_ready = explosion_data.get('ready', False) if explosion_data else False
    exp_direction = explosion_data.get('direction') if explosion_data else None
    exp_squeeze_pct = squeeze_pct  # Use passed squeeze_pct as fallback
    
    # Try to get more accurate squeeze_pct from explosion data
    if explosion_data and 'squeeze_pct' in explosion_data:
        exp_squeeze_pct = explosion_data.get('squeeze_pct', squeeze_pct)
    elif explosion_data and 'squeeze' in explosion_data:
        exp_squeeze_pct = explosion_data.get('squeeze', {}).get('squeeze_percentile', squeeze_pct)
    
    # EXPLOSION READY - Highest priority when explosion detector says GO!
    if exp_ready and not is_late:
        # Explosion detector says imminent move - trust its direction!
        if exp_direction == 'LONG' and whale_pct >= 55:
            if ml_is_neutral or ml_is_bullish or (ml_is_bearish and ml_confidence < 75):
                setup_type = 'EXPLOSION_LONG'
                tp_multiplier = 1.5  # Bigger TPs for confirmed explosion
        elif exp_direction == 'SHORT' and whale_pct <= 45:
            if ml_is_neutral or ml_is_bearish or (ml_is_bullish and ml_confidence < 75):
                setup_type = 'EXPLOSION_SHORT'
                tp_multiplier = 1.5
        # If explosion direction unknown, use whale direction
        elif exp_direction is None and whale_pct >= 60 and rule_dir_normalized == 'LONG':
            if ml_is_neutral or ml_is_bullish or (ml_is_bearish and ml_confidence < 70):
                setup_type = 'EXPLOSION_LONG'
                tp_multiplier = 1.4
        elif exp_direction is None and whale_pct <= 40 and rule_dir_normalized == 'SHORT':
            if ml_is_neutral or ml_is_bearish or (ml_is_bullish and ml_confidence < 70):
                setup_type = 'EXPLOSION_SHORT'
                tp_multiplier = 1.4
    
    # HIGH EXPLOSION SCORE (≥60) - Strong setup even if not "ready"
    elif exp_score >= 60 and not is_late:
        if whale_pct >= 60 and rule_dir_normalized == 'LONG':
            if ml_is_neutral or ml_is_bullish or (ml_is_bearish and ml_confidence < 70):
                setup_type = 'COMPRESSION_LONG'
                tp_multiplier = 1.4
        elif whale_pct <= 40 and rule_dir_normalized == 'SHORT':
            if ml_is_neutral or ml_is_bearish or (ml_is_bullish and ml_confidence < 70):
                setup_type = 'COMPRESSION_SHORT'
                tp_multiplier = 1.4
    
    # ═══════════════════════════════════════════════════════════════════════════
    # ⚡ COMPRESSION READY: BB LOADED (>80%) + Whales positioned + NOT late
    # Fallback when explosion_data not available but BB is very tight
    # ═══════════════════════════════════════════════════════════════════════════
    elif exp_squeeze_pct >= 80 and not is_late and setup_type is None:
        # BB is LOADED - explosion imminent! Trust whale direction
        if whale_pct >= 60 and rule_dir_normalized == 'LONG':
            if ml_is_neutral or ml_is_bullish or (ml_is_bearish and ml_confidence < 70):
                setup_type = 'COMPRESSION_LONG'
                tp_multiplier = 1.4  # Expect bigger move from compression release
        elif whale_pct <= 40 and rule_dir_normalized == 'SHORT':
            if ml_is_neutral or ml_is_bearish or (ml_is_bullish and ml_confidence < 70):
                setup_type = 'COMPRESSION_SHORT'
                tp_multiplier = 1.4
    ...
    # ML EXIT CAUTION: ML sees bearish, OI falling - be careful
    if setup_type == 'ML_EXIT_CAUTION':
        position_override = True
        position_override_reason = f"⚠️ CAUTION - ML SHORT {ml_confidence:.0f}% + OI {oi_change:+.1f}% (positions closing)"
    
    if setup_type == 'ML_REVERSAL_CAUTION':
        position_override = True
        position_override_reason = f"⚠️ CAUTION - ML LONG {ml_confidence:.0f}% + OI {oi_change:+.1f}% (shorts may cover)"
    
    # STRUCTURE OVERRIDE: Structure + Flow + Whales all agree → ALLOW trade despite ML
    # Do NOT set position_override - we WANT to trade!
    if setup_type == 'STRUCTURE_OVERRIDE_LONG':
        position_override = False  # Explicitly allow the trade
        # Will set conflict_note below with explanation
    
    if setup_type == 'STRUCTURE_OVERRIDE_SHORT':
        position_override = False  # Explicitly allow the trade
        # Will set conflict_note below with explanation
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # ⚡ COMPRESSION SETUP: BB LOADED + Whales positioned → Trust whale direction!
    # When BB is >80%, a big move is imminent. Trust whale direction over ML WAIT.
    # ═══════════════════════════════════════════════════════════════════════════
    elif setup_type in ['COMPRESSION_LONG', 'COMPRESSION_SHORT']:
        # BB is LOADED - explosion imminent! Trust whale direction
        base_score = int(rule_score * 0.80 + ml_score * 0.20)  # Weight rules heavily
        final_score = max(55, base_score)  # Floor at 55 for compression setups
        final_direction = rule_dir_normalized
        exp_dir = "LONG 📈" if setup_type == 'COMPRESSION_LONG' else "SHORT 📉"
        conflict_note = f"⚡ COMPRESSION: BB {exp_squeeze_pct:.0f}% LOADED + Whales {whale_pct:.0f}% → {exp_dir} explosion imminent!"
        # Don't apply position_override for compression setups
        position_override = False
    
    # ═══════════════════════════════════════════════════════════════════════════
    # 🚀 EXPLOSION SETUP: Full explosion detector confirms imminent move!
    # Combines: Compression + Squeeze + Liquidity + Session = HIGH CONVICTION
    # ═══════════════════════════════════════════════════════════════════════════
    elif setup_type in ['EXPLOSION_LONG', 'EXPLOSION_SHORT']:
        # Explosion detector confirms - highest conviction!
        base_score = int(rule_score * 0.75 + ml_score * 0.25)  # Trust rules heavily
        final_score = max(60, base_score)  # Floor at 60 for explosion setups
        final_direction = rule_dir_normalized
        exp_dir = "LONG 📈" if setup_type == 'EXPLOSION_LONG' else "SHORT 📉"
        conflict_note = f"🚀 EXPLOSION READY: Score {exp_score}/100 + Whales {whale_pct:.0f}% → {exp_dir}!"
        # Don't apply position_override for explosion setups
        position_override = False
    
    # ═══════════════════════════════════════════════════════════════════════════
    # 🏗️ STRUCTURE OVERRIDE: Structure + Flow + Whales ALL agree → Trust structure!
    # ML may see temporary pullback, but fundamentals are aligned
    # ═══════════════════════════════════════════════════════════════════════════
    elif setup_type in ['STRUCTURE_OVERRIDE_LONG', 'STRUCTURE_OVERRIDE_SHORT']:
        # Structure, Money Flow, and Whales all agree - trust the consensus over ML
        base_score = int(rule_score * 0.80 + ml_score * 0.20)  # Weight rules heavily
        final_score = max(55, base_score)  # Floor at 55 for structure override
        final_direction = rule_dir_normalized
        
        # Build explanation for Combined Learning
        flow_str = money_flow_phase or "BULLISH"
        struct_str = structure or "Bullish"
        if setup_type == 'STRUCTURE_OVERRIDE_LONG':
            conflict_note = f"🏗️ STRUCTURE OVERRIDE: Structure ({struct_str}) + Flow ({flow_str}) + Whales ({whale_pct:.0f}%) ALL bullish → Trust structure over ML SHORT"
        else:
            conflict_note = f"🏗️ STRUCTURE OVERRIDE: Structure ({struct_str}) + Flow ({flow_str}) + Whales ({100-whale_pct:.0f}% short) ALL bearish → Trust structure over ML LONG"
        
        # Don't apply position_override for structure override
        position_override = False
        ...
        elif setup_type in ['ML_EXIT_CAUTION', 'ML_REVERSAL_CAUTION']:
            final_score = max(30, final_score - 12)  # Not as bad as exit trap, but still penalized
    ...
    elif setup_type == 'EXPLOSION_LONG':
        final_action = f"🚀 {final_action} (EXPLOSION {exp_score}/100 → LONG)"
    elif setup_type == 'EXPLOSION_SHORT':
        final_action = f"🚀 {final_action} (EXPLOSION {exp_score}/100 → SHORT)"
    elif setup_type == 'COMPRESSION_LONG':
        final_action = f"⚡ {final_action} (BB {exp_squeeze_pct:.0f}% LOADED → LONG)"
    elif setup_type == 'COMPRESSION_SHORT':
        final_action = f"⚡ {final_action} (BB {exp_squeeze_pct:.0f}% LOADED → SHORT)"
    elif setup_type == 'STRUCTURE_OVERRIDE_LONG':
        final_action = f"🏗️ {final_action} (Structure + Flow + Whales agree LONG)"
    elif setup_type == 'STRUCTURE_OVERRIDE_SHORT':
        final_action = f"🏗️ {final_action} (Structure + Flow + Whales agree SHORT)"
