...
if app_mode == "🔍 Scanner":
    ...
    <div style='background: linear-gradient(135deg, #1a2a3a 0%, #1a1a2e 100%); border: 1px solid #00d4ff33; border-radius: 10px; padding: 15px; margin: 15px 0;'>
        ...
        <span class='subtitle-text' style='font-size: 0.85em; margin-left: 15px;'>• Think like a whale - Global market phase detection</span>
    </div>
    """, unsafe_allow_html=True)
        ...
        if pulse_data:
            ...
            <div style='display: flex; gap: 20px; align-items: center; flex-wrap: wrap;'>
                ...
                <div style='color: #555; font-size: 0.75em;'>
                    {coins_scanned} {asset_name} • {scan_time}{f" • 📊 {pulse_data.get('whale_records_stored', 0)} stored" if pulse_data.get('whale_records_stored', 0) > 0 else ""}
                </div>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown("<span style='color: #666;'>Click 'Scan Market' to analyze global market phase</span>", unsafe_allow_html=True)
    ...
    # Option to store data for historical validation (all market types)
    if "Crypto" in market_type:
        store_whale_history = st.checkbox(
            "📊 Store whale data for historical validation", 
            value=True,  # Default ON - builds valuable database
            key="store_whale_history_pulse",
            help="Stores OI, whale positioning, funding rate for each coin. Builds database for historical validation. Adds ~2-3 min to scan."
        )
    ...
    if scan_market_btn and not st.session_state.market_pulse_running:
        ...
        direction_counts = {'LONG': 0, 'SHORT': 0, 'WAIT': 0}
        ...
        for i, symbol in enumerate(all_pairs):
            progress_pct = (i + 1) / len(all_pairs)
            pulse_progress.progress(progress_pct)
            
            # Update status with data storage info
            if store_whale_history and "Crypto" in market_type:
                pulse_status.markdown(f"🔍 Scanning **{symbol}** ({i+1}/{len(all_pairs)}) • 📊 Storing whale data...")
            elif store_whale_history and ("Stock" in market_type or "ETF" in market_type):
                pulse_status.markdown(f"🔍 Scanning **{symbol}** ({i+1}/{len(all_pairs)}) • 📊 Storing institutional data...")
            else:
                pulse_status.markdown(f"🔍 Scanning **{symbol}** ({i+1}/{len(all_pairs)})")
            
            try:
                # Fetch data using appropriate function
                df = fetch_func(symbol, '15m', 200)
                if df is None or len(df) < 50:
                    continue
                
                # Use same analyze_symbol_full as Single Analysis (no API for speed)
                analysis = analyze_symbol_full(df, symbol, '15m', market_type, 'day_trade', fetch_whale_api=False)
                ...
                # Extract phase info
                mf_phase = analysis.get('money_flow_phase', 'UNKNOWN').upper()
        ...
        st.session_state.market_pulse = {
            ...
            'whale_records_stored': whale_records_stored  # New: track historical data
        }
    ...
    if pulse_data:
        with st.expander("📊 **Market Pulse Details** - Full Phase Breakdown", expanded=True):
            ...
            # Phase distribution
            st.markdown("**📈 Market Phase Distribution:**")
            phase_cols = st.columns(4)
            phase_info = [
                ('ACCUMULATION', '💰', '#00ff88', 'Smart money buying'),
                ('MARKUP', '🚀', '#00d4ff', 'Trend in progress'),
                ('DISTRIBUTION', '📤', '#ff9800', 'Whales taking profit'),
                ('MARKDOWN', '📉', '#ff4444', 'Market falling')
            ]
            
            phase_pcts = pulse_data.get('phase_pcts', {})
            phase_counts = pulse_data.get('phase_counts', {})
            asset_label = pulse_data.get('asset_name', 'coins')  # Dynamic label
            
            for i, (phase, icon, color, desc) in enumerate(phase_info):
                with phase_cols[i]:
            ...
            dir_counts = pulse_data.get('direction_counts', {})
            dir_cols = st.columns(3)
            
            with dir_cols[0]:
                st.markdown(f"""
                <div style='background: #0a2a0a; border-radius: 8px; padding: 15px; text-align: center;'>
                    <div style='color: #00ff88; font-size: 1.8em; font-weight: bold;'>{dir_counts.get('LONG', 0)}</div>
                    <div style='color: #00ff88;'>🟢 LONG</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with dir_cols[1]:
                ...
                <div style='background: #2a2a0a; border-radius: 8px; padding: 15px; text-align: center;'>
                    <div style='color: #ffcc00; font-size: 1.8em; font-weight: bold;'>{dir_counts.get('WAIT', 0)}</div>
                    <div style='color: #ffcc00;'>🟡 WAIT</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with dir_cols[2]:
                ...
                <div style='background: #2a0a0a; border-radius: 8px; padding: 15px; text-align: center;'>
                    <div style='color: #ff4444; font-size: 1.8em; font-weight: bold;'>{dir_counts.get('SHORT', 0)}</div>
                    <div style='color: #ff4444;'>🔴 SHORT</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with stat_cols[0]:
                st.metric("Avg Score", f"{pulse_data.get('avg_score', 50):.0f}")
            with stat_cols[1]:
                st.metric("Avg Position", f"{pulse_data.get('avg_position_pct', 50):.0f}%")
            with stat_cols[2]:
                st.metric("Scan Coverage", f"{pulse_data.get('coins_scanned', 0)}/{pulse_data.get('total_coins', 0)}")
        ...
        for i, symbol in enumerate(pairs):
            for tf in timeframes_to_scan:
                ...
                if scan_count > 1:
                    ...
                    time_info = f"⏱️ {format_time(elapsed)} elapsed | ~{format_time(est_remaining)} remaining"
                else:
        ...
        with st.spinner(f"Generating comprehensive analysis for {symbol_input}..."):
            ...
            if df is not None and len(df) >= 50:
                ...
                try:
                    ...
                    # Run validation (uses cached data if available)
                    ...
                    if hist_validation and hist_validation.matches_found >= 20:
                        ...
                        with st.expander(f"📊 **HISTORICAL VALIDATION** - {hist_validation.history_grade} ({hist_validation.matches_found} similar patterns)", expanded=True):
                            st.markdown(f"""
