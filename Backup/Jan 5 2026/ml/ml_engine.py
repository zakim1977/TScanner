...
class MLPrediction:
    ...
    optimal_tp2_pct: float      # Optimal TP2 distance % (Fib 1.618 extension)
    optimal_tp3_pct: float      # Optimal TP3 distance % (Fib 2.618 extension)
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            'optimal_tp3_pct': self.optimal_tp3_pct,
...
class MLEngine:
    ...
    def _load_mode_models(self):
        """Load mode-specific trained models for all asset types (crypto, stock, etf)."""
        modes = ['scalp', 'daytrade', 'swing', 'investment']
        asset_types = ['', '_stock', '_etf']  # '' = crypto (default)
        
        for mode in modes:
            for asset_suffix in asset_types:
                try:
                    suffix = f"{mode}{asset_suffix}"
                    dir_path = os.path.join(self.MODEL_DIR, f'direction_model_{suffix}.pkl')
                    eta_path = os.path.join(self.MODEL_DIR, f'eta_model_{suffix}.pkl')
                    sl_path = os.path.join(self.MODEL_DIR, f'sl_model_{suffix}.pkl')
                    scaler_path = os.path.join(self.MODEL_DIR, f'scaler_{suffix}.pkl')
                    meta_path = os.path.join(self.MODEL_DIR, f'metadata_{suffix}.json')
                    
                    if os.path.exists(dir_path) and os.path.exists(scaler_path):
                        # Key format: "scalp" for crypto, "swing_stock" for stocks, "swing_etf" for ETFs
                        model_key = suffix
                        self.mode_models[model_key] = {
        ...
        if self.mode_models:
            ...
            print(f"✅ ML models loaded: {modes_loaded}")
    
    def has_mode_model(self, mode: str = None, timeframe: str = None, market_type: str = 'crypto') -> bool:
        ...
        # Determine model key based on market type
        model_key = self._get_model_key(mode, market_type)
        return model_key in self.mode_models
    
    def _get_model_key(self, mode: str, market_type: str = 'crypto') -> str:
        """Get the model key based on mode and market type."""
        # Normalize market type
        if 'Stock' in market_type or 'stock' in market_type.lower():
            return f"{mode}_stock"
        elif 'ETF' in market_type or 'etf' in market_type.lower():
            return f"{mode}_etf"
        else:
            # Crypto - no suffix
            return mode
    ...
    def is_model_loaded(self) -> bool:
        ...
        # Check either general model or mode-specific models
        return self.is_trained or len(self.mode_models) > 0
    ...
    def predict_for_mode(
        ...
        **kwargs
        ...
        Args:
            ...
            market_type: Asset type ('crypto', 'Stock', 'ETF')
        ...
        # Get model key based on mode and market type
        model_key = self._get_model_key(mode, market_type)
        
        # Check if we have this specific model
        if model_key in self.mode_models:
            return self._predict_with_mode_model(model_key, features_dict, **kwargs)
        else:
            # Fall back to regular prediction
            return self.predict(**kwargs) if not features_dict else self.predict(**features_dict)
    
    def _predict_with_mode_model(self, model_key: str, features_dict: Dict = None, **kwargs) -> MLPrediction:
        """Predict using a mode-specific trained model.
        
        Args:
            model_key: Key to lookup model (e.g., 'scalp', 'swing_stock', 'investment_etf')
        """
        from .mode_specific_trainer import FEATURE_NAMES as MODE_FEATURE_NAMES, extract_features_from_row
        import pandas as pd
        
        mode_model = self.mode_models[model_key]
        ...
        # Build reasoning
        reasoning_parts = [f"Mode-specific ML ({model_key.upper()}) prediction"]
        if eta_candles:
        ...
        return MLPrediction(
            ...
            # TP2/TP3 using Fib extensions from TP1 (not hardcoded multipliers)
            optimal_tp2_pct=profit_thresh * 1.618,  # Golden ratio extension
            optimal_tp3_pct=profit_thresh * 2.618,  # Next Fib extension
            ...
            model_version=f"mode_specific_{model_key}",
    ...
    def _predict_with_model(self, feature_set: FeatureSet) -> MLPrediction:
        ...
        # Calculate TP3 using Fib extension
        optimal_tp3_pct = optimal_tp1_pct * 2.618
        ...
        return MLPrediction(
            ...
            optimal_tp3_pct=optimal_tp3_pct,
    ...
    def _predict_heuristic(self, feature_set: FeatureSet) -> MLPrediction:
        ...
        # Default TP/SL - Using Fib extensions for TP2/TP3
        if direction == 'LONG':
            optimal_tp1_pct = 2.5
            optimal_tp2_pct = optimal_tp1_pct * 1.618  # Fib extension
            optimal_tp3_pct = optimal_tp1_pct * 2.618  # Fib extension
            optimal_sl_pct = 1.5
        elif direction == 'SHORT':
            optimal_tp1_pct = 2.0
            optimal_tp2_pct = optimal_tp1_pct * 1.618  # Fib extension
...
def reload_engine():
    """Force reload the ML engine (useful after training new models)."""
    global _engine
    _engine = None
    return _get_engine()
...
def get_ml_prediction(**kwargs) -> MLPrediction:
    ...
    Uses mode-specific model if timeframe is provided and model exists.
    Supports different models for crypto, stocks, and ETFs.
    """
    engine = _get_engine()
    
    # Extract timeframe and market_type from kwargs (remove to avoid duplicate)
    timeframe = kwargs.pop('timeframe', None)
    market_type = kwargs.pop('market_type', 'crypto')
    
    if timeframe:
        mode = engine.get_mode_from_timeframe(timeframe)
        model_key = engine._get_model_key(mode, market_type)
        
        if engine.has_mode_model(mode=mode, market_type=market_type):
            return engine.predict_for_mode(timeframe=timeframe, market_type=market_type, **kwargs)
        # Try fallback to crypto model if stock/etf model not available
        elif market_type != 'crypto' and engine.has_mode_model(mode=mode, market_type='crypto'):
            return engine.predict_for_mode(timeframe=timeframe, market_type='crypto', **kwargs)
    
    # Fall back to regular prediction (heuristic)
