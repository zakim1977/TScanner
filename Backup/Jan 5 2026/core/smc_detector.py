...
def detect_order_blocks_ict(df: pd.DataFrame, swing_length: int = 10) -> Dict:
    """
    ...
    if df is None or len(df) < swing_length * 2:
        return result
    ...
    try:
        ...
        for sl_idx in swing_lows:
            ...
            # Check for strong up move after swing low (at least 1x ATR)
            future_high = df['High'].iloc[sl_idx+1:min(sl_idx+6, len(df))].max()
            move_up = future_high - df['Low'].iloc[sl_idx]
            
            if move_up >= atr * 0.8:  # Relaxed threshold (0.8x ATR instead of 1.5x)
                ...
                for j in range(sl_idx, max(sl_idx - 3, 0), -1):
                    if df['Close'].iloc[j] < df['Open'].iloc[j]:  # Down candle
                        ...
                        # Check if mitigated (price closed below OB bottom)
                        is_mitigated = False
                        for k in range(j + 1, len(df)):
                            if df['Close'].iloc[k] < ob_bottom:
                                is_mitigated = True
                                break
        ...
        for sh_idx in swing_highs:
            ...
            # Check for strong down move after swing high (at least 1x ATR)
            future_low = df['Low'].iloc[sh_idx+1:min(sh_idx+6, len(df))].min()
            move_down = df['High'].iloc[sh_idx] - future_low
            
            if move_down >= atr * 0.8:  # Relaxed threshold
                ...
                for j in range(sh_idx, max(sh_idx - 3, 0), -1):
                    if df['Close'].iloc[j] > df['Open'].iloc[j]:  # Up candle
                        ...
                        # Check if mitigated (price closed above OB top)
                        is_mitigated = False
                        for k in range(j + 1, len(df)):
                            if df['Close'].iloc[k] > ob_top:
                                is_mitigated = True
                                break
...
def detect_smc(df: pd.DataFrame, use_ict: bool = True) -> Dict:
    """
    ...
    ob_func = detect_order_blocks_ict if use_ict else detect_order_blocks
    
    return {
        'order_blocks': ob_func(df),
        'fvg': detect_fvg(df),
