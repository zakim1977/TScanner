...
def calculate_predictive_score(
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # LAYER 1: WHALE POSITIONING → Direction (0-40 points)
    # THIS IS THE LEADING INDICATOR - Whales position BEFORE moves happen
    # OI is CONFIRMATION, not the primary signal!
    # ═══════════════════════════════════════════════════════════════════════════
    
    # Get thresholds from knowledge base (single source of truth)
    wt = MONEY_FLOW_PHASE_RULES['whale_thresholds']
    
    # --- WHALE POSITIONING IS PRIMARY ---
    if whale_pct >= wt['extreme_bullish']:  # 75%
        direction = "BULLISH"
        direction_confidence = "HIGH"
        direction_score = 40  # Max score for extreme conviction
        direction_reason = f"🐋 EXTREME whale long ({whale_pct:.0f}%) - Highest conviction"
        
    elif whale_pct >= wt['strong_bullish']:  # 70%
        direction = "BULLISH"
        direction_confidence = "HIGH"
        direction_score = 36
        direction_reason = f"🐋 Whales heavily long ({whale_pct:.0f}%) - LEADING signal"
        
    elif whale_pct >= wt['bullish']:  # 65%
        direction = "BULLISH"
        direction_confidence = "HIGH"
        direction_score = 32
        direction_reason = f"🐋 Whales long ({whale_pct:.0f}%) - Clear direction"
        
    elif whale_pct >= wt['lean_bullish']:  # 60%
        direction = "LEAN_BULLISH"
        direction_confidence = "MEDIUM"
        direction_score = 28
        direction_reason = f"Whales lean long ({whale_pct:.0f}%) - Edge present"
        
    elif whale_pct >= wt['neutral_high']:  # 55%
        direction = "LEAN_BULLISH"
        direction_confidence = "LOW"
        direction_score = 22
        direction_reason = f"Whales slightly long ({whale_pct:.0f}%) - Slight edge"
        
    elif whale_pct <= wt['extreme_bearish']:  # 25%
        direction = "BEARISH"
        direction_confidence = "HIGH"
        direction_score = 40  # Max score for extreme conviction
        direction_reason = f"🐋 EXTREME whale short ({100-whale_pct:.0f}%S) - Highest conviction"
        
    elif whale_pct <= wt['strong_bearish']:  # 30%
        direction = "BEARISH"
        direction_confidence = "HIGH"
        direction_score = 36
        direction_reason = f"🐋 Whales heavily short ({100-whale_pct:.0f}%S) - LEADING signal"
        
    elif whale_pct <= wt['bearish']:  # 35%
        direction = "BEARISH"
        direction_confidence = "HIGH"
        direction_score = 32
        direction_reason = f"🐋 Whales short ({100-whale_pct:.0f}%S) - Clear direction"
        
    elif whale_pct <= wt['lean_bearish']:  # 40%
        direction = "LEAN_BEARISH"
        direction_confidence = "MEDIUM"
        direction_score = 28
        direction_reason = f"Whales lean short ({100-whale_pct:.0f}%S) - Edge present"
        
    elif whale_pct <= wt['neutral_low']:  # 45%
        direction = "LEAN_BEARISH"
        direction_confidence = "LOW"
        direction_score = 22
        direction_reason = f"Whales slightly short ({100-whale_pct:.0f}%S) - Slight edge"
        
    else:
        # TRUE NEUTRAL (45-55%) - no clear whale edge
        direction = "NEUTRAL"
        direction_confidence = "LOW"
        direction_score = 15  # Increased from 10 - neutral isn't bad, just unclear
        direction_reason = f"Whales neutral ({whale_pct:.0f}%L) - Wait for positioning"
    
    # --- OI CONFIRMATION BONUS (up to +4 points) ---
    # OI confirming whale direction = higher confidence, but doesn't gate the signal
    oi_up = oi_change > 0.5
    oi_down = oi_change < -0.5
    price_up = price_change > 0.5
    price_down = price_change < -0.5
    
    oi_confirms_bullish = (oi_up and price_up)  # New longs entering
    oi_confirms_bearish = (oi_up and price_down)  # New shorts entering
    
    if direction in ["BULLISH", "LEAN_BULLISH"] and oi_confirms_bullish:
        direction_score = min(40, direction_score + 5)
        direction_reason += f" | OI confirms (+{oi_change:.1f}%)"
        direction_confidence = "HIGH"
    elif direction in ["BEARISH", "LEAN_BEARISH"] and oi_confirms_bearish:
        direction_score = min(40, direction_score + 5)
        direction_reason += f" | OI confirms (+{oi_change:.1f}%)"
        direction_confidence = "HIGH"
    elif oi_up or abs(oi_change) > 2:
        # OI moving but not confirming - note it but don't penalize
        direction_reason += f" | OI: {oi_change:+.1f}%"
    
    # ═══════════════════════════════════════════════════════════════════════════
    # LAYER 2: WHALE vs RETAIL DIVERGENCE → Squeeze/Edge (0-30 points)
    # Divergence = Trapped traders = Fuel for explosive moves
    # But also consider ABSOLUTE whale positioning even without divergence!
    # ═══════════════════════════════════════════════════════════════════════════
    ...
    wt = MONEY_FLOW_PHASE_RULES['whale_thresholds']
    dt = MONEY_FLOW_PHASE_RULES['divergence_thresholds']
    
    # --- ABSOLUTE WHALE POSITIONING BONUS ---
    # Even without divergence, strong whale positioning is meaningful
    # Whales know things retail doesn't - this is the LEADING edge
    whale_conviction_bonus = 0
    if whale_pct >= wt['strong_bullish'] or whale_pct <= wt['strong_bearish']:  # 70%/30%
        whale_conviction_bonus = 10  # Strong conviction - increased from 8
    elif whale_pct >= wt['bullish'] or whale_pct <= wt['bearish']:  # 65%/35%
        whale_conviction_bonus = 7  # Good conviction - increased from 5
    elif whale_pct >= wt['lean_bullish'] or whale_pct <= wt['lean_bearish']:  # 60%/40%
        whale_conviction_bonus = 4  # Some conviction
    
    # --- DIVERGENCE SCORING ---
    if abs_divergence >= 20:
        # EXTREME divergence - strong squeeze setup
        squeeze_potential = "EXTREME"
        squeeze_score = 22 + whale_conviction_bonus
        if divergence > 0:
            squeeze_reason = f"🔥 SQUEEZE: Whales {whale_pct:.0f}% vs Retail {retail_pct:.0f}%L - shorts TRAPPED"
        else:
            squeeze_reason = f"🔥 SQUEEZE: Whales {100-whale_pct:.0f}%S vs Retail {100-retail_pct:.0f}%S - longs TRAPPED"
            
    elif abs_divergence >= dt['squeeze_setup']:  # 15%
        # STRONG divergence - squeeze potential
        squeeze_potential = "HIGH"
        squeeze_score = 18 + whale_conviction_bonus
        if divergence > 0:
            squeeze_reason = f"🔥 Squeeze setup: W:{whale_pct:.0f}% vs R:{retail_pct:.0f}% - shorts vulnerable"
        else:
            squeeze_reason = f"🔥 Squeeze setup: W:{100-whale_pct:.0f}%S vs R:{100-retail_pct:.0f}%S - longs vulnerable"
            
    elif abs_divergence >= dt['clear_edge']:  # 10%
        # MODERATE divergence - clear edge
        squeeze_potential = "MEDIUM"
        squeeze_score = 14 + whale_conviction_bonus
        squeeze_reason = f"Edge: Whales {whale_pct:.0f}% vs Retail {retail_pct:.0f}%"
        
    elif abs_divergence >= dt['slight_edge']:  # 5%
        # SLIGHT divergence - small edge
        squeeze_potential = "LOW"
        squeeze_score = 10 + whale_conviction_bonus
        squeeze_reason = f"Slight edge: W:{whale_pct:.0f}% vs R:{retail_pct:.0f}%"
            
    else:
        # NO divergence - whale conviction is the ONLY edge
        if whale_conviction_bonus >= 7:
            squeeze_potential = "WHALE_CONVICTION"
            squeeze_score = 8 + whale_conviction_bonus
            squeeze_reason = f"No divergence but STRONG whale conviction ({whale_pct:.0f}%L)"
        elif whale_conviction_bonus > 0:
            squeeze_potential = "SLIGHT_CONVICTION"
            squeeze_score = 6 + whale_conviction_bonus
            squeeze_reason = f"No divergence, slight whale lean ({whale_pct:.0f}%L)"
        else:
            squeeze_potential = "NONE"
            squeeze_score = 8  # Base score for neutral
            squeeze_reason = f"Neutral: W:{whale_pct:.0f}% ≈ R:{retail_pct:.0f}% - no edge"
    
    squeeze_score = min(30, squeeze_score)  # Cap at 30
    ...
    # --- TA Component (15 pts max) - SECONDARY to position ---
    # In a LEADING system, TA is for timing fine-tuning, NOT for gating trades
    if ta_score >= 70:
        ta_component = 15
        ta_label = "Strong"
    elif ta_score >= 55:
        ...
        ta_component = 9  # Increased floor - don't penalize early entries
        ta_label = "Moderate"
    else:
        ta_component = 6  # Raised floor - poor TA shouldn't kill leading signals
        ta_label = "Weak"
    
    # --- Move Position Component (20 pts max) - PRIMARY for timing ---
    # For PREDICTIVE trading, position is MORE important than TA!
    # Early entries with leading signals = maximum edge
    move_position = "UNKNOWN"
    move_pts = 8  # Default neutral - increased from 7
    
    # Get position thresholds from knowledge base
    pt = MONEY_FLOW_PHASE_RULES['position_thresholds']
    ...
    if current_price and swing_high and swing_low and swing_high > swing_low:
        ...
        # Use WHALE DIRECTION to determine optimal position, not structure
        # This is key - we follow LEADING indicator (whales), not LAGGING (structure)
        whale_bullish = direction in ['BULLISH', 'LEAN_BULLISH']
        whale_bearish = direction in ['BEARISH', 'LEAN_BEARISH']
        
        if whale_bullish:
            # For LONGS: want to be NEAR LOWS (early entry before move up)
            if position_pct <= pt['early_long']:  # 35%
                move_position = "EARLY"
                move_pts = 18  # Max points for optimal entry! (increased)
            elif position_pct <= 50:
                move_position = "MIDDLE"
                move_pts = 12
            elif position_pct <= pt['late_long']:  # 65%
                move_position = "LATE"
                move_pts = 5
            else:
            ...
            # For SHORTS: want to be NEAR HIGHS (early entry before move down)
            if position_pct >= pt['early_short']:  # 65%
                move_position = "EARLY"
                move_pts = 18  # Max points for optimal short entry
            elif position_pct >= 50:
                move_position = "MIDDLE"
                move_pts = 12
            elif position_pct >= pt['late_short']:  # 35%
                move_position = "LATE"
                move_pts = 5
            else:
                move_position = "CHASING"
            ...
            # NEUTRAL whales - use absolute position for reference
            if position_pct <= 30:
                move_position = "NEAR LOW"
                move_pts = 12  # Good for potential longs if signal develops
            elif position_pct >= 70:
                move_position = "NEAR HIGH"
                move_pts = 12  # Good for potential shorts if signal develops
            else:
    ...
    # --- Combined Timing Score (capped at 30) ---
    timing_score = min(30, ta_component + move_pts)
