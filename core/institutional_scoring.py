...
# ═══════════════════════════════════════════════════════════════════════════════
# COMBINED SCORING (Tech + Institutional) - NOW USES KNOWLEDGE BASE!
# ═══════════════════════════════════════════════════════════════════════════════
...
def calculate_combined_score(
    ...
    NOW USES THE UNIFIED SCORING ENGINE for consistent scoring!
    ...
    """
    # Import unified scoring engine - SINGLE SOURCE OF TRUTH
    from .unified_scoring import get_all_scores
    ...
    if hasattr(institutional_score, 'institutional_score'):
        ...
        scenario = getattr(institutional_score, 'scenario', None)
    else:
        inst_score_val = 50
        confidence = "LOW"
        alignment = "NEUTRAL"
    ...
    # Build unified_verdict dict for unified scoring engine
    # Map status to action
    status_to_action = {
        'READY': 'LONG' if direction == 'BULLISH' else 'SHORT' if direction == 'BEARISH' else 'WAIT',
        'WAIT': 'WAIT',
        'WATCH': 'WAIT',
        'CONFLICTING': 'AVOID',
        'AVOID': 'AVOID'
    }
    action = status_to_action.get(status_val, 'WAIT')
    
    # Strong bias adjustments
    if inst_score_val >= 70 and direction == 'BULLISH':
        action = 'LONG'
    elif inst_score_val >= 70 and direction == 'BEARISH':
        action = 'SHORT'
    elif inst_score_val <= 30:
        action = 'AVOID'
    
    unified_verdict = {
        'unified_action': action,  # Use unified_action key!
        'confidence': confidence,
        'scenario': {'name': scenario} if scenario else {},
        'institutional_bias': direction
    }
    
    # Use unified scoring engine - SINGLE SOURCE OF TRUTH
    result = get_all_scores(technical_score, unified_verdict)
    
    combined = result['combined_score']
    inst_score = result['inst_score']  # DERIVED from verdict!
    
    # Determine status from result
    final_action = result.get('final_action', 'WAIT')
    if final_action in ['STRONG BUY', 'BUY'] and combined >= 70:
        status = "READY"
    elif final_action in ['STRONG BUY', 'BUY', 'CAUTIOUS BUY'] and combined >= 55:
        status = "WATCH"
    elif final_action in ['WAIT', 'HOLD']:
        status = "WAIT"
    else:
        status = "AVOID"
    ...
    details = {
        ...
        'institutional_score': inst_score,  # DERIVED!
...
def check_watchlist_triggers(
    ...
    Works with dict items from watchlist.
    """
    ...
    invalidation_price = item.get('invalidation_price', 0)
    if invalidation_price > 0:
        if direction == "LONG" and current_price < invalidation_price:
            return "INVALIDATED", f"Price below invalidation ({invalidation_price:.4f})"
        elif direction == "SHORT" and current_price > invalidation_price:
            return "INVALIDATED", f"Price above invalidation ({invalidation_price:.4f})"
    
    # Check triggers
    trigger_above = item.get('trigger_above', 0)
    trigger_below = item.get('trigger_below', 0)
    
    if trigger_above > 0 and current_price > trigger_above:
        return "TRIGGERED", f"Price broke above trigger ({trigger_above:.4f})"
    
    if trigger_below > 0 and current_price < trigger_below:
        return "TRIGGERED", f"Price broke below trigger ({trigger_below:.4f})"
    
    # Check whale positioning change
    whale_at_add = item.get('whale_pct_at_add', item.get('whale_pct', 50))
    whale_change = current_whale_pct - whale_at_add
    if direction == "LONG" and whale_change < -10:
        return "INVALIDATED", f"Whales turned bearish"
    elif direction == "SHORT" and whale_change > 10:
