...
class StockHistoricalDataImporter:
    ...
    def import_historical_data(
        ...
        Args:
            ...
            fill_gaps: If True, only fetch data from last date to today (smart update)
        ...
        symbol_last_dates = {}  # For fill_gaps mode
        
        if skip_existing or fill_gaps:
            try:
                all_symbols = self.store.get_all_symbols()
                for sym in all_symbols:
                    snapshots = self.store.get_snapshots(sym)
                    if len(snapshots) >= 10:
                        already_imported.add(sym)
                        # Get last date for fill_gaps
                        if fill_gaps:
                            dates = [s.date for s in snapshots if hasattr(s, 'date')]
                            if dates:
                                last = max(dates)
                                if isinstance(last, str):
                                    last = datetime.strptime(last[:10], '%Y-%m-%d')
                                symbol_last_dates[sym] = last
            except:
                pass
            
            if already_imported:
                mode = "fill gaps" if fill_gaps else "skip"
                print(f"⏭️  Found {len(already_imported)} symbols with data (will {mode})")
        
        stats = {
            'symbols_processed': 0,
            'symbols_failed': [],
            'symbols_skipped': [],
            'symbols_updated': [],  # NEW: For fill_gaps mode
            'records_imported': 0,
        ...
        for idx, symbol in enumerate(symbols):
            ...
            if symbol in already_imported:
                if fill_gaps:
                    # Only fetch from last date to today
                    ...
                    if last_date:
                        ...
                        if gap_days <= 1:
                            ...
                            self._report_progress(symbol, progress, f"✅ {symbol}: Already up to date")
                            continue
                        days_to_fetch = gap_days + 7  # Add buffer
                        self._report_progress(symbol, progress, f"🔄 {symbol}: Filling {gap_days} day gap...")
                    else:
                        # Has data but can't determine last date, fetch default
                        days_to_fetch = lookback_days
                elif skip_existing:
                    # Skip entirely
                    stats['symbols_skipped'].append(symbol)
            ...
            self._report_progress(symbol, progress, f"Processing {symbol} ({days_to_fetch} days)...")
            
            try:
                records = self._import_symbol(symbol, days_to_fetch)
                stats['records_imported'] += records
                stats['symbols_processed'] += 1
                
                if symbol in already_imported and fill_gaps:
                    stats['symbols_updated'].append(symbol)
        ...
        if stats['records_imported'] > 0:
            try:
                ...
                if updated > 0:
                    pass  # Backtest outcomes calculated
            except Exception as e:
                pass  # Backtest error ignored
    ...
    def get_symbol_last_date(self, symbol: str) -> Optional[datetime]:
    ...
    def get_all_gaps(self, max_gap_days: int = 7) -> Dict[str, int]:
        """
        Check all symbols for data gaps.
        
        Returns:
            Dict of {symbol: days_since_last_data} for symbols with gaps > max_gap_days
        """
        gaps = {}
        today = datetime.now()
        ...
        try:
            ...
            for symbol in all_symbols:
                last_date = self.get_symbol_last_date(symbol)
                if last_date:
                    # Handle string dates
