...
class CoinglassHistoricalFetcher:
    ...
    def fetch_oi_history(self, symbol: str, interval: str = "4h", limit: int = 500, start_time: int = None, end_time: int = None) -> List[Dict]:
        """
        Fetch Open Interest history
        Params: exchange=Binance, symbol=BTCUSDT (full pair!), interval, limit
        start_time/end_time: Unix timestamp in milliseconds (optional)
        """
        ...
        result = self._request("/api/futures/open-interest/history", params)
    ...
    def fetch_funding_history(self, symbol: str, interval: str = "4h", limit: int = 500, start_time: int = None, end_time: int = None) -> List[Dict]:
        """
        ...
        result = self._request("/api/futures/funding-rate/history", params)
    ...
    def fetch_long_short_ratio_history(self, symbol: str, interval: str = "4h", limit: int = 500, start_time: int = None, end_time: int = None) -> List[Dict]:
        """
        ...
        result = self._request("/api/futures/top-long-short-account-ratio/history", params)
    ...
    def fetch_global_long_short_history(self, symbol: str, interval: str = "4h", limit: int = 500, start_time: int = None, end_time: int = None) -> List[Dict]:
        """
        ...
        result = self._request("/api/futures/global-long-short-account-ratio/history", params)
    ...
    def fetch_price_history(self, symbol: str, interval: str = "4h", limit: int = 500, start_time: int = None, end_time: int = None) -> List[Dict]:
        ...
        try:
            ...
            # Binance uses startTime/endTime
            if start_time:
                params["startTime"] = start_time
            if end_time:
                params["endTime"] = end_time
...
class HistoricalDataImporter:
    ...
    def get_data_coverage(self) -> Dict:
        ...
        try:
            ...
            if len(all_dates) >= 2:
                ...
                for d in all_dates:
                    ...
                    if prev_date:
                        ...
                        if days_diff > 1:
                            # Calculate actual missing days (exclude boundaries that have data)
                            missing_days = days_diff - 1
                            # First missing day is day after prev_date
                            first_missing = prev_date + timedelta(days=1)
                            # Last missing day is day before curr_date
                            last_missing = curr_date - timedelta(days=1)
                            
                            gaps.append({
                                'start': first_missing.strftime('%Y-%m-%d'),
                                'end': last_missing.strftime('%Y-%m-%d'),
                                'days': missing_days,
                                'description': f"Missing {missing_days} days: {first_missing.strftime('%Y-%m-%d')} to {last_missing.strftime('%Y-%m-%d')}"
                            })
                    prev_date = curr_date
            ...
            if end_str:
                ...
                # Only report gap if there are actually missing days (more than 1 day difference)
                if days_gap > 1:
                    # Actual missing days (exclude the last day that has data)
                    missing_days = days_gap - 1
                    # First missing day is day after last data
                    first_missing = end_date + timedelta(days=1)
                    # Last missing day is yesterday (today might still be collecting)
    ...
    def import_historical_data(
        ...
        if from_date and to_date:
            ...
            print(f"   Time range: {start_time_ms} to {end_time_ms}")
        ...
        for idx, symbol in enumerate(symbols):
            ...
            try:
                records = self._import_symbol(symbol, interval, limit, start_time_ms, end_time_ms)
    ...
    def _import_symbol(self, symbol: str, interval: str, limit: int, start_time: int = None, end_time: int = None) -> int:
        """Import data for one symbol
        start_time/end_time: Unix timestamp in milliseconds (optional, for date range mode)
        """
        ...
        print("    - Price (Binance)")
        price_data = self.fetcher.fetch_price_history(symbol, interval, limit, start_time, end_time)
        
        print("    - Open Interest")
        oi_data = self.fetcher.fetch_oi_history(symbol, interval, limit, start_time, end_time)
        
        print("    - Funding Rate")
        funding_data = self.fetcher.fetch_funding_history(symbol, interval, limit, start_time, end_time)
        
        print("    - Top L/S (whales)")
        ls_data = self.fetcher.fetch_long_short_ratio_history(symbol, interval, limit, start_time, end_time)
        
        print("    - Global L/S (retail)")
        global_ls = self.fetcher.fetch_global_long_short_history(symbol, interval, limit, start_time, end_time)
