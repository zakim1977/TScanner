...
def calculate_trade_levels(
    ...
    Returns:
        TradeLevels with SL/TP based on structure, enhanced by ML
    """
    ...
    # If ML prediction available, use it to enhance R:R requirements
    if ml_prediction:
        optimal_tp_pct = ml_prediction.get('optimal_tp1_pct', 0)
        optimal_sl_pct = ml_prediction.get('optimal_sl_pct', 0)
        
        if optimal_tp_pct > 0 and optimal_sl_pct > 0:
            # Calculate ML's expected R:R
            ml_rr = optimal_tp_pct / optimal_sl_pct
            
            # Update min R:R requirements if ML suggests higher
            # TP1 should reach at least ML's optimal, TP2/TP3 scale up
            if ml_rr > calculator.config.get('min_rr_tp1', 1.0):
                calculator.config['min_rr_tp1'] = max(calculator.config['min_rr_tp1'], ml_rr * 0.8)
                calculator.config['min_rr_tp2'] = max(calculator.config['min_rr_tp2'], ml_rr * 1.2)
                calculator.config['min_rr_tp3'] = max(calculator.config['min_rr_tp3'], ml_rr * 1.6)
            
            # Store ML info for reference
            calculator.ml_optimal_tp_pct = optimal_tp_pct
            calculator.ml_optimal_sl_pct = optimal_sl_pct
