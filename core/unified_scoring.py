...
# HONEST ML LABELING
# ═══════════════════════════════════════════════════════════════════════════════
# This ensures we always use Probabilistic ML (no heuristic fallback)

def _get_ml_label_info() -> Dict:
    """
    Get honest label information for ML predictions.
    
    Returns dict with:
        - is_real_ml: True if actual trained ML
        - confirms_label: Text like "✅ ML confirms" 
        - says_label: Text like "🤖 ML says"
        - source: "Trained ML" or "Basic ML"
    """
    try:
        from ml import is_real_trained_ml, get_prediction_source
        is_real = is_real_trained_ml()
    ...
    if is_real:
        return {
            'is_real_ml': True,
            'confirms_label': "✅ Trained ML confirms",
            'says_label': "🤖 Trained ML says",
            'disagrees_label': "⚠️ Trained ML says",
            'source': source,
            'icon': "🤖"
        }
    else:
        return {
            'is_real_ml': False,
            'confirms_label': "✅ ML agrees",
            'says_label': "🤖 ML says",
            'disagrees_label': "🤖 ML says",
            'source': "ML",
            'icon': "🤖"
        }

# Cache the labels (only computed once)
_ML_LABELS = None

def get_ml_labels() -> Dict:
    """Get cached ML label info."""
    global _ML_LABELS
    if _ML_LABELS is None:
        _ML_LABELS = _get_ml_label_info()
    return _ML_LABELS
...
def generate_combined_learning(
    ...
    # NEW: Explosion Readiness Data
    explosion_data: dict = None,         # From explosion_detector
    # NEW: Holistic data from blend_hybrid_prediction (SINGLE SOURCE OF TRUTH!)
    holistic_data: dict = None,          # Contains divergence_story, flow_story, etc.
    # Symbol for meme coin detection
    symbol: str = "",                    # Symbol name for meme coin detection
    # NEW: Probabilistic ML data - FULL reasoning chain!
    prob_ml_data: dict = None,           # Contains probabilities, direction, action_message, f1_scores
) -> dict:
    """
    ═══════════════════════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # SINGLE SOURCE OF TRUTH: Use holistic_data from blend_hybrid_prediction
    # This ensures Combined Learning shows EXACTLY what the header shows!
    # ═══════════════════════════════════════════════════════════════════════════
    
    # Extract holistic story data if provided (PREFERRED - single source of truth!)
    if holistic_data:
        divergence_story = holistic_data.get('divergence_story', 'NEUTRAL')
        divergence_desc = holistic_data.get('divergence_desc', '')
        flow_story = holistic_data.get('flow_story', 'NEUTRAL')
        flow_desc = holistic_data.get('flow_desc', '')
        position_zone = holistic_data.get('position_zone', 'MIDDLE')
        position_desc = holistic_data.get('position_desc', '')
        setup_type = holistic_data.get('setup_type', '')
        holistic_verdict = holistic_data.get('holistic_verdict', '')
        engines_agree = holistic_data.get('engines_agree', False)
    else:
        divergence_story = None
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # STEP 1: Get decision from MASTER_RULES (fallback if no holistic_data)
    # ═══════════════════════════════════════════════════════════════════════════
    ...
    if MASTER_RULES_AVAILABLE and whale_pct is not None and retail_pct is not None:
        try:
            decision = get_trade_decision(
                ...
                symbol=symbol,  # For meme coin detection
                # SMC Level Proximity
            ...
            # ═══════════════════════════════════════════════════════════════════
            # PRE-PROCESS: Extract explosion state for unified conclusion
            # ═══════════════════════════════════════════════════════════════════
            exp_state = explosion_data.get('state', 'UNKNOWN') if explosion_data else 'UNKNOWN'
            exp_score = explosion_data.get('score', 0) if explosion_data else 0
            exp_ready = explosion_data.get('ready', False) if explosion_data else False
            exp_direction = explosion_data.get('direction') if explosion_data else None
            squeeze_pct = explosion_data.get('squeeze_pct', 50) if explosion_data else 50
            entry_valid = explosion_data.get('entry_valid', False) if explosion_data else False
            
            # Use holistic flow_story if available (from blend_hybrid_prediction)
            if flow_story:
                # Override MASTER_RULES OI interpretation with holistic version
                if flow_story == "WHALE_ACCUMULATION":
                    decision.oi_interpretation = "Whale accumulation (buying dip)"
                elif flow_story == "NEW_LONGS":
                    decision.oi_interpretation = "New longs entering"
                elif flow_story == "NEW_SHORTS":
                    decision.oi_interpretation = "New shorts entering"
                elif flow_story == "SHORT_COVERING":
                    decision.oi_interpretation = "Short covering (fake rally)"
                elif flow_story == "WHALE_EXIT_WARNING":
                    decision.oi_interpretation = "⚠️ Whale exit warning!"
            ...
            if final_verdict and final_verdict in ['LONG', 'SHORT', 'WAIT']:
                ...
                # ═══════════════════════════════════════════════════════════════
                # BUILD UNIFIED CONCLUSION - Synthesize ALL metrics together
                # ═══════════════════════════════════════════════════════════════
                
                # Determine timing quality
                timing_quality = decision.position_label  # EARLY, MIDDLE, LATE
                timing_pct = position_pct
                
                # Determine whale conviction
                whale_conviction = 'STRONG' if whale_pct >= 65 or whale_pct <= 35 else 'MODERATE' if whale_pct >= 55 or whale_pct <= 45 else 'WEAK'
                
                # Determine OI signal
                oi_signal = 'BUILDING' if oi_change > 3 else 'STABLE' if oi_change > -1 else 'DECLINING'
                
                # Build conclusion based on final verdict + ALL metrics
                if final_verdict == 'WAIT':
                    # WAIT scenarios - explain WHY waiting
                    wait_reasons = []
                    
                    if timing_quality == 'LATE':
                        wait_reasons.append(f"late entry ({timing_pct:.0f}% into move)")
                    
                    if exp_state == 'COMPRESSION':
                        wait_reasons.append(f"squeeze loading ({squeeze_pct:.0f}% BB)")
                    elif exp_state == 'DISTRIBUTION':
                        wait_reasons.append("distribution phase")
                    
                    if whale_conviction == 'WEAK':
                        wait_reasons.append(f"weak whale conviction ({whale_pct:.0f}%)")
                    
                    if ml_prediction and ml_prediction != 'WAIT' and engine_mode == 'hybrid':
                        ml_labels = get_ml_labels()
                        if direction in ['BULLISH', 'LEAN_BULLISH'] and ml_prediction == 'SHORT':
                            wait_reasons.append(f"{ml_labels['source']}/Rules conflict")
                        elif direction in ['BEARISH', 'LEAN_BEARISH'] and ml_prediction == 'LONG':
                            wait_reasons.append(f"{ml_labels['source']}/Rules conflict")
                    
                    if wait_reasons:
                        conclusion = f"⏳ WAIT - {', '.join(wait_reasons)}"
                    elif final_verdict_reason:
                        conclusion = f"⏳ WAIT - {final_verdict_reason}"
                    else:
                        conclusion = f"⏳ WAIT - No clear edge"
                    
                    conclusion_action = "WAIT for better setup"
                    
                    # Only add as CONFLICT if ML/Heuristic says something different
                    if engine_mode == 'hybrid' and ml_prediction:
                        ml_labels = get_ml_labels()
                        if ml_prediction == 'LONG':
                            conflicts.append(f"{ml_labels['icon']} {ml_labels['source']} predicts LONG but entry timing poor")
                        elif ml_prediction == 'SHORT' and direction in ['BULLISH', 'LEAN_BULLISH']:
                            conflicts.append(f"{ml_labels['icon']} {ml_labels['source']} predicts SHORT vs bullish positioning")
                            
                elif final_verdict == 'LONG':
                    # LONG scenarios - build conviction case
                    long_factors = []
                    
                    if whale_conviction == 'STRONG':
                        long_factors.append(f"whales {whale_pct:.0f}% bullish")
                    elif whale_conviction == 'MODERATE':
                        long_factors.append(f"whales lean bullish ({whale_pct:.0f}%)")
                    
                    if timing_quality == 'EARLY':
                        long_factors.append(f"early entry ({timing_pct:.0f}%)")
                    
                    if oi_signal == 'BUILDING':
                        long_factors.append(f"OI building (+{oi_change:.1f}%)")
                    
                    # Add explosion context
                    if exp_state == 'IGNITION':
                        long_factors.append("🚀 squeeze igniting")
                    elif exp_state == 'LIQUIDITY_CLEAR':
                        long_factors.append("🎯 liquidity swept")
                    elif exp_ready:
                        long_factors.append(f"explosion ready ({exp_score}/100)")
                    
                    if ml_prediction == 'LONG' and engine_mode == 'hybrid':
                        ml_labels = get_ml_labels()
                        long_factors.append(f"{ml_labels['source']} confirms")
                    
                    if long_factors:
                        conclusion = f"🟢 LONG - {', '.join(long_factors)}"
                    else:
                        conclusion = f"🟢 LONG SETUP - Whales {whale_pct:.0f}% bullish"
                    ...
                    if engine_mode == 'hybrid' and ml_prediction:
                        ...
                        if ml_prediction == 'SHORT':
                            conflicts.append(f"⚠️ {ml_labels['source']} predicts SHORT - use caution")
                        elif ml_prediction == 'WAIT':
                            conflicts.append(f"{ml_labels['icon']} {ml_labels['source']} predicts WAIT - consider smaller size")
                    
                    if exp_state == 'DISTRIBUTION':
                        conflicts.append(f"⚠️ Distribution phase detected - whales may be exiting")
                    
                elif final_verdict == 'SHORT':
                    # SHORT scenarios - build conviction case
                    short_factors = []
                    
                    if whale_conviction == 'STRONG':
                        short_factors.append(f"whales {whale_pct:.0f}% bearish")
                    elif whale_conviction == 'MODERATE':
                        short_factors.append(f"whales lean bearish ({whale_pct:.0f}%)")
                    
                    if timing_quality == 'EARLY':
                        short_factors.append(f"early entry ({timing_pct:.0f}%)")
                    
                    if oi_signal == 'BUILDING':
                        short_factors.append(f"OI building (+{oi_change:.1f}%)")
                    
                    # Add explosion context
                    if exp_state == 'IGNITION':
                        short_factors.append("🚀 squeeze igniting")
                    elif exp_state == 'LIQUIDITY_CLEAR':
                        short_factors.append("🎯 liquidity swept")
                    elif exp_ready:
                        short_factors.append(f"explosion ready ({exp_score}/100)")
                    
                    if ml_prediction == 'SHORT' and engine_mode == 'hybrid':
                        ml_labels = get_ml_labels()
                        short_factors.append(f"{ml_labels['source']} confirms")
                    
                    if short_factors:
                        conclusion = f"🔴 SHORT - {', '.join(short_factors)}"
                    else:
                        conclusion = f"🔴 SHORT SETUP - Whales {whale_pct:.0f}% bearish"
                    ...
                    if engine_mode == 'hybrid' and ml_prediction:
                        ml_labels = get_ml_labels()
                        if ml_prediction == 'LONG':
                            conflicts.append(f"⚠️ {ml_labels['source']} predicts LONG - use caution")
                        elif ml_prediction == 'WAIT':
                            conflicts.append(f"{ml_labels['icon']} {ml_labels['source']} predicts WAIT - consider smaller size")
            else:
            ...
            # ═══════════════════════════════════════════════════════════════════
            # 🧠 PROBABILISTIC ML - FIRST (Most Important!)
            # Shows FULL logic: raw probabilities → interpretation → verdict
            # ═══════════════════════════════════════════════════════════════════
            if prob_ml_data:
                prob_dir = prob_ml_data.get('direction', 'WAIT')
                prob_conf = prob_ml_data.get('confidence', 0)
                prob_action = prob_ml_data.get('action_message', '')
                prob_mode = prob_ml_data.get('mode', 'daytrade')
                f1_scores = prob_ml_data.get('f1_scores') or {}  # Ensure dict, not None
                
                # Safe F1 score extraction helper
                def get_f1(label):
                    if not f1_scores or not isinstance(f1_scores, dict):
                        return 0
                    val = f1_scores.get(label, 0)
                    return val * 100 if isinstance(val, (int, float)) else 0
                
                # Build clear reasoning chain
                prob_story_parts = []
                
                # 1. VERDICT with confidence
                dir_emoji = "🟢" if prob_dir == 'LONG' else "🔴" if prob_dir == 'SHORT' else "⏳"
                prob_story_parts.append(f"{dir_emoji} **{prob_dir}** ({prob_conf:.0f}% confidence)")
                
                # 2. Action message (e.g., "BULL TRAP - Will reverse DOWN")
                if prob_action:
                    prob_story_parts.append(f"🎯 {prob_action}")
                
                # 3. RAW PROBABILITIES with clear explanations
                if prob_mode in ['scalp', 'daytrade']:
                    cont_bull = prob_ml_data.get('continuation_bull', 0) * 100
                    cont_bear = prob_ml_data.get('continuation_bear', 0) * 100
                    fake_bull = prob_ml_data.get('fakeout_to_bull', 0) * 100
                    fake_bear = prob_ml_data.get('fakeout_to_bear', 0) * 100
                    vol_exp = prob_ml_data.get('vol_expansion', 0) * 100
                    
                    # Explain what each probability means
                    lookahead = "2.5hrs" if prob_mode == 'daytrade' else "30min"
                    
                    prob_story_parts.append(f"\n📊 **What ML Predicts** (next {lookahead}):")
                    prob_story_parts.append(f"   • Price continues UP: **{cont_bull:.0f}%** (F1: {get_f1('continuation_bull'):.0f}% accuracy)")
                    prob_story_parts.append(f"   • Price continues DOWN: **{cont_bear:.0f}%** (F1: {get_f1('continuation_bear'):.0f}% accuracy)")
                    prob_story_parts.append(f"   • Bear trap → reverses UP: **{fake_bull:.0f}%** (F1: {get_f1('fakeout_to_bull'):.0f}% accuracy)")
                    prob_story_parts.append(f"   • Bull trap → reverses DOWN: **{fake_bear:.0f}%** (F1: {get_f1('fakeout_to_bear'):.0f}% accuracy)")
                    prob_story_parts.append(f"   • Volatility expansion: **{vol_exp:.0f}%**")
                    
                    # 4. REASONING - How we arrived at the verdict
                    bull_strength = cont_bull + fake_bull
                    bear_strength = cont_bear + fake_bear
                    
                    # Identify dominant pattern
                    bull_is_trap = fake_bull > cont_bull
                    bear_is_trap = fake_bear > cont_bear
                    
                    prob_story_parts.append(f"\n🎯 **How We Decided:**")
                    prob_story_parts.append(f"   • Bullish signals: {cont_bull:.0f}% + {fake_bull:.0f}% = **{bull_strength:.0f}%**")
                    prob_story_parts.append(f"   • Bearish signals: {cont_bear:.0f}% + {fake_bear:.0f}% = **{bear_strength:.0f}%**")
                    
                    # Show the comparison logic
                    if prob_dir == 'LONG':
                        prob_story_parts.append(f"   → **{bull_strength:.0f}% > {bear_strength:.0f}%** = BULLISH wins")
                        if bull_is_trap and fake_bull > 10:
                            prob_story_parts.append(f"   → Pattern: Bear trap ({fake_bull:.0f}%) = Price faked down, will reverse UP → **LONG**")
                        else:
                            prob_story_parts.append(f"   → Pattern: Bullish continuation ({cont_bull:.0f}%) → **LONG**")
                    elif prob_dir == 'SHORT':
                        prob_story_parts.append(f"   → **{bear_strength:.0f}% > {bull_strength:.0f}%** = BEARISH wins")
                        if bear_is_trap and fake_bear > 10:
                            prob_story_parts.append(f"   → Pattern: Bull trap ({fake_bear:.0f}%) = Price faked up, will reverse DOWN → **SHORT**")
                        else:
                            prob_story_parts.append(f"   → Pattern: Bearish continuation ({cont_bear:.0f}%) → **SHORT**")
                    else:
                        # Calculate relative strength (consistent with decision logic using 1.2x threshold)
                        if bear_strength > bull_strength:
                            relative_stronger = (bear_strength / max(bull_strength, 0.01) - 1) * 100
                            prob_story_parts.append(f"   → Bearish is {relative_stronger:.0f}% stronger (need 20% to decide) → **WAIT**")
                        else:
                            relative_stronger = (bull_strength / max(bear_strength, 0.01) - 1) * 100
                            prob_story_parts.append(f"   → Bullish is {relative_stronger:.0f}% stronger (need 20% to decide) → **WAIT**")
                    
                elif prob_mode == 'swing':
                    trend_bull = prob_ml_data.get('trend_holds_bull', 0) * 100
                    trend_bear = prob_ml_data.get('trend_holds_bear', 0) * 100
                    rev_bull = prob_ml_data.get('reversal_to_bull', 0) * 100
                    rev_bear = prob_ml_data.get('reversal_to_bear', 0) * 100
                    drawdown = prob_ml_data.get('drawdown', 0) * 100
                    
                    prob_story_parts.append(f"\n📊 **What ML Predicts** (next 40hrs):")
                    prob_story_parts.append(f"   • Bullish trend continues 2%+: **{trend_bull:.0f}%**")
                    prob_story_parts.append(f"   • Bearish trend continues 2%+: **{trend_bear:.0f}%**")
                    prob_story_parts.append(f"   • Reversal UP (bottom forming): **{rev_bull:.0f}%**")
                    prob_story_parts.append(f"   • Reversal DOWN (top forming): **{rev_bear:.0f}%**")
                    prob_story_parts.append(f"   • 3%+ drawdown risk: **{drawdown:.0f}%**")
                    
                    bull_total = trend_bull + rev_bull * 1.5  # Reversals weighted more
                    bear_total = trend_bear + rev_bear * 1.5
                    
                    prob_story_parts.append(f"\n🎯 **How We Decided:**")
                    if prob_dir == 'LONG':
                        if rev_bull > trend_bull:
                            prob_story_parts.append(f"   → Strong reversal signal ({rev_bull:.0f}%) = **Bottom forming** → LONG")
                        else:
                            prob_story_parts.append(f"   → Bullish trend ({trend_bull:.0f}%) dominating → **LONG**")
                    elif prob_dir == 'SHORT':
                        if rev_bear > trend_bear:
                            prob_story_parts.append(f"   → Strong reversal signal ({rev_bear:.0f}%) = **Top forming** → SHORT")
                        else:
                            prob_story_parts.append(f"   → Bearish trend ({trend_bear:.0f}%) dominating → **SHORT**")
                    
                elif prob_mode == 'investment':
                    accum = prob_ml_data.get('accumulation', 0) * 100
                    distrib = prob_ml_data.get('distribution', 0) * 100
                    rev_bull = prob_ml_data.get('reversal_to_bull', 0) * 100
                    rev_bear = prob_ml_data.get('reversal_to_bear', 0) * 100
                    lg_drawdown = prob_ml_data.get('large_drawdown', 0) * 100
                    
                    prob_story_parts.append(f"\n📊 **What ML Predicts** (next 5 days):")
                    prob_story_parts.append(f"   • Smart money accumulating: **{accum:.0f}%** (bullish)")
                    prob_story_parts.append(f"   • Smart money distributing: **{distrib:.0f}%** (bearish)")
                    prob_story_parts.append(f"   • Major reversal UP: **{rev_bull:.0f}%**")
                    prob_story_parts.append(f"   • Major reversal DOWN: **{rev_bear:.0f}%**")
                    prob_story_parts.append(f"   • 7%+ drawdown risk: **{lg_drawdown:.0f}%**")
                    
                    prob_story_parts.append(f"\n🎯 **How We Decided:**")
                    if prob_dir == 'LONG':
                        prob_story_parts.append(f"   → Accumulation ({accum:.0f}%) + Rev UP ({rev_bull:.0f}%) = **Smart money buying** → LONG")
                    elif prob_dir == 'SHORT':
                        prob_story_parts.append(f"   → Distribution ({distrib:.0f}%) + Rev DN ({rev_bear:.0f}%) = **Smart money selling** → SHORT")
                
                # Combine into story
                prob_story = "\n".join(prob_story_parts)
                stories.append(('🧠 Probabilistic ML', prob_story))
            
            # ═══════════════════════════════════════════════════════════════════
            # Supporting Data (after ML verdict)
            # ═══════════════════════════════════════════════════════════════════
            stories.append(('🐋 Whale Analysis', decision.whale_story))
            
            # Use holistic flow_desc for OI story if available (SINGLE SOURCE OF TRUTH!)
            if flow_desc:
                stories.append(('📈 Open Interest', flow_desc))
            else:
                stories.append(('📈 Open Interest', decision.oi_story))
                
            stories.append(('📍 Entry Timing', decision.position_story))
            
            # Add Traditional ML story if in hybrid mode (for comparison)
            if engine_mode == 'hybrid' and ml_prediction:
                ml_labels = get_ml_labels()
                ml_emoji = "🟢" if ml_prediction == 'LONG' else "🔴" if ml_prediction == 'SHORT' else "⏳"
                stories.append((f'{ml_labels["icon"]} Traditional ML', f"{ml_emoji} says: {ml_prediction}"))
            
            # ═══════════════════════════════════════════════════════════════════
            # NEW: Add Explosion Readiness Analysis
            # ═══════════════════════════════════════════════════════════════════
            explosion_story = ""
            explosion_boost = 0
            explosion_warning = None
            
            if explosion_data:
                exp_score = explosion_data.get('score', 0)
                exp_state = explosion_data.get('state', 'UNKNOWN')
                exp_ready = explosion_data.get('ready', False)
                exp_direction = explosion_data.get('direction')
                squeeze_pct = explosion_data.get('squeeze_pct', 50)
                entry_valid = explosion_data.get('entry_valid', False)
                exp_signals = explosion_data.get('signals', [])
                
                # Build explosion story
                if exp_state == 'IGNITION':
                    explosion_story = f"🚀 IGNITION - Squeeze releasing! Prime entry window"
                    explosion_boost = 15
                    if exp_direction and final_verdict and exp_direction != final_verdict:
                        explosion_warning = f"⚠️ Explosion direction ({exp_direction}) differs from signal"
                elif exp_state == 'LIQUIDITY_CLEAR':
                    explosion_story = f"🎯 LIQUIDITY CLEARED - Stops swept, reversal zone active"
                    explosion_boost = 12
                elif exp_state == 'COMPRESSION':
                    explosion_story = f"⚡ COMPRESSION - BB squeeze {squeeze_pct:.0f}% percentile. Pressure building, wait for ignition"
                    if entry_valid:
                        explosion_boost = 5
                    else:
                        explosion_warning = "⏳ Compression detected but not ready - WAIT for breakout"
                elif exp_state == 'EXPANSION':
                    if entry_valid:
                        explosion_story = f"📈 EXPANSION - Move started, still early enough to enter"
                        explosion_boost = 8
                    else:
                        explosion_story = f"📈 EXPANSION - Move already extended, late entry risk"
                        explosion_warning = "⚠️ Move already in progress - may be late"
                elif exp_state == 'DISTRIBUTION':
                    explosion_story = f"🔴 DISTRIBUTION - Whales exiting. Avoid new longs"
                    explosion_warning = "❌ Distribution phase - AVOID"
                elif exp_state == 'ACCUMULATION':
                    explosion_story = f"📦 ACCUMULATION - Whales building quietly. Wait for ignition"
                
                # Add explosion story
                if explosion_story:
                    stories.append(('🚀 Explosion Readiness', explosion_story))
                
                # Add specific signals detected
                if exp_signals and len(exp_signals) > 0:
                    signals_text = " | ".join(exp_signals[:3])  # Max 3 signals
                    stories.append(('⚡ Catalysts', signals_text))
                
                # Add warnings
                if explosion_warning:
                    conflicts.append(explosion_warning)
            ...
            # Build explosion narrative section
            explosion_narrative = ""
            if explosion_data and explosion_data.get('state') not in ['UNKNOWN', 'RANGING', None]:
                exp_state = explosion_data.get('state', 'UNKNOWN')
                exp_score = explosion_data.get('score', 0)
                explosion_narrative = f"""
**🚀 Explosion Readiness** (Score: {exp_score}/100)
State: {exp_state}
{explosion_story if explosion_story else 'No major catalyst detected'}
"""
            
            # ═══════════════════════════════════════════════════════════════════
            # BUILD UNIFIED SYNTHESIS - One coherent story
            # ═══════════════════════════════════════════════════════════════════
            
            # Create unified story that combines all metrics
            unified_parts = []
            
            # 1. Whale positioning (always show)
            if whale_pct >= 65:
                unified_parts.append(f"🐋 Whales strongly bullish ({whale_pct:.0f}%)")
            elif whale_pct >= 55:
                unified_parts.append(f"📈 Whales lean bullish ({whale_pct:.0f}%)")
            elif whale_pct <= 35:
                unified_parts.append(f"🐻 Whales strongly bearish ({whale_pct:.0f}%)")
            elif whale_pct <= 45:
                unified_parts.append(f"📉 Whales lean bearish ({whale_pct:.0f}%)")
            else:
                unified_parts.append(f"↔️ Whales neutral ({whale_pct:.0f}%)")
            
            # 2. Retail comparison (always show squeeze potential)
            whale_retail_diff = whale_pct - retail_pct
            if whale_retail_diff > 15:
                unified_parts.append(f"Retail lagging at {retail_pct:.0f}% = HIGH squeeze potential")
            elif whale_retail_diff > 10:
                unified_parts.append(f"Retail at {retail_pct:.0f}% = squeeze building")
            elif whale_retail_diff < -10:
                unified_parts.append(f"⚠️ Retail ahead at {retail_pct:.0f}% = TRAP risk")
            elif whale_retail_diff < -5:
                unified_parts.append(f"Retail slightly ahead ({retail_pct:.0f}%)")
            else:
                unified_parts.append(f"Whale/Retail balanced ({retail_pct:.0f}%)")
            
            # 3. OI context (always show)
            if oi_change > 5:
                unified_parts.append(f"OI surging (+{oi_change:.1f}%) = strong conviction")
            elif oi_change > 2:
                unified_parts.append(f"OI building (+{oi_change:.1f}%)")
            elif oi_change < -5:
                unified_parts.append(f"OI dropping ({oi_change:.1f}%) = positions closing")
            elif oi_change < -2:
                unified_parts.append(f"OI declining ({oi_change:.1f}%)")
            else:
                unified_parts.append(f"OI stable ({oi_change:+.1f}%)")
            
            # 4. Entry timing (always show)
            if position_pct <= 25:
                unified_parts.append(f"✅ EARLY entry ({position_pct:.0f}%)")
            elif position_pct <= 45:
                unified_parts.append(f"👍 Good entry ({position_pct:.0f}%)")
            elif position_pct <= 60:
                unified_parts.append(f"👀 Mid-move ({position_pct:.0f}%)")
            elif position_pct <= 75:
                unified_parts.append(f"⚠️ Extended ({position_pct:.0f}%)")
            else:
                unified_parts.append(f"❌ LATE entry ({position_pct:.0f}%) - wait for pullback")
            
            # 5. Compression with DIRECTION (always show)
            # Determine expected direction based on whale positioning
            if whale_pct >= 55:
                exp_direction = "LONG 📈"
            elif whale_pct <= 45:
                exp_direction = "SHORT 📉"
            else:
                exp_direction = "unclear"
            
            # Now squeeze_pct uses HIGH = compressed convention
            if squeeze_pct >= 80:
                unified_parts.append(f"⚡ BB SQUEEZED ({squeeze_pct:.0f}%) → Explosion imminent!")
            elif squeeze_pct >= 60:
                unified_parts.append(f"⚡ BB TIGHT ({squeeze_pct:.0f}%) → Compression building")
            elif squeeze_pct <= 30:
                unified_parts.append(f"📊 BB relaxed ({squeeze_pct:.0f}%) - no squeeze")
            
            # 6. Explosion state (if significant)
            if exp_state == 'IGNITION':
                unified_parts.append(f"🚀 IGNITION {exp_direction}!")
            elif exp_state == 'LIQUIDITY_CLEAR':
                unified_parts.append("🎯 Liquidity swept - reversal zone")
            elif exp_state == 'DISTRIBUTION':
                unified_parts.append("🔴 Distribution - whales exiting")
            
            # 7. Money flow (if significant)
            if money_flow_phase in ['ACCUMULATION']:
                unified_parts.append("💰 Accumulation phase")
            elif money_flow_phase in ['MARKUP']:
                unified_parts.append("📈 Markup phase")
            elif money_flow_phase in ['DISTRIBUTION', 'PROFIT TAKING']:
                unified_parts.append(f"⚠️ {money_flow_phase}")
            
            # 8. ML alignment (if hybrid) - USE HONEST LABELS!
            ml_labels = get_ml_labels()
            if engine_mode == 'hybrid' and ml_prediction:
                if ml_prediction == final_verdict:
                    unified_parts.append(ml_labels['confirms_label'])
                elif ml_prediction == 'WAIT':
                    unified_parts.append(f"{ml_labels['icon']} {ml_labels['source']} says WAIT")
                elif ml_prediction != final_verdict and final_verdict in ['LONG', 'SHORT']:
                    unified_parts.append(f"{ml_labels['disagrees_label']} {ml_prediction}")
            
            # Build unified story - use pipe separator for readability
            unified_story = " | ".join(unified_parts)
            
            # Build full narrative with unified synthesis
            full_narrative = f"""
**🎯 FINAL VERDICT**
{conclusion}
...
"""
            ...
            return {
                ...
                'unified_story': unified_story,  # NEW: Single coherent synthesis
                ...
                # Explosion data
                'explosion_state': exp_state,
                'explosion_score': exp_score,
                'explosion_ready': exp_ready,
                'explosion_boost': explosion_boost,
            }
    ...
    return {
        ...
        # Explosion data (fallback)
        'explosion_state': explosion_data.get('state') if explosion_data else None,
        'explosion_score': explosion_data.get('score', 0) if explosion_data else 0,
        'explosion_ready': explosion_data.get('ready', False) if explosion_data else False,
        'explosion_boost': 0,
    }
...
def calculate_predictive_score(
    ...
    if MASTER_RULES_AVAILABLE:
        ...
        # Entry timing based on TA + Position ONLY (Layer 3 is independent!)
        # This evaluates entry quality, not overall trade validity
        # trade_direction affects the FINAL verdict, not the entry timing assessment
        if decision.position_label == 'EARLY' and direction in ['BULLISH', 'LEAN_BULLISH']:
            entry_timing = 'NOW'  # EARLY is great for longs
        elif decision.position_label == 'LATE' and direction in ['BEARISH', 'LEAN_BEARISH']:
            entry_timing = 'NOW'  # LATE is great for shorts
        elif decision.position_label == 'MIDDLE':
            entry_timing = 'SOON'  # MIDDLE is okay
        elif ta_score >= 60:
            entry_timing = 'SOON'  # Good TA
        elif decision.position_label == 'EARLY':
            entry_timing = 'SOON'  # EARLY even with weak TA
        elif decision.position_label == 'LATE' and direction in ['BULLISH', 'LEAN_BULLISH']:
            entry_timing = 'WAIT'  # LATE for longs = chasing
        else:
            entry_timing = 'WAIT'
        ...
        # Timing reason - ONLY about TA + Position (Layer 3 content)
        if entry_timing == 'NOW':
            timing_reason = f"✅ {move_position} ({position_pct:.0f}%) + TA {ta_score}"
        elif entry_timing == 'SOON':
            timing_reason = f"📍 {move_position} ({position_pct:.0f}%) | TA {ta_score}"
        else:
            timing_reason = f"⏳ {move_position} ({position_pct:.0f}%) | TA {ta_score}"
...
# ═══════════════════════════════════════════════════════════════════════════════
# ML-PRIMARY UNIFIED PREDICTION
# ═══════════════════════════════════════════════════════════════════════════════
# 
# NEW ARCHITECTURE:
# 1. ML is PRIMARY - ML direction is THE final direction
# 2. Rules provide CONTEXT - explain why, stories, warnings
# 3. Pattern Detection - OPTIONAL confirmation
#
# This replaces the old approach where rules determined direction.
# ═══════════════════════════════════════════════════════════════════════════════

def get_ml_unified_prediction(
    # ML Prediction (PRIMARY)
    ml_direction: str,        # 'LONG', 'SHORT', 'WAIT' - THIS IS THE FINAL DIRECTION
    ml_confidence: float,     # 0-100
    ml_top_factors: list = None,  # Top features driving prediction
    
    # Rule-based context (for stories, not direction)
    rule_score: int = 50,
    whale_pct: float = 50,
    retail_pct: float = 50,
    position_pct: float = 50,
    ...
    ML DICTATES THE FINAL DIRECTION. Rules provide context/stories only.
    
    Signal Priority:
    1. ML Direction → FINAL (LONG/SHORT/WAIT)
    2. Rules → Stories (explain WHY)
    3. Pattern → Confirmation (if aligns)
    
    ═══════════════════════════════════════════════════════════════════════════════
    """
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # STEP 1: ML IS PRIMARY - Set final direction from ML
    # ═══════════════════════════════════════════════════════════════════════════════
    
    final_direction = ml_direction  # ML dictates direction
    final_confidence = ml_confidence
    
    # Determine action based on ML confidence
    if ml_direction == 'LONG':
        if ml_confidence >= 75:
            setup_type = "STRONG_LONG"
            ...
            base_action = "📊 LEAN LONG"
        else:
            setup_type = "WEAK_LONG"
            base_action = "📊 WEAK LONG"
            
    elif ml_direction == 'SHORT':
        if ml_confidence >= 75:
            setup_type = "STRONG_SHORT"
            ...
            base_action = "📊 LEAN SHORT"
        else:
            setup_type = "WEAK_SHORT"
            base_action = "📊 WEAK SHORT"
    else:
        setup_type = "WAIT"
        base_action = "⏳ WAIT"
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # STEP 2: Generate stories from rules (for context, NOT direction)
    # ═══════════════════════════════════════════════════════════════════════════════
    ...
    # Whale story
    if divergence >= 15:
        whale_story = f"🐋 Whales WAY ahead ({whale_pct:.0f}% vs Retail {retail_pct:.0f}%)"
        whale_aligns = (ml_direction == 'LONG')
    elif divergence >= 8:
        whale_story = f"🐋 Whales ahead ({whale_pct:.0f}% vs Retail {retail_pct:.0f}%)"
        whale_aligns = (ml_direction == 'LONG')
    elif divergence <= -15:
        whale_story = f"🚨 Retail TRAPPED ({retail_pct:.0f}% vs Whale {whale_pct:.0f}%)"
        whale_aligns = (ml_direction == 'SHORT')
    elif divergence <= -8:
        whale_story = f"⚠️ Retail ahead ({retail_pct:.0f}% vs Whale {whale_pct:.0f}%)"
        whale_aligns = (ml_direction == 'SHORT')
    else:
        whale_story = f"⚖️ No clear edge (W:{whale_pct:.0f}% vs R:{retail_pct:.0f}%)"
        whale_aligns = (ml_direction == 'WAIT')
    
    # Flow story (OI + Price)
    if oi_change >= 2 and price_change >= 1:
        flow_story = f"📈 New LONGS entering (OI {oi_change:+.1f}%, Price {price_change:+.1f}%)"
        flow_aligns = (ml_direction == 'LONG')
    elif oi_change >= 2 and price_change <= -1:
        flow_story = f"📉 New SHORTS entering (OI {oi_change:+.1f}%, Price {price_change:+.1f}%)"
        flow_aligns = (ml_direction == 'SHORT')
    elif oi_change <= -2 and price_change >= 1:
        flow_story = f"⚠️ Short covering (OI {oi_change:+.1f}%, Price {price_change:+.1f}%) - Don't chase!"
        flow_aligns = (ml_direction == 'WAIT')
    elif oi_change <= -2 and price_change <= -1:
        flow_story = f"📉 Longs exiting (OI {oi_change:+.1f}%, Price {price_change:+.1f}%)"
        flow_aligns = (ml_direction == 'SHORT')
    else:
        flow_story = f"➡️ Neutral flow (OI {oi_change:+.1f}%, Price {price_change:+.1f}%)"
        flow_aligns = True
    
    # Position story
    if position_pct <= 30:
        pos_story = f"🟢 EARLY position ({position_pct:.0f}%) - Good for longs"
        pos_aligns = (ml_direction == 'LONG')
    elif position_pct >= 70:
        pos_story = f"🔴 LATE position ({position_pct:.0f}%) - Good for shorts"
        pos_aligns = (ml_direction == 'SHORT')
    else:
        pos_story = f"⚖️ MIDDLE position ({position_pct:.0f}%)"
        pos_aligns = True
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # STEP 3: Pattern confirmation (optional)
    # ═══════════════════════════════════════════════════════════════════════════════
    
    pattern_aligns = False
    pattern_story = ""
    
    if pattern_detected and pattern_confidence > 0:
        if pattern_direction == 'BULLISH' and ml_direction == 'LONG':
            pattern_aligns = True
            pattern_story = f"✅ Pattern confirms: {pattern_detected} ({pattern_confidence:.0f}%) - ALIGNS"
        elif pattern_direction == 'BEARISH' and ml_direction == 'SHORT':
            pattern_aligns = True
            pattern_story = f"✅ Pattern confirms: {pattern_detected} ({pattern_confidence:.0f}%) - ALIGNS"
        else:
            pattern_story = f"⚠️ Pattern note: {pattern_detected} ({pattern_confidence:.0f}%) - Does not align"
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # STEP 4: Count alignments
    # ═══════════════════════════════════════════════════════════════════════════════
    
    alignments = sum([whale_aligns, flow_aligns, pos_aligns, pattern_aligns])
    total_checks = 3 + (1 if pattern_detected else 0)
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # STEP 5: Build final action string
    # ═══════════════════════════════════════════════════════════════════════════════
    
    if alignments >= 3:
        final_action = f"🎯 {base_action} (IDEAL - {alignments}/{total_checks} align)"
        setup_type = f"IDEAL_{ml_direction}"
    elif alignments >= 2:
        final_action = f"✅ {base_action} ({alignments}/{total_checks} align)"
    elif alignments >= 1:
        final_action = f"📊 {base_action} ({alignments}/{total_checks} align)"
    else:
        final_action = f"⚠️ {base_action} (No alignment - ML only)"
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # STEP 6: Build combined story
    # ═══════════════════════════════════════════════════════════════════════════════
    
    stories = [whale_story, flow_story, pos_story]
    if pattern_story:
        stories.append(pattern_story)
    
    combined_story = " | ".join(stories)
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # STEP 7: Calculate unified score (ML confidence adjusted by alignment)
    # ═══════════════════════════════════════════════════════════════════════════════
    ...
    return {
        # Primary outputs (ML-driven)
        'final_direction': final_direction,  # FROM ML
        'final_score': final_score,
        'final_action': final_action,
        ...
        'ml_top_factors': ml_top_factors or [],
        
        # Rules context (stories only, not direction)
        'whale_story': whale_story,
        'whale_aligns': whale_aligns,
        'flow_story': flow_story,
        'flow_aligns': flow_aligns,
        'pos_story': pos_story,
        'pos_aligns': pos_aligns,
        
        # Pattern confirmation
        'pattern_detected': pattern_detected,
        'pattern_confidence': pattern_confidence,
        'pattern_aligns': pattern_aligns,
        'pattern_story': pattern_story,
        ...
        'engines_agree': whale_aligns and flow_aligns,
        'conflict_note': combined_story,
...
def blend_hybrid_prediction(
    ...
    retail_pct: float = 50,    # Retail long % (0-100) - for divergence detection
    oi_change: float = 0,      # OI change 24h %
    price_change: float = 0,   # Price change 24h % (for flow story)
    historical_win_rate: float = None,
    money_flow_phase: str = None,
    structure: str = None,
    squeeze_pct: float = 50,
    explosion_data: dict = None,
    htf_structure: str = None,
    symbol: str = "",          # Symbol for meme coin detection
) -> dict:
    """
    ═══════════════════════════════════════════════════════════════════════════════
    HOLISTIC MARKET ANALYSIS - ONE STORY, ONE VERDICT
    ═══════════════════════════════════════════════════════════════════════════════
    
    This function tells ONE coherent story by combining:
    
    1. DIVERGENCE STORY: Who has the edge? (Whale% - Retail%)
       - Not "whale 63% = bullish" but "whale 7% ahead of retail = NO EDGE"
       
    2. FLOW STORY: What's happening with money? (OI + Price TOGETHER)
       - Not "OI -2% = bearish" but "OI↓ + Price↓ = Longs EXITING"
       
    3. POSITION FIT: Is entry good for THIS story?
       - Not "35% = middle" but "middle for a bullish story = use limit order"
    
    4. EXPLOSION/SQUEEZE: Is energy loaded for a move?
       - High compression + divergence = explosive potential
    
    The verdict is HOLISTIC - all pieces tell ONE story together.
    ═══════════════════════════════════════════════════════════════════════════════
    """
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # STEP 1: DIVERGENCE STORY - Who has the edge?
    # 
    # The KEY insight: It's not about whale% alone!
    # Whale 63% + Retail 56% = only 7% divergence = NO CLEAR EDGE
    # Whale 75% + Retail 45% = 30% divergence = WHALES WAY AHEAD
    # ═══════════════════════════════════════════════════════════════════════════════
    
    divergence = whale_pct - retail_pct
    
    if divergence >= 15:
        div_story = "WHALES_WAY_AHEAD"
        div_edge = "STRONG_BULLISH"
        div_desc = f"Whales {whale_pct:.0f}% vs Retail {retail_pct:.0f}% (+{divergence:.0f}%) → Whales WAY ahead"
    elif divergence >= 8:
        div_story = "WHALES_AHEAD"
        div_edge = "LEAN_BULLISH"
        div_desc = f"Whales {whale_pct:.0f}% vs Retail {retail_pct:.0f}% (+{divergence:.0f}%) → Whales ahead"
    elif divergence <= -15:
        div_story = "RETAIL_TRAPPED"
        div_edge = "STRONG_BEARISH"
        div_desc = f"Retail {retail_pct:.0f}% vs Whales {whale_pct:.0f}% ({divergence:.0f}%) → Retail TRAPPED!"
    elif divergence <= -8:
        div_story = "RETAIL_AHEAD"
        div_edge = "LEAN_BEARISH"
        div_desc = f"Retail {retail_pct:.0f}% vs Whales {whale_pct:.0f}% ({divergence:.0f}%) → Retail ahead (trap risk)"
    else:
        div_story = "NO_CLEAR_EDGE"
        div_edge = "NEUTRAL"
        div_desc = f"W:{whale_pct:.0f}% vs R:{retail_pct:.0f}% ({divergence:+.0f}%) → NO CLEAR EDGE"
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # STEP 2: FLOW STORY - What's happening with money?
    #
    # OI and Price TOGETHER tell the flow story, BUT must consider WHALE CONTEXT!
    # 
    # Traditional interpretation:
    # - OI↑ + Price↑ = New LONGS entering (bullish continuation)
    # - OI↑ + Price↓ = New SHORTS entering (bearish continuation)
    # - OI↓ + Price↑ = Shorts COVERING (FAKE rally - don't chase!)
    # - OI↓ + Price↓ = Longs EXITING (bearish, unless capitulation at lows)
    #
    # BUT with whale context:
    # - OI↑ + Price↓ + Whale >= 65% = WHALE ACCUMULATION (buying the dip!)
    # - OI↑ + Price↑ + Whale <= 40% = Possible SHORT SQUEEZE
    # ═══════════════════════════════════════════════════════════════════════════════
    
    oi_up = oi_change >= 2.0
    oi_down = oi_change <= -2.0
    price_up = price_change >= 1.0
    price_down = price_change <= -1.0
    
    # Check whale context for smarter flow interpretation
    whale_heavily_long = whale_pct >= 65
    whale_heavily_short = whale_pct <= 35
    
    if oi_up and price_up:
        if whale_heavily_short:
            # Whales are short but price rising with new positions - possible squeeze
            flow_story = "POSSIBLE_SQUEEZE"
            flow_dir = "CAUTIOUS_BULLISH"
            flow_desc = f"OI {oi_change:+.1f}% + Price {price_change:+.1f}% → Possible short squeeze (whale only {whale_pct:.0f}%)"
        else:
            flow_story = "NEW_LONGS"
            flow_dir = "BULLISH"
            flow_desc = f"OI {oi_change:+.1f}% + Price {price_change:+.1f}% → New LONGS entering"
    elif oi_up and price_down:
        if whale_heavily_long:
            # KEY FIX: Whales are heavily long, OI up + price down = ACCUMULATION not shorts!
            flow_story = "WHALE_ACCUMULATION"
            flow_dir = "BULLISH"
            flow_desc = f"OI {oi_change:+.1f}% + Price {price_change:+.1f}% → WHALE ACCUMULATION (buying dip!)"
        else:
            flow_story = "NEW_SHORTS"
            flow_dir = "BEARISH"
            flow_desc = f"OI {oi_change:+.1f}% + Price {price_change:+.1f}% → New SHORTS entering"
    elif oi_down and price_up:
        flow_story = "SHORT_COVERING"
        flow_dir = "FAKE_BULLISH"
        flow_desc = f"OI {oi_change:+.1f}% + Price {price_change:+.1f}% → SHORT COVERING (fake rally!)"
    elif oi_down and price_down:
        if whale_heavily_long:
            # Whales long but OI dropping + price down = whales might be exiting!
            flow_story = "WHALE_EXIT_WARNING"
            flow_dir = "BEARISH"
            flow_desc = f"OI {oi_change:+.1f}% + Price {price_change:+.1f}% → ⚠️ WHALE EXIT WARNING!"
        else:
            flow_story = "LONGS_EXITING"
            flow_dir = "BEARISH"
            flow_desc = f"OI {oi_change:+.1f}% + Price {price_change:+.1f}% → LONGS EXITING"
    else:
        flow_story = "NEUTRAL"
        flow_dir = "NEUTRAL"
        flow_desc = f"OI {oi_change:+.1f}% + Price {price_change:+.1f}% → Neutral flow"
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # STEP 3: POSITION FIT - Is entry good for THIS story?
    #
    # Position only matters RELATIVE to the story:
    # - EARLY (0-30%) = Good for LONGS, risky for shorts
    # - LATE (70-100%) = Good for SHORTS, risky for longs
    # - MIDDLE (30-70%) = Wait or use limit order
    # ═══════════════════════════════════════════════════════════════════════════════
    
    if position_pct <= 30:
        pos_zone = "EARLY"
        pos_desc = f"Position {position_pct:.0f}% (near LOWS)"
    elif position_pct >= 70:
        pos_zone = "LATE"
        pos_desc = f"Position {position_pct:.0f}% (near HIGHS)"
    else:
        pos_zone = "MIDDLE"
        pos_desc = f"Position {position_pct:.0f}% (MID-RANGE)"
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # STEP 4: EXPLOSION/SQUEEZE CHECK - Is energy loaded?
    # ═══════════════════════════════════════════════════════════════════════════════
    
    exp_score = explosion_data.get('score', 0) if explosion_data else 0
    exp_ready = explosion_data.get('ready', False) if explosion_data else False
    # squeeze_pct now uses HIGH = compressed convention
    bb_loaded = squeeze_pct >= 75  # High percentile = bands are tight = LOADED
    energy_loaded = exp_ready or exp_score >= 60 or bb_loaded
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # STEP 5: HOLISTIC VERDICT - All stories combine to ONE conclusion
    # 
    # This is the KEY: We don't check individual thresholds.
    # We ask: "What is the COMPLETE PICTURE telling us?"
    # ═══════════════════════════════════════════════════════════════════════════════
    ...
    setup_type = "NO_TRADE"
    tp_multiplier = 1.0
    limit_entry_suggested = False
    conflict_note = ""
    
    # Track if ML and Rules normalized directions
    rule_is_bullish = rule_direction in ['BULLISH', 'LEAN_BULLISH', 'WEAK_BULLISH', 'TA_BULLISH']
    rule_is_bearish = rule_direction in ['BEARISH', 'LEAN_BEARISH', 'WEAK_BEARISH', 'TA_BEARISH']
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # ML CONFIDENCE CHECK - Low confidence = ML is UNCERTAIN, don't trust it!
    # 
    # 55% confidence means "I've seen mixed results" = basically a coin flip
    # Only trust ML when it's CONFIDENT (>= 65%)
    # HIGH CONFIDENCE (>= 85%) = ML override rules (professional approach)
    # ═══════════════════════════════════════════════════════════════════════════════
    ML_CONFIDENCE_THRESHOLD = 65  # Below this, ML is considered UNCERTAIN
    ML_HIGH_CONFIDENCE_THRESHOLD = 85  # Above this, ML overrides rules
    
    ml_is_confident = ml_confidence >= ML_CONFIDENCE_THRESHOLD
    ml_is_high_confidence = ml_confidence >= ML_HIGH_CONFIDENCE_THRESHOLD
    ml_is_uncertain = not ml_is_confident
    
    if ml_is_confident:
        ml_is_bullish = ml_direction == 'LONG'
        ml_is_bearish = ml_direction == 'SHORT'
    else:
        # ML confidence too low - treat as NEUTRAL (uncertain)
        ml_is_bullish = False
        ml_is_bearish = False
    
    engines_agree = (rule_is_bullish and ml_is_bullish) or (rule_is_bearish and ml_is_bearish)
    
    ml_score = int(ml_confidence)
    
    # ───────────────────────────────────────────────────────────────────────────────
    # VERDICT LOGIC: Based on COMBINED stories, not individual parameters
    # ───────────────────────────────────────────────────────────────────────────────
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # RULE 1: NO DIVERGENCE EDGE → WAIT (regardless of ML/TA)
    # If whales and retail are similarly positioned, there's no smart money edge
    # ═══════════════════════════════════════════════════════════════════════════════
    if div_story == "NO_CLEAR_EDGE":
        final_direction = "WAIT"
        final_score = 35
        setup_type = "NO_EDGE"
        conflict_note = f"🟡 NO EDGE: {div_desc}. No informational advantage."
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # RULE 2: BULLISH DIVERGENCE STORIES
    # Whales ahead of retail - but FLOW must confirm!
    # ═══════════════════════════════════════════════════════════════════════════════
    elif div_edge in ["STRONG_BULLISH", "LEAN_BULLISH"]:
        
        # 2A: Bullish divergence BUT money leaving → WHALE EXIT!
        if flow_story == "LONGS_EXITING":
            final_direction = "WAIT"
            final_score = 30
            setup_type = "WHALE_EXIT_WARNING"
            conflict_note = f"🚨 WHALE EXIT: {div_desc} BUT {flow_desc}. Whales may be closing!"
        
        # 2B: Bullish divergence BUT short covering → DON'T CHASE
        elif flow_story == "SHORT_COVERING":
            final_direction = "WAIT"
            final_score = 35
            setup_type = "DONT_CHASE_RALLY"
            conflict_note = f"⚠️ FAKE RALLY: {div_desc} but {flow_desc}. Don't chase!"
        
        # 2C: Bullish divergence + Bullish flow + EARLY → IDEAL LONG ✅
        elif flow_dir == "BULLISH" and pos_zone == "EARLY":
            final_direction = "LONG"
            final_score = 78 if div_edge == "STRONG_BULLISH" else 68
            setup_type = "IDEAL_LONG"
            tp_multiplier = 1.5 if energy_loaded else 1.3
            conflict_note = f"🟢 IDEAL LONG: {div_desc} + {flow_desc} + {pos_desc}"
        
        # 2D: Bullish divergence + Bullish flow + MIDDLE → LONG with caution
        elif flow_dir == "BULLISH" and pos_zone == "MIDDLE":
            final_direction = "LONG"
            final_score = 62 if div_edge == "STRONG_BULLISH" else 55
            setup_type = "LONG_MID"
            limit_entry_suggested = True
            tp_multiplier = 1.2
            conflict_note = f"🟢 LONG (limit order): {div_desc} + {flow_desc} + {pos_desc}"
        
        # 2E: Bullish divergence + Bullish flow + LATE → DON'T CHASE
        elif flow_dir == "BULLISH" and pos_zone == "LATE":
            final_direction = "WAIT"
            final_score = 40
            setup_type = "DONT_CHASE_LATE"
            limit_entry_suggested = True
            conflict_note = f"⚠️ DON'T CHASE: {div_desc} but {pos_desc}. Wait for pullback!"
        
        # 2F: Bullish divergence + Neutral flow + Good position → LEAN LONG
        elif flow_dir == "NEUTRAL" and pos_zone in ["EARLY", "MIDDLE"]:
            final_direction = "LONG"
            final_score = 55 if div_edge == "STRONG_BULLISH" else 50
            setup_type = "LEAN_LONG"
            conflict_note = f"📊 LEAN LONG: {div_desc} + {flow_desc} + {pos_desc}"
        
        # 2G: Bullish divergence + EARLY + Energy loaded → PREDICTIVE LONG
        elif pos_zone == "EARLY" and energy_loaded:
            final_direction = "LONG"
            final_score = 65 if div_edge == "STRONG_BULLISH" else 58
            setup_type = "PREDICTIVE_LONG"
            tp_multiplier = 1.4
            conflict_note = f"🚀 PREDICTIVE LONG: {div_desc} + {pos_desc} + Energy loaded ({exp_score}/100)"
        
        # 2H: Bullish divergence but bearish flow 
        # IMPORTANT: When whales are WAY AHEAD (+15%+) and new shorts enter,
        # those shorts are likely RETAIL betting against whales = RETAIL TRAP
        elif flow_dir == "BEARISH":
            if div_edge == "STRONG_BULLISH" and flow_story == "NEW_SHORTS":
                # Strong bullish divergence + New shorts = RETAIL TRAP setup!
                # Retail is shorting against whales - this is GOOD for longs
                final_direction = "LONG"
                final_score = 68 if pos_zone == "EARLY" else 60 if pos_zone == "MIDDLE" else 50
                setup_type = "RETAIL_TRAP_LONG"
                tp_multiplier = 1.4 if energy_loaded else 1.2
                limit_entry_suggested = True if pos_zone == "MIDDLE" else False
                conflict_note = f"🎯 RETAIL TRAP: {div_desc}. New shorts entering against whales = potential squeeze. {pos_desc}"
            else:
                # Weaker divergence or different flow - more cautious
                final_direction = "WAIT"
                final_score = 42
                setup_type = "DIVERGENCE_CONFLICT"
                conflict_note = f"⚠️ MIXED: {div_desc} vs {flow_desc}. Wait for clarity."
        
        # 2I: Default bullish - some edge but not ideal
        else:
            final_direction = "LONG" if pos_zone != "LATE" else "WAIT"
            final_score = 50
            setup_type = "WEAK_LONG"
            conflict_note = f"📊 Weak LONG: {div_desc}. Not ideal setup."
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # RULE 3: BEARISH DIVERGENCE STORIES
    # Retail ahead of whales (trapped) - but FLOW must confirm!
    # ═══════════════════════════════════════════════════════════════════════════════
    elif div_edge in ["STRONG_BEARISH", "LEAN_BEARISH"]:
        
        # 3A: Bearish divergence + Bearish flow + LATE → IDEAL SHORT ✅
        if flow_dir == "BEARISH" and pos_zone == "LATE":
            final_direction = "SHORT"
            final_score = 78 if div_edge == "STRONG_BEARISH" else 68
            setup_type = "IDEAL_SHORT"
            tp_multiplier = 1.5 if energy_loaded else 1.3
            conflict_note = f"🔴 IDEAL SHORT: {div_desc} + {flow_desc} + {pos_desc}"
        
        # 3B: Bearish divergence + Bearish flow + MIDDLE → SHORT with caution
        elif flow_dir == "BEARISH" and pos_zone == "MIDDLE":
            final_direction = "SHORT"
            final_score = 62 if div_edge == "STRONG_BEARISH" else 55
            setup_type = "SHORT_MID"
            limit_entry_suggested = True
            tp_multiplier = 1.2
            conflict_note = f"🔴 SHORT (limit order): {div_desc} + {flow_desc} + {pos_desc}"
        
        # 3C: Bearish divergence + Bearish flow + EARLY → RISKY SHORT
        elif flow_dir == "BEARISH" and pos_zone == "EARLY":
            final_direction = "WAIT"
            final_score = 40
            setup_type = "RISKY_SHORT_LOWS"
            conflict_note = f"⚠️ RISKY: {div_desc} + {flow_desc} but {pos_desc}. Don't short lows!"
        
        # 3D: Bearish divergence + Fake bullish (short covering) → WAIT then SHORT
        elif flow_story == "SHORT_COVERING":
            final_direction = "WAIT"
            final_score = 42
            setup_type = "WAIT_FOR_SHORT"
            limit_entry_suggested = True
            conflict_note = f"⏳ WAIT TO SHORT: {div_desc}. {flow_desc} - wait for rally to exhaust."
        
        # 3E: Bearish divergence + Neutral flow + Good position → LEAN SHORT
        elif flow_dir == "NEUTRAL" and pos_zone in ["LATE", "MIDDLE"]:
            final_direction = "SHORT"
            final_score = 55 if div_edge == "STRONG_BEARISH" else 50
            setup_type = "LEAN_SHORT"
            conflict_note = f"📊 LEAN SHORT: {div_desc} + {flow_desc} + {pos_desc}"
        
        # 3F: Bearish divergence + LATE + Energy loaded → PREDICTIVE SHORT
        elif pos_zone == "LATE" and energy_loaded:
            final_direction = "SHORT"
            final_score = 65 if div_edge == "STRONG_BEARISH" else 58
            setup_type = "PREDICTIVE_SHORT"
            tp_multiplier = 1.4
            conflict_note = f"🚀 PREDICTIVE SHORT: {div_desc} + {pos_desc} + Energy loaded ({exp_score}/100)"
        
        # 3G: Bearish divergence but bullish flow
        # IMPORTANT: When retail is trapped and new longs enter,
        # those longs are likely RETAIL buying against bearish whales = BULL TRAP
        elif flow_dir == "BULLISH":
            if div_edge == "STRONG_BEARISH" and flow_story == "NEW_LONGS":
                # Strong bearish divergence + New longs = BULL TRAP setup!
                # Retail is buying against whales shorting - good for shorts
                final_direction = "SHORT"
                final_score = 68 if pos_zone == "LATE" else 60 if pos_zone == "MIDDLE" else 50
                setup_type = "BULL_TRAP_SHORT"
                tp_multiplier = 1.4 if energy_loaded else 1.2
                limit_entry_suggested = True if pos_zone == "MIDDLE" else False
                conflict_note = f"🎯 BULL TRAP: {div_desc}. New longs entering against bearish whales = potential dump. {pos_desc}"
            else:
                # Weaker divergence or different flow - more cautious
                final_direction = "WAIT"
                final_score = 42
        ...
        # 3H: Default bearish - some edge but not ideal
        else:
            final_direction = "SHORT" if pos_zone != "EARLY" else "WAIT"
            final_score = 50
            setup_type = "WEAK_SHORT"
            conflict_note = f"📊 Weak SHORT: {div_desc}. Not ideal setup."
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # RULE 4: DEFAULT - No clear story
    # ═══════════════════════════════════════════════════════════════════════════════
    else:
        final_direction = "WAIT"
        final_score = 35
        setup_type = "NO_STORY"
        conflict_note = f"🟡 NO CLEAR STORY: {div_desc} + {flow_desc}"
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # STEP 6: SCORE ADJUSTMENTS based on confirmations
    # ═══════════════════════════════════════════════════════════════════════════════
    
    # Boost if ML CONFIDENTLY agrees with our holistic verdict
    if ml_is_confident:
        if final_direction == "LONG" and ml_is_bullish:
            final_score += 10
            conflict_note += f" | ML confirms ({ml_confidence:.0f}%) ✅"
        elif final_direction == "SHORT" and ml_is_bearish:
            final_score += 10
            conflict_note += f" | ML confirms ({ml_confidence:.0f}%) ✅"
        elif final_direction == "LONG" and ml_is_bearish:
            # ML confidently disagrees - add warning but DON'T override IDEAL setups
            if setup_type not in ["IDEAL_LONG", "IDEAL_SHORT"]:
                final_score -= 5
            conflict_note += f" | ⚠️ ML disagrees ({ml_confidence:.0f}% {ml_direction})"
        elif final_direction == "SHORT" and ml_is_bullish:
            if setup_type not in ["IDEAL_LONG", "IDEAL_SHORT"]:
                final_score -= 5
            conflict_note += f" | ⚠️ ML disagrees ({ml_confidence:.0f}% {ml_direction})"
    else:
        # ML is UNCERTAIN (< 65% confidence) - note it but don't let it influence
        conflict_note += f" | ML uncertain ({ml_confidence:.0f}%)"
    
    # Boost if historical win rate is good
    if historical_win_rate is not None and historical_win_rate >= 60:
        final_score += 5
        conflict_note += f" | History: {historical_win_rate:.0f}% ✅"
    elif historical_win_rate is not None and historical_win_rate <= 35:
        final_score -= 10
        conflict_note += f" | History: {historical_win_rate:.0f}% ⚠️"
    
    # Boost for energy/explosion
    if energy_loaded and final_direction in ["LONG", "SHORT"]:
        final_score += 5
    
    # ═══════════════════════════════════════════════════════════════════════════════
    # MEME COIN PENALTY - Higher manipulation risk
    # ═══════════════════════════════════════════════════════════════════════════════
    is_meme_coin_flag = False
    if symbol:
        try:
            from .MASTER_RULES import is_meme_coin
            meme_check = is_meme_coin(symbol)
            if meme_check['is_meme']:
                final_score -= 15
                is_meme_coin_flag = True
    ...
    # ═══════════════════════════════════════════════════════════════════════════════
    # STEP 7: BUILD FINAL ACTION STRING
    # ═══════════════════════════════════════════════════════════════════════════════
    
    base_action = get_action_from_score(final_score, final_direction)
    
    if setup_type in ["IDEAL_LONG", "IDEAL_SHORT"]:
        final_action = f"🎯 {base_action} (IDEAL)"
    elif setup_type in ["PREDICTIVE_LONG", "PREDICTIVE_SHORT"]:
        final_action = f"🚀 {base_action} (PREDICTIVE)"
    elif setup_type == "RETAIL_TRAP_LONG":
        final_action = f"🪤 {base_action} (Retail Trap)"
    elif setup_type == "BULL_TRAP_SHORT":
        final_action = f"🪤 {base_action} (Bull Trap)"
    elif "WHALE_EXIT" in setup_type:
        final_action = f"🚨 WAIT (Whale Exit Warning)"
    elif "NO_EDGE" in setup_type:
        final_action = f"🟡 WAIT (No Edge - W:{whale_pct:.0f}% vs R:{retail_pct:.0f}%)"
    elif "DONT_CHASE" in setup_type:
        final_action = f"⚠️ WAIT (Don't Chase)"
    elif "CONFLICT" in setup_type:
        final_action = f"⚠️ WAIT (Story Conflict)"
    elif engines_agree and final_direction != "WAIT":
        final_action = f"⚡ {base_action} (ML agrees)"
    elif limit_entry_suggested:
        final_action = f"📊 {base_action} (use limit)"
    else:
        final_action = f"{base_action}"
    ...
    return {
        ...
        'rule_score': rule_score,
        'ml_score': ml_score,
        'ml_is_confident': ml_is_confident,  # NEW: Is ML confident enough to trust?
        'ml_is_uncertain': ml_is_uncertain,  # NEW: Is ML uncertain (< 65%)?
        'position_override': final_direction == "WAIT",
        'setup_type': setup_type,
        'tp_multiplier': tp_multiplier,
        'limit_entry_suggested': limit_entry_suggested,
        'whale_retail_spread': divergence,
        'is_meme_coin': is_meme_coin_flag,   # Meme coin detected
        # NEW: Holistic story data for display
        'divergence_story': div_story,
        'divergence_desc': div_desc,
        'flow_story': flow_story,
        'flow_desc': flow_desc,
        'position_zone': pos_zone,
        'position_desc': pos_desc,
        'holistic_verdict': conflict_note,
    }